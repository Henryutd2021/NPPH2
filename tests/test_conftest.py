...
def sample_hourly_data_for_iso(request):
    ...
    target_iso = getattr(request, "param", {}).get("target_iso", DEFAULT_TARGET_ISO)
    ...
    selected_as_cols = as_price_cols.get(target_iso, [f'p_RegU_{target_iso}', f'loc_RegU_{target_iso}']) # Default if ISO not in map

    for i, col_name in enumerate(selected_as_cols):
        price_data_dict[col_name] = [(10.0 + i*2 + j % 3) for j in range(num_hours)]
    ...
    selected_factor_cols = factor_cols_map.get(target_iso, [f'deploy_factor_RegU_{target_iso}'])

    ans_mile_data = {col: [1.0 - (i*0.01) % 0.1 for i in range(num_hours)] for col in selected_factor_cols if 'mileage' in col or 'score' in col or 'ratio' in col}
    if not ans_mile_data: ans_mile_data = {'hour': range(1,num_hours+1)} # ensure dataframe not empty
    else: ans_mile_data['hour'] = range(1,num_hours+1)


    ans_deploy_data = {col: [0.5 + (i*0.02) % 0.2 for i in range(num_hours)] for col in selected_factor_cols if 'deploy_factor' in col}
    if not ans_deploy_data: ans_deploy_data = {'hour': range(1,num_hours+1)}
    else: ans_deploy_data['hour'] = range(1,num_hours+1)

    ans_winrate_data = {col: [0.8 + (i*0.01) % 0.15 for i in range(num_hours)] for col in selected_factor_cols if 'winning_rate' in col}
    if not ans_winrate_data: ans_winrate_data = {'hour': range(1,num_hours+1)}
    else: ans_winrate_data['hour'] = range(1,num_hours+1)


    return {
        "df_price_hourly": pd.DataFrame(price_data_dict),
        "df_ANSprice_hourly": pd.DataFrame(price_data_dict), # Assuming AS prices are in the same file for simplicity here
...
def sample_system_data():
    ...
    values = [
        ...
        '0,50,100,150', '0.025,0.024,0.023', '0.010,0.009,0.008', # p_bp_HTE, ke_HTE, kt_HTE (MWh/kg, MWth/kg)
    ...
    if len(parameters) != len(values):
        raise ValueError(f"Parameter list length ({len(parameters)}) and value list length ({len(values)}) mismatch in test_conftest.py")
...
def sample_model_config(request):
    ...
    for key, value in current_config.items():
        try:
            ...
            print(f"Warning: Could not patch config.{key}, it might not exist directly or needs module path.")
...
def sample_model(sample_hourly_data_for_iso, sample_system_data, sample_model_config):
    ...
    data_inputs = {**sample_hourly_data_for_iso, "df_system": sample_system_data}
