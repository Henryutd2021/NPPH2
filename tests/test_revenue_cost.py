...
class TestRevenueCost(unittest.TestCase):
    ...
    def _create_base_data_inputs(self, iso="ERCOT", num_hours=3):
        ...
        parameters = [
            ...
            # Added qSteam_Turbine_max_MWth
            'pTurbine_max_MW', 'qSteam_Total_MWth', 'qSteam_Turbine_min_MWth', 'qSteam_Turbine_max_MWth',
        ...
        if len(parameters) != len(values):
            raise ValueError(
                f"Mismatch in test_revenue_cost setup parameter/value lengths ({len(parameters)} vs {len(values)}).")
        ...
        with patch.multiple('config', **effective_flags):
            ...
            for t_idx, t in enumerate(model.TimePeriods):
                ...
                if hasattr(model, 'pIES'):
                    model.pIES[t].value = 10 * t_val
                if hasattr(model, 'mHydrogenProduced') and model.mHydrogenProduced.is_indexed():
                    model.mHydrogenProduced[t].value = 5 * t_val
                if hasattr(model, 'H2_to_market') and model.H2_to_market.is_indexed():
                    model.H2_to_market[t].value = 2 * t_val
                if hasattr(model, 'H2_from_storage') and model.H2_from_storage.is_indexed():
                    model.H2_from_storage[t].value = 1 * t_val

                if hasattr(model, 'pTurbine') and model.pTurbine.is_indexed():
                    model.pTurbine[t].value = 20 * t_val
                if hasattr(model, 'pElectrolyzer') and model.pElectrolyzer.is_indexed():
                    model.pElectrolyzer[t].value = 15 * t_val  # Actual power
                if hasattr(model, 'pElectrolyzerSetpoint') and model.pElectrolyzerSetpoint.is_indexed():
                    # Setpoint
                    model.pElectrolyzerSetpoint[t].value = 15 * t_val

                if hasattr(model, 'BatteryCharge') and model.BatteryCharge.is_indexed():
                    model.BatteryCharge[t].value = 3 * t_val
                if hasattr(model, 'BatteryDischarge') and model.BatteryDischarge.is_indexed():
                    model.BatteryDischarge[t].value = 2 * t_val
                ...
                if hasattr(model, 'H2_to_storage') and model.H2_to_storage.is_indexed():
                ...
                if hasattr(model, 'vElectrolyzerStartup') and model.vElectrolyzerStartup.is_indexed():
                    model.vElectrolyzerStartup[t].value = 1 if t_idx == 0 else 0

                # AS Bids and Deployed (example for RegUp ERCOT/SPP)
                if model.CAN_PROVIDE_ANCILLARY_SERVICES:
                    # Could be 'Spin' for SPP reserves etc.
                    service_key = 'RegU'
                    # Bid variables
                    total_var = getattr(model, f'Total_{service_key}', None)
                    if total_var is not None and total_var.is_indexed():
                        total_var[t].value = 2.0 * t_val

                    elec_bid_var = getattr(
                        model, f'{service_key}_Electrolyzer', None)
                    if elec_bid_var is not None and elec_bid_var.is_indexed():
                        elec_bid_var[t].value = 1.0 * t_val

                    # Example for SPP Spin (Internal Name: SR)
                    spin_key = 'SR'
                    total_spin_var = getattr(model, f'Total_{spin_key}', None)
                    if total_spin_var is not None and total_spin_var.is_indexed():
                        total_spin_var[t].value = 3.0 * t_val
                    elec_spin_var = getattr(
                        model, f'{spin_key}_Electrolyzer', None)
                    if elec_spin_var is not None and elec_spin_var.is_indexed():
                        elec_spin_var[t].value = 1.5*t_val

                    # Deployed variables (only if simulating dispatch)
                    if simulate_dispatch:
                        elec_dep_var = getattr(
                            model, f'{service_key}_Electrolyzer_Deployed', None)
                        if elec_dep_var is not None and elec_dep_var.is_indexed() and elec_bid_var is not None and elec_bid_var.is_indexed():
                            win_rate_param = getattr(
                                model, f"winning_rate_{service_key}_{target_iso}", None)
                            win_rate_val = pyo.value(win_rate_param[t]) if win_rate_param and win_rate_param.is_indexed(
                            ) and t in win_rate_param else 1.0
                            elec_dep_var[t].value = elec_bid_var[t].value * \
                                win_rate_val

                        elec_spin_dep_var = getattr(
                            model, f'{spin_key}_Electrolyzer_Deployed', None)
                        if elec_spin_dep_var is not None and elec_spin_dep_var.is_indexed() and elec_spin_var is not None and elec_spin_var.is_indexed():
                            ...
                            win_rate_param_sp = getattr(
                                model, f"winning_rate_{param_iso_spin_name}_{target_iso}", None)
                            win_rate_val_sp = pyo.value(win_rate_param_sp[t]) if win_rate_param_sp and win_rate_param_sp.is_indexed(
                            ) and t in win_rate_param_sp else 1.0
                            deploy_factor_param_sp = getattr(
                                model, f"deploy_factor_{param_iso_spin_name}_{target_iso}", None)
                            deploy_factor_val_sp = pyo.value(deploy_factor_param_sp[t]) if deploy_factor_param_sp and deploy_factor_param_sp.is_indexed(
                            ) and t in deploy_factor_param_sp else 0.0
                            elec_spin_dep_var[t].value = elec_spin_var[t].value * \
                                win_rate_val_sp * deploy_factor_val_sp
    ...
    def test_hydrogen_revenue_calculation_with_storage_and_subsidy(self):
        ...
        model = self._setup_model_and_fix_vars(
            num_hours=num_hours, ENABLE_H2_STORAGE=True)
        ...
        expected_sales_rev = (
            model.H2_to_market[t1].value + model.H2_from_storage[t1].value) * pyo.value(model.H2_value) * 1.0
        expected_subsidy_rev = model.mHydrogenProduced[t1].value * pyo.value(
            model.hydrogen_subsidy_per_kg) * 1.0
        expected_total_revenue = expected_sales_rev + expected_subsidy_rev
        actual_revenue = pyo.value(HydrogenRevenue_rule(model))
        self.assertAlmostEqual(
            actual_revenue, expected_total_revenue, places=2)

    def test_hydrogen_revenue_calculation_no_storage(self):
        """Test HydrogenRevenue_rule without storage but with subsidy."""
        num_hours = 1
        model = self._setup_model_and_fix_vars(
            num_hours=num_hours, ENABLE_H2_STORAGE=False)
        t1 = model.TimePeriods.first()
    ...
    def test_opex_cost_calculation(self):
        ...
        num_hours = 1
        model = self._setup_model_and_fix_vars(
            num_hours=num_hours, ENABLE_STARTUP_SHUTDOWN=True, ENABLE_H2_STORAGE=True)  # All components on
        t1 = model.TimePeriods.first()
        ...
        cost_vom_turbine = model.pTurbine[t1].value * \
            pyo.value(model.vom_turbine) * time_factor
        cost_vom_electrolyzer = model.pElectrolyzer[t1].value * pyo.value(
            model.vom_electrolyzer_LTE) * time_factor
        cost_vom_battery = (model.BatteryCharge[t1].value + model.BatteryDischarge[t1].value) * pyo.value(
            model.vom_battery_per_mwh_cycled) * time_factor
        cost_water = model.mHydrogenProduced[t1].value * \
            pyo.value(model.cost_water_per_kg_h2) * time_factor
        cost_ramping = (model.pElectrolyzerRampPos[t1].value + 0) * pyo.value(
            model.cost_electrolyzer_ramping_LTE)  # RampNeg is 0
        cost_storage_cycle = (model.H2_to_storage[t1].value + model.H2_from_storage[t1].value) * pyo.value(
            model.vom_storage_cycle) * time_factor
        cost_startup = model.vElectrolyzerStartup[t1].value * \
            pyo.value(model.cost_startup_electrolyzer_LTE)
    ...
    def test_ancillary_revenue_ercot_bidding_mode(self):
        ...
        win_regu_t1 = pyo.value(
            getattr(model, f'winning_rate_RegU_{iso}')[t1])  # 0.9
        ...
        expected_regu_rev_t1 = ((bid_regu_t1 * win_regu_t1 * p_regu_t1) +      # Capacity: Bid * WinRate * MCP_Cap
                                # Performance: Bid * Mileage * Perf * LMP
                                (bid_regu_t1 * 1.0 * 1.0 * lmp_t1) +
                                (loc_regu_t1)                      # Adder
                                ) * time_factor
        ...
        if total_sr_var is not None and total_sr_var.is_indexed():
            for t_sr in model.TimePeriods:
                total_sr_var[t_sr].fix(0)  # Fix other bids to 0
    ...
    def test_ancillary_revenue_spp_dispatch_mode(self):
        ...
        with patch('revenue_cost.get_total_deployed_as') as mock_get_total_deployed:
            ...
            mock_get_total_deployed.side_effect = lambda m, t, service_name: \
                ...
                (deployed_spin_val if service_name == 'SR' and t ==
                 t1 else 0.0)  # Match internal name SR for Spin
            ...
            expected_regu_rev_t1 = (
                cap_regu + perf_regu + adder_regu) * time_factor
            ...
            expected_spin_rev_t1 = (
                cap_spin + energy_spin + adder_spin) * time_factor
