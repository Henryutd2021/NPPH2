...
class TestConstraints(unittest.TestCase):
    ...
    def _create_base_data_inputs(self, iso="ERCOT"):
        ...
        as_price_cols = [f'p_RegU_{iso}', f'p_RegD_{iso}', f'p_SR_{iso}'] # Example for ERCOT
        ...
        if len(parameters) != len(values):
            raise ValueError(f"Mismatch in test_constraints setup parameter/value lengths ({len(parameters)} vs {len(values)}).")
    ...
    def _setup_model_with_config(self, target_iso="ERCOT", simulate_dispatch=False, **kwargs_config_flags):
        ...
        with patch.multiple('config', **effective_flags):
            ...
            importlib.reload(app_config) # Reload to pick up patched values if config is structured that way
    ...
    def test_steam_balance_constraint(self):
        ...
        # HTE mode (default)
        model_hte = self._setup_model_with_config(ENABLE_NUCLEAR_GENERATOR=True, ENABLE_ELECTROLYZER=True, ENABLE_LOW_TEMP_ELECTROLYZER=False)
        self.assertTrue(hasattr(model_hte, 'steam_balance_constr'), "Steam balance constraint missing for HTE")
        self.assertTrue(model_hte.steam_balance_constr[1].active, "Steam balance should be active for HTE")

        # LTE mode
        model_lte = self._setup_model_with_config(ENABLE_NUCLEAR_GENERATOR=True, ENABLE_ELECTROLYZER=True, ENABLE_LOW_TEMP_ELECTROLYZER=True)
        self.assertTrue(hasattr(model_lte, 'steam_balance_constr'), "Steam balance constraint missing for LTE")
        self.assertTrue(model_lte.steam_balance_constr[1].active, "Steam balance should be active for LTE (even if qSteam_Electrolyzer is 0)")
        ...
        # No electrolyzer
        model_no_elec = self._setup_model_with_config(ENABLE_NUCLEAR_GENERATOR=True, ENABLE_ELECTROLYZER=False)
        self.assertTrue(hasattr(model_no_elec, 'steam_balance_constr'), "Steam balance constraint missing")
        self.assertTrue(model_no_elec.steam_balance_constr[1].active, "Steam balance should be active")


    def test_power_balance_constraint(self):
        """Test power_balance_rule."""
        model = self._setup_model_with_config(ENABLE_NUCLEAR_GENERATOR=True, ENABLE_ELECTROLYZER=True, ENABLE_BATTERY=True)
    ...
    def test_constant_turbine_power_constraint(self):
        ...
        model_lte_on = self._setup_model_with_config(ENABLE_NUCLEAR_GENERATOR=True, ENABLE_ELECTROLYZER=True, ENABLE_LOW_TEMP_ELECTROLYZER=True)
        ...
        model_hte = self._setup_model_with_config(ENABLE_NUCLEAR_GENERATOR=True, ENABLE_ELECTROLYZER=True, ENABLE_LOW_TEMP_ELECTROLYZER=False)
        self.assertFalse(hasattr(model_hte, 'const_turbine_power_constr'), "Constant turbine power constraint should not exist for HTE")
    ...
    def test_electrolyzer_ramp_constraints(self):
        ...
        self.assertEqual(model.Electrolyzer_RampUp_constr[1], pyo.Constraint.Skip)
        self.assertEqual(model.Electrolyzer_RampDown_constr[1], pyo.Constraint.Skip)
    ...
    def test_h2_storage_constraints(self):
        ...
        model = self._setup_model_with_config(ENABLE_ELECTROLYZER=True, ENABLE_H2_STORAGE=True)
    ...
    def test_startup_shutdown_constraints(self):
        ...
        model = self._setup_model_with_config(ENABLE_ELECTROLYZER=True, ENABLE_STARTUP_SHUTDOWN=True)
        self.assertTrue(hasattr(model, 'electrolyzer_on_off_logic_constr'))
        self.assertTrue(hasattr(model, 'electrolyzer_min_power_when_on_constr'))
    ...
    def test_degradation_constraint(self):
        ...
        model = self._setup_model_with_config(ENABLE_ELECTROLYZER=True, ENABLE_ELECTROLYZER_DEGRADATION_TRACKING=True, ENABLE_STARTUP_SHUTDOWN=True)
    ...
    def test_battery_constraints(self):
        ...
        self.assertTrue(hasattr(model, 'battery_power_capacity_link_constr')) # This is a single constraint, not indexed by TimePeriods
    ...
    def test_ancillary_service_capability_constraints(self):
        ...
        model_as_active = self._setup_model_with_config(
            ENABLE_NUCLEAR_GENERATOR=True, ENABLE_ELECTROLYZER=True, ENABLE_BATTERY=False # Ensures CAN_PROVIDE_AS is True
        )
        ...
        self.assertTrue(hasattr(model_as_active, 'Electrolyzer_AS_Pmin_constr')) # Check another
        ...
        if hasattr(model_as_inactive, 'Turbine_AS_Pmax_constr'): # It might not be added at all
             self.assertEqual(model_as_inactive.Turbine_AS_Pmax_constr[1], pyo.Constraint.Skip)
    ...
    def test_as_linking_constraints(self):
        ...
        model_as_inactive = self._setup_model_with_config(ENABLE_NUCLEAR_GENERATOR=False)
        self.assertFalse(model_as_inactive.CAN_PROVIDE_ANCILLARY_SERVICES)
    ...
    def test_dispatch_simulation_constraints(self):
        ...
        if hasattr(model_sim_off, 'link_RegUp_Electrolyzer_deployed_constr'): # Dynamic name
            self.assertEqual(model_sim_off.link_RegUp_Electrolyzer_deployed_constr[1], pyo.Constraint.Skip)
        if hasattr(model_sim_off, 'define_actual_electrolyzer_power_constr'):
            self.assertEqual(model_sim_off.define_actual_electrolyzer_power_constr[1], pyo.Constraint.Skip)
        ...
        deployed_constr_name = 'link_RegUp_Electrolyzer_deployed_constr' # As defined in model.py add_deployment_link_constraints
        self.assertTrue(hasattr(model_sim_on, deployed_constr_name), f"{deployed_constr_name} missing")
        self.assertTrue(getattr(model_sim_on, deployed_constr_name)[1].active)

        self.assertTrue(hasattr(model_sim_on, 'define_actual_electrolyzer_power_constr'))
        self.assertTrue(model_sim_on.define_actual_electrolyzer_power_constr[1].active)
    ...
    def test_get_as_components_helper(self):
        ...
        if hasattr(model, 'SR_Turbine'): model.SR_Turbine[t].fix(10)
        model.RegDown_Electrolyzer[t].fix(3)
        model.RegUp_Battery[t].fix(2)

        as_info = get_as_components(model, t)

        self.assertEqual(pyo.value(as_info['up_reserves_bid_turbine']), 15) # 5 (RegUp) + 10 (SR)
        self.assertEqual(pyo.value(as_info['down_reserves_bid_turbine']), 0)
        self.assertEqual(pyo.value(as_info['up_reserves_bid_h2']), 0)
        self.assertEqual(pyo.value(as_info['down_reserves_bid_h2']), 3) # 3 (RegDown)
        self.assertEqual(pyo.value(as_info['up_reserves_bid_battery']), 2) # 2 (RegUp)
        self.assertEqual(pyo.value(as_info['down_reserves_bid_battery']), 0)

        self.assertEqual(pyo.value(as_info['up_reserves_bid']), 17) # 15 + 0 + 2
        self.assertEqual(pyo.value(as_info['down_reserves_bid']), 3) # 0 + 3 + 0
