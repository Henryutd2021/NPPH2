...
class TestModel(unittest.TestCase):
    ...
    def _create_base_data_inputs(self, iso="ERCOT"):
        ...
        if iso == "ERCOT":
            price_data_dict.update({f'p_RegU_{iso}': [10]*num_hours, f'p_SR_{iso}': [5]*num_hours})
        elif iso == "SPP":
            price_data_dict.update({f'p_RegU_{iso}': [10]*num_hours, f'p_Spin_{iso}': [5]*num_hours})
        ...
        if len(parameters) != len(values):
             raise ValueError(f"Mismatch in test_model setup parameter/value lengths ({len(parameters)} vs {len(values)}).")
        ...
        return {
            ...
            'df_ANSmile_hourly': pd.DataFrame({'hour': range(1,num_hours+1)}), # Minimal
            'df_ANSdeploy_hourly': pd.DataFrame({'hour': range(1,num_hours+1)}),# Minimal
            'df_ANSwinrate_hourly': pd.DataFrame({'hour': range(1,num_hours+1)}),# Minimal
    ...
    def test_get_sys_param_various_types(self):
        ...
        sample_data_price = pd.DataFrame({'Price ($/MWh)': [50.0]}) # Minimal price data

        sample_data_inputs = {
            'df_system': sample_data_sys,
            'df_price_hourly': sample_data_price,
            'df_ANSprice_hourly': sample_data_price, # Add other minimal required dataframes
        ...
        with patch.multiple('config', ENABLE_NUCLEAR_GENERATOR=False, ENABLE_ELECTROLYZER=False, ENABLE_BATTERY=False):
            ...
            model = create_model(sample_data_inputs, "ERCOT", simulate_dispatch=False) # This sets the global model.df_system
            ...
            self.assertFalse(original_get_sys_param('bool_cap_false_str')) # Test case insensitivity for boolean
            self.assertEqual(original_get_sys_param('string_param'), 'my_string')
            self.assertEqual(original_get_sys_param('int_param'), 100.0) # Converts to float
            ...
            self.assertEqual(original_get_sys_param('bp_param'), '200,300') # Returns string for breakpoints

            self.assertIsNone(original_get_sys_param('nan_param')) # NaN becomes None, then default (None here)
            self.assertEqual(original_get_sys_param('nan_param', default=0.0), 0.0)
            self.assertIsNone(original_get_sys_param('none_param'))
            self.assertEqual(original_get_sys_param('none_param', default='def'), 'def')
            ...
            self.assertEqual(original_get_sys_param('missing_param', default='fallback'), 'fallback')
    ...
    def test_model_creation_lte_mode(self):
        ...
        model = self._setup_model_with_config(ENABLE_LOW_TEMP_ELECTROLYZER=True, ENABLE_ELECTROLYZER=True, ENABLE_NUCLEAR_GENERATOR=True)
        self.assertTrue(model.LTE_MODE)
        self.assertTrue(hasattr(model, 'pElectrolyzer'))
        self.assertFalse(hasattr(model, 'qSteam_Electrolyzer'), "qSteam_Electrolyzer should not exist for LTE")
        self.assertTrue(hasattr(model, 'cost_electrolyzer_capacity_USD_per_MW_year_LTE'))
        # Check if kt_H2_values exists and is zero for LTE
        self.assertTrue(hasattr(model, 'kt_H2_values'), "kt_H2_values should exist even for LTE")
        if hasattr(model, 'pElectrolyzer_efficiency_breakpoints') and model.pElectrolyzer_efficiency_breakpoints:
            self.assertAlmostEqual(model.kt_H2_values[model.pElectrolyzer_efficiency_breakpoints.first()], 0.0, places=5, msg="kt should be zero for LTE")
    ...
    def test_model_creation_with_battery(self):
        ...
        self.assertTrue(hasattr(model, 'BatteryCapex_USD_per_MWh_year')) # Parameter


    def test_model_creation_with_h2_storage(self):
        """Test model creation with H2 storage enabled."""
        model = self._setup_model_with_config(ENABLE_H2_STORAGE=True, ENABLE_ELECTROLYZER=True)
    ...
    def test_model_creation_with_startup_shutdown(self):
        ...
        model = self._setup_model_with_config(ENABLE_STARTUP_SHUTDOWN=True, ENABLE_ELECTROLYZER=True)
        ...
        self.assertTrue(hasattr(model, 'cost_startup_electrolyzer_USD_per_startup_LTE')) # or HTE depending on LTE_MODE


    def test_model_creation_with_degradation(self):
        """Test model creation with degradation tracking enabled."""
        model = self._setup_model_with_config(ENABLE_ELECTROLYZER_DEGRADATION_TRACKING=True, ENABLE_ELECTROLYZER=True)
        self.assertTrue(hasattr(model, 'DegradationState'))
        self.assertTrue(hasattr(model, 'DegradationFactorOperation_Units_per_Hour_at_MaxLoad_LTE')) # or HTE
    ...
    def test_ancillary_service_vars_and_params_no_as(self):
        ...
        model = self._setup_model_with_config(ENABLE_NUCLEAR_GENERATOR=False, ENABLE_ELECTROLYZER=False, ENABLE_BATTERY=False)
        self.assertFalse(model.CAN_PROVIDE_ANCILLARY_SERVICES)
        self.assertIsInstance(model.Total_RegUp, pyo.Param) # Should be Param fixed to 0
        self.assertEqual(pyo.value(model.Total_RegUp[model.TimePeriods.first()]), 0)
    ...
    def test_ancillary_service_vars_and_params_as_enabled_bidding_mode(self):
        ...
        self.assertTrue(hasattr(model, f'p_RegU_{target_iso}')) # Price param should exist
    ...
    def test_precomputation_logic(self):
        ...
        # HTE model
        model_hte = self._setup_model_with_config(ENABLE_ELECTROLYZER=True, ENABLE_LOW_TEMP_ELECTROLYZER=False)
        ...
        if hasattr(model_hte, 'pElectrolyzer_efficiency_breakpoints') and model_hte.pElectrolyzer_efficiency_breakpoints:
            ...
            self.assertAlmostEqual(model_hte.ke_H2_inv_values[first_bp_hte], expected_inv_eff_hte)

            if hasattr(model_hte, 'kt_H2_values'): # kt_H2_values exists for HTE
                self.assertTrue(hasattr(model_hte, 'qSteam_values_at_pElec_bp'))
                # Ensure the value exists in the Param before accessing
                self.assertIn(first_bp_hte, model_hte.kt_H2_values)
                kt_val = pyo.value(model_hte.kt_H2_values[first_bp_hte])
                expected_q_steam_hte = (kt_val * model_hte.ke_H2_inv_values[first_bp_hte] * first_bp_hte)
                self.assertAlmostEqual(pyo.value(model_hte.qSteam_values_at_pElec_bp[first_bp_hte]), expected_q_steam_hte)
        else:
             self.fail("Missing pElectrolyzer_efficiency_breakpoints for HTE precomputation test")


        # LTE model
        model_lte = self._setup_model_with_config(ENABLE_ELECTROLYZER=True, ENABLE_LOW_TEMP_ELECTROLYZER=True)
        ...
        if hasattr(model_lte, 'pElectrolyzer_efficiency_breakpoints') and model_lte.pElectrolyzer_efficiency_breakpoints:
            ...
            self.assertAlmostEqual(model_lte.ke_H2_inv_values[first_bp_lte], expected_inv_eff_lte)
            # qSteam_values_at_pElec_bp should not be meaningfully populated for LTE or kt_H2_values is zero
            self.assertTrue(hasattr(model_lte, 'kt_H2_values'), "kt_H2_values should exist for LTE (as zero)")
            # Ensure the value exists in the Param before accessing
            self.assertIn(first_bp_lte, model_lte.kt_H2_values)
            self.assertAlmostEqual(pyo.value(model_lte.kt_H2_values[first_bp_lte]), 0.0, places=5, msg="kt should be zero for LTE")
            self.assertFalse(hasattr(model_lte, 'qSteam_values_at_pElec_bp'), "qSteam_values_at_pElec_bp should not be created for LTE")
        else:
             self.fail("Missing pElectrolyzer_efficiency_breakpoints for LTE precomputation test")
