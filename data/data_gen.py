...
def generate_system_data(output_path: Path):
    ...
    # Example: 50 kWh/ton_H2 -> 0.05 kWh/kg_H2 -> 0.00005 MWh/kg_H2. This param is MW_aux per kg/hr H2.
    aux_power_consumption_per_kg_h2_val = 0.05
    ...
    batt_capex_usd_per_mw_year = batt_capex_mw_total * \
        batt_crf  # This is power capacity cost
    ...
    batt_fom_usd_per_kw_year = fom_fraction_of_capex_per_kw * \
        batt_capex_mw_total_usd_per_kw  # $/kW-year
    ...
    # 1% of $/MWh-cap per year
    batt_fom_usd_per_mwh_year = (
        batt_capex_mwh_total_usd_per_kwh * 0.01) * 1000.0
    ...
    system_params = {
        ...
        # MODIFIED: No energy capacity cost (power-only costing)
        'BatteryCapex_USD_per_MWh_year': 0.0,
        # Annualized power capacity cost (only cost)
        'BatteryCapex_USD_per_MW_year': batt_capex_usd_per_mw_year,
        # MODIFIED: No energy-based O&M (power-only costing)
        'BatteryFixedOM_USD_per_MWh_year': 0.0,
        # Power-based O&M (assuming 4h duration)
        'BatteryFixedOM_USD_per_MW_year': batt_fom_usd_per_mwh_year * 4,
...
def generate_price_hourly(output_path: Path, num_hours=HOURS_IN_YEAR):
    ...
    base_lmp = 45
    peak_factor = 1.5 + 0.5 * \
        np.sin(np.linspace(0, 2 * np.pi * (num_hours / HOURS_IN_YEAR), num_hours))
    daily_variation = 20 * \
        np.sin(np.linspace(0, 2 * np.pi * (num_hours / 24), num_hours))
    weekly_variation = 5 * \
        np.sin(np.linspace(0, 2 * np.pi * (num_hours / (24 * 7)), num_hours))
...
def generate_deploy_factor_hourly(output_path: Path, iso: str, lmp_series: pd.Series, num_hours=HOURS_IN_YEAR):
    ...
    for service_key in iso_services:
        ...
        deploy_magnitude = np.random.uniform(
            0.01, 0.6, num_hours)  # Max 60% deployment
        deploy_values = deploy_magnitude * deploy_mask
        df_deploy[factor_col_name] = np.round(deploy_values, 4)
        generated_factors = True

    # Ensure we don't save an empty DF if no reserve services
    if not generated_factors and not df_deploy.empty:
        ...
        print(
            f"No reserve deployment factors to generate for {iso}. Skipping file save for {output_path.name}")
...
def generate_mileage_multiplier_hourly(output_path: Path, iso: str, num_hours=HOURS_IN_YEAR):
    ...
    print(
        f"Generating hourly mileage/performance factors for {iso}: {output_path}")
    df_mileage_perf = pd.DataFrame(index=pd.date_range(
    ...
    if not generated_factors and df_mileage_perf.empty:  # Check if DF is still empty
        print(
            f"No regulation mileage/performance factors to generate for {iso}. Skipping file save for {output_path.name}")
        return

    df_mileage_perf.index.name = 'Timestamp'
    df_mileage_perf.to_csv(output_path)
    print(
        f"Hourly mileage/performance factors for {iso} saved to {output_path}")
...
def generate_winning_rate_hourly(output_path: Path, iso: str, lmp_series: pd.Series, num_hours=HOURS_IN_YEAR):
    ...
    for service_key in iso_services:
        ...
        # Ensure rates are between 0.1 and 1.0
...
if __name__ == "__main__":
    ...
    for target_iso in ISO_SERVICE_MAP.keys():
        ...
        try:
            ...
            print(
                f"ERROR: Failed to load generated LMP for {target_iso}: {e}. Skipping.")
            continue
        ...
        generate_mileage_multiplier_hourly(
            mileage_perf_path, target_iso, num_hours=HOURS_IN_YEAR)
