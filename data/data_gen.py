...
def generate_system_data(output_path: Path):
    ...
    print(f"Generating common system parameters (including HTE/LTE): {output_path}")
    ...
    p_elec_min_load_mw = p_elec_max_capacity_mw * elec_min_load_fraction # Calculate the value
    discount_rate = 0.08 # General discount rate
    electrolyzer_economic_lifetime_years = 20 # Common assumption for LCOH analysis base

    # --- Common Electrolyzer Settings ---
    p_elec_breakpoints_mw = [p_elec_min_load_mw, 100.0, 250.0, 500.0] # Start breakpoint at min load
    ...
    aux_power_consumption_per_kg_h2_val = 3.5 # kWh/kg (Estimate for compressor + BOP)
    ...
    lte_efficiency_kwh_per_kg = [57.0, 56.0, 55.0, 54.0] # Eff at min_load, 100, 250, 500 MW
    ...
    lte_kt_h2_values_str = ', '.join(['0.00000'] * len(p_elec_breakpoints_mw)) # Zero for LTE
    ...
    hte_lifetime_years = 20 # Assume matches economic life for CRF calc (though stack life shorter)
    ...
    hte_efficiency_kwh_per_kg = [42.0, 41.0, 40.0, 39.0] # Eff at min_load, 100, 250, 500 MW
    ...
    hte_kt_h2_mwh_per_kg = [0.015] * len(p_elec_breakpoints_mw) # Using constant value
    ...
    system_params = {
        ...
        # *** ADDED GENERIC BREAKPOINT AND EFFICIENCY PARAMETERS ***
        'pElectrolyzer_Breakpoints_MW': p_elec_breakpoints_str, # Generic Breakpoints
        'ke_H2_Values_MWh_per_kg': lte_ke_h2_values_str, # Generic ke (use LTE as default)
        'kt_H2_Values_MWth_per_kg': lte_kt_h2_values_str, # Generic kt (use LTE as default=0)
        # *** ***
        ...
        'hydrogen_subsidy_per_kg': hydrogen_subsidy_per_kg_val,
        'aux_power_consumption_per_kg_h2': aux_power_consumption_per_kg_h2_val,
        'H2_value_USD_per_kg': H2_value_USD_per_kg,
        'cost_electrolyzer_ramping_USD_per_MW_ramp': 0.5, # Generic, can be overridden by specific if needed
        'Ramp_qSteam_Electrolyzer_limit_MWth_per_Hour': 500.0, # Steam ramp limit (relevant for HTE model logic)
        ...
        'kt_H2_Values_MWth_per_kg_HTE': hte_kt_h2_values_str, # Now non-zero based on derivation
...
def generate_mileage_multiplier_hourly(output_path: Path, iso: str, num_hours=HOURS_IN_YEAR):
    ...
    df_mileage = pd.DataFrame(index=pd.date_range(start='2023-01-01', periods=num_hours, freq='h'))
