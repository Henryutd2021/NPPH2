...
class AncillaryServicesAnalyzer:
    ...
    def __init__(self, results_dir: str = "../../output/opt/cs1"):
        ...
        # ISO-specific ancillary service mappings (Parameter names used in data files)
        self.iso_parameter_services = {
        ...
        # Mapping from ISO parameter names to internal service names (used in CSV columns)
        self.iso_to_internal_mapping = {
            "SPP": {
                "RegU": "RegUp", "RegD": "RegDown", "Spin": "SR", "Sup": "NSR",
                "RamU": "RampUp", "RamD": "RampDown", "UncU": "UncU"
            },
            "CAISO": {
                "RegU": "RegUp", "RegD": "RegDown", "Spin": "SR", "NSpin": "NSR",
            ...
            "PJM": {
                "RegUp": "RegUp", "RegDown": "RegDown", "Syn": "SR", "Rse": "NSR",
                "TMR": "ThirtyMin"
            },
            "NYISO": {
                "RegUp": "RegUp", "RegDown": "RegDown", "Spin10": "SR", "NSpin10": "NSR",
                "Res30": "ThirtyMin"
            },
            "ISONE": {
                "Spin10": "SR", "NSpin10": "NSR", "OR30": "ThirtyMin"
            },
            "MISO": {
                "RegUp": "RegUp", "RegDown": "RegDown", "Spin": "SR", "Sup": "NSR",
                "STR": "ThirtyMin", "RamU": "RampUp", "RamD": "RampDown"
            }
        }

        # Service type categorization (using internal names)
        self.service_categories = {
            "regulation": ["RegUp", "RegDown"],
            "spinning_reserve": ["SR"],
            "non_spinning_reserve": ["NSR"],
            "ramping": ["RampUp", "RampDown"],
            "other": ["UncU", "ECRS", "ThirtyMin"]
        }
    ...
    def load_hourly_results(self) -> Dict[str, pd.DataFrame]:
        ...
        for file_path in result_files:
            try:
                ...
                iso = None
                for potential_iso in self.iso_parameter_services.keys():
    ...
    def analyze_ancillary_service_provision(self, df: pd.DataFrame, iso: str, reactor_name: str) -> Dict:
        ...
        # Get available services for this ISO (internal names)
        available_internal_services = list(
            self.iso_to_internal_mapping.get(iso, {}).values())

        logger.debug(
            f"Available internal services for {iso}: {available_internal_services}")

        for internal_service in available_internal_services:
            # Analyze bid quantities using internal service names
            bid_cols = [
                col for col in df.columns if f'{internal_service}_' in col and '_Bid_MW' in col
            ]
            deployed_cols = [
                col for col in df.columns if f'{internal_service}_' in col and '_Deployed_MW' in col
            ]

            logger.debug(
                f"Found bid columns for {internal_service}: {bid_cols}")
            logger.debug(
                f"Found deployed columns for {internal_service}: {deployed_cols}")
            ...
            if bid_cols:
                ...
                if not total_bids.empty:
                    ...
                    # Check if there are any non-zero bids
                    non_zero_bids = service_bid_total[service_bid_total > 0]

                    if len(non_zero_bids) > 0:
                        # Service provision frequency and characteristics
                        provision_hours = len(non_zero_bids)
                        provision_frequency = provision_hours / \
                            len(service_bid_total)

                        metrics[f'{internal_service}_provision_frequency'] = provision_frequency
                        metrics[f'{internal_service}_avg_bid_MW'] = non_zero_bids.mean()
                        metrics[f'{internal_service}_max_bid_MW'] = service_bid_total.max(
                        )
                        metrics[f'{internal_service}_total_bid_MWh'] = service_bid_total.sum(
                        )

                        # Calculate service variability
                        if len(non_zero_bids) > 1:
                            metrics[f'{internal_service}_bid_variability'] = non_zero_bids.std(
                            ) / non_zero_bids.mean()
                        else:
                            metrics[f'{internal_service}_bid_variability'] = 0

                        # Consecutive hours analysis
                        consecutive_provision = self._analyze_consecutive_hours(
                            service_bid_total > 0)
                        metrics[f'{internal_service}_avg_consecutive_hours'] = consecutive_provision['avg_consecutive']
                        metrics[f'{internal_service}_max_consecutive_hours'] = consecutive_provision['max_consecutive']
                    else:
                        # No bids for this service - set to 0 instead of leaving empty
                        metrics[f'{internal_service}_provision_frequency'] = 0
                        metrics[f'{internal_service}_avg_bid_MW'] = 0
                        metrics[f'{internal_service}_max_bid_MW'] = 0
                        metrics[f'{internal_service}_total_bid_MWh'] = 0
                        metrics[f'{internal_service}_bid_variability'] = 0
                        metrics[f'{internal_service}_avg_consecutive_hours'] = 0
                        metrics[f'{internal_service}_max_consecutive_hours'] = 0
            ...
            if deployed_cols:
                ...
                if not total_deployed.empty:
                    ...
                    non_zero_deployed = service_deployed_total[service_deployed_total > 0]

                    if len(non_zero_deployed) > 0:
                        deployed_hours = len(non_zero_deployed)
                        metrics[f'{internal_service}_deployment_frequency'] = deployed_hours / len(
                            service_deployed_total)
                        metrics[f'{internal_service}_avg_deployed_MW'] = non_zero_deployed.mean(
                        )
                        metrics[f'{internal_service}_max_deployed_MW'] = service_deployed_total.max(
                        )
                    else:
                        metrics[f'{internal_service}_deployment_frequency'] = 0
                        metrics[f'{internal_service}_avg_deployed_MW'] = 0
                        metrics[f'{internal_service}_max_deployed_MW'] = 0

        # Calculate service diversity and portfolio metrics
        total_services_provided = sum(1 for service in available_internal_services
        ...
        metrics['service_diversity_ratio'] = total_services_provided / \
            len(available_internal_services) if available_internal_services else 0
    ...
    def save_results(self, results_df: pd.DataFrame, output_file: str = "analysis_results.csv"):
        ...
        try:
            ...
            output_dir = Path("../../output/ancillary")
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('../../output/logs/ancillary_analysis.log'),
