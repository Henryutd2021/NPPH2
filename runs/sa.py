...
PARAMETERS_TO_VARY = {
    ...
    'cost_startup_electrolyzer_USD_per_startup_LTE': [-50, 50, 7],  # LTE startup cost
    'cost_startup_electrolyzer_USD_per_startup_HTE': [-50, 50, 7],  # HTE startup cost
    'cost_water_USD_per_kg_h2': [-30, 30, 7],  # Water cost
    'cost_h2_storage_USD_per_kg': [-40, 40, 9],  # H2 storage cost
    'cost_electrolyzer_capacity_USD_per_MW_year_LTE': [-20, 20, 5],  # LTE capex
    'cost_electrolyzer_capacity_USD_per_MW_year_HTE': [-20, 20, 5],  # HTE capex
    'cost_electrolyzer_ramping_USD_per_MW_ramp': [-30, 30, 7],  # Ramping cost
    'hydrogen_subsidy_value_usd_per_kg': [0, 5, 6],  # Absolute values, not percentage change
    ...
    'Turbine_RampDown_Rate_Percent_per_Min': [-50, 100, 7],  # Turbine ramp down rate
    'Electrolyzer_RampUp_Rate_Percent_per_Min_LTE': [-50, 100, 7],  # LTE ramp rate
    'Electrolyzer_RampDown_Rate_Percent_per_Min_LTE': [-50, 100, 7],  # LTE ramp down rate
    'Electrolyzer_RampUp_Rate_Percent_per_Min_HTE': [-50, 100, 7],  # HTE ramp rate
    'Electrolyzer_RampDown_Rate_Percent_per_Min_HTE': [-50, 100, 7],  # HTE ramp down rate
    ...
    'Turbine_Thermal_Elec_Efficiency_Const': [-15, 15, 7],  # Turbine efficiency
    'DegradationFactorOperation_Units_per_Hour_at_MaxLoad_LTE': [0, 300, 7],  # LTE degradation
    'DegradationFactorOperation_Units_per_Hour_at_MaxLoad_HTE': [0, 300, 7],  # HTE degradation
    'DegradationFactorStartup_Units_per_Startup_LTE': [0, 300, 7],  # LTE startup degradation
    'DegradationFactorStartup_Units_per_Startup_HTE': [0, 300, 7],  # HTE startup degradation
    'BatteryChargeEff': [-15, 15, 7],  # Battery charge efficiency
    'BatteryDischargeEff': [-15, 15, 7],  # Battery discharge efficiency
    'storage_charge_eff_fraction': [-15, 15, 7],  # H2 storage charge efficiency
    'storage_discharge_eff_fraction': [-15, 15, 7],  # H2 storage discharge efficiency
    'H2_storage_charge_rate_max_kg_per_hr': [-30, 30, 7],  # H2 storage charge rate
    'H2_storage_discharge_rate_max_kg_per_hr': [-30, 30, 7],  # H2 storage discharge rate

    # Market Parameters
    'energy_price_multiplier': [-40, 40, 9],  # Overall energy price scaling
    'ancillary_service_price_multiplier': [-40, 40, 9],  # Overall AS price scaling
}

# Additional scenario analysis parameters
SCENARIO_ANALYSES = {
    'technology_combinations': [
        {'name': 'Nuclear + LTE', 'config': {'ENABLE_NUCLEAR_GENERATOR': True, 'ENABLE_ELECTROLYZER': True, 'ENABLE_LOW_TEMP_ELECTROLYZER': True, 'ENABLE_BATTERY': False, 'ENABLE_H2_STORAGE': False}},
        {'name': 'Nuclear + HTE', 'config': {'ENABLE_NUCLEAR_GENERATOR': True, 'ENABLE_ELECTROLYZER': True, 'ENABLE_LOW_TEMP_ELECTROLYZER': False, 'ENABLE_BATTERY': False, 'ENABLE_H2_STORAGE': False}},
        {'name': 'Nuclear + LTE + Storage', 'config': {'ENABLE_NUCLEAR_GENERATOR': True, 'ENABLE_ELECTROLYZER': True, 'ENABLE_LOW_TEMP_ELECTROLYZER': True, 'ENABLE_BATTERY': False, 'ENABLE_H2_STORAGE': True}},
        {'name': 'Nuclear + HTE + Storage', 'config': {'ENABLE_NUCLEAR_GENERATOR': True, 'ENABLE_ELECTROLYZER': True, 'ENABLE_LOW_TEMP_ELECTROLYZER': False, 'ENABLE_BATTERY': False, 'ENABLE_H2_STORAGE': True}},
        {'name': 'Nuclear + LTE + Battery', 'config': {'ENABLE_NUCLEAR_GENERATOR': True, 'ENABLE_ELECTROLYZER': True, 'ENABLE_LOW_TEMP_ELECTROLYZER': True, 'ENABLE_BATTERY': True, 'ENABLE_H2_STORAGE': False}},
        {'name': 'Nuclear + HTE + Battery', 'config': {'ENABLE_NUCLEAR_GENERATOR': True, 'ENABLE_ELECTROLYZER': True, 'ENABLE_LOW_TEMP_ELECTROLYZER': False, 'ENABLE_BATTERY': True, 'ENABLE_H2_STORAGE': False}},
        {'name': 'Full System (LTE)', 'config': {'ENABLE_NUCLEAR_GENERATOR': True, 'ENABLE_ELECTROLYZER': True, 'ENABLE_LOW_TEMP_ELECTROLYZER': True, 'ENABLE_BATTERY': True, 'ENABLE_H2_STORAGE': True}},
        {'name': 'Full System (HTE)', 'config': {'ENABLE_NUCLEAR_GENERATOR': True, 'ENABLE_ELECTROLYZER': True, 'ENABLE_LOW_TEMP_ELECTROLYZER': False, 'ENABLE_BATTERY': True, 'ENABLE_H2_STORAGE': True}},
    ],
    'market_scenarios': [
        {'name': 'Base Case', 'energy_price_multiplier': 1.0, 'ancillary_service_price_multiplier': 1.0},
        {'name': 'High Energy Prices', 'energy_price_multiplier': 1.5, 'ancillary_service_price_multiplier': 1.0},
        {'name': 'High AS Prices', 'energy_price_multiplier': 1.0, 'ancillary_service_price_multiplier': 1.5},
        {'name': 'Low Energy Prices', 'energy_price_multiplier': 0.7, 'ancillary_service_price_multiplier': 1.0},
        {'name': 'Low AS Prices', 'energy_price_multiplier': 1.0, 'ancillary_service_price_multiplier': 0.7},
        {'name': 'High Overall Prices', 'energy_price_multiplier': 1.3, 'ancillary_service_price_multiplier': 1.3},
        {'name': 'Low Overall Prices', 'energy_price_multiplier': 0.7, 'ancillary_service_price_multiplier': 0.7},
    ],
    'h2_value_scenarios': [
        {'name': 'Current H2 Value', 'H2_value_USD_per_kg': 4.0, 'hydrogen_subsidy_value_usd_per_kg': 0.0, 'hydrogen_subsidy_duration_years': 10},
        {'name': 'Medium H2 Value', 'H2_value_USD_per_kg': 6.0, 'hydrogen_subsidy_value_usd_per_kg': 0.0, 'hydrogen_subsidy_duration_years': 10},
        {'name': 'High H2 Value', 'H2_value_USD_per_kg': 10.0, 'hydrogen_subsidy_value_usd_per_kg': 0.0, 'hydrogen_subsidy_duration_years': 10},
        {'name': 'Current H2 + Short Subsidy', 'H2_value_USD_per_kg': 4.0, 'hydrogen_subsidy_value_usd_per_kg': 3.0, 'hydrogen_subsidy_duration_years': 5},
        {'name': 'Current H2 + Medium Subsidy', 'H2_value_USD_per_kg': 4.0, 'hydrogen_subsidy_value_usd_per_kg': 3.0, 'hydrogen_subsidy_duration_years': 10},
        {'name': 'Current H2 + Long Subsidy', 'H2_value_USD_per_kg': 4.0, 'hydrogen_subsidy_value_usd_per_kg': 3.0, 'hydrogen_subsidy_duration_years': 20},
    ],
    'fixed_size_scenarios': [
        {'name': 'Small Electrolyzer (100MW)', 'user_specified_electrolyzer_capacity_MW': 100},
        {'name': 'Medium Electrolyzer (250MW)', 'user_specified_electrolyzer_capacity_MW': 250},
        {'name': 'Large Electrolyzer (500MW)', 'user_specified_electrolyzer_capacity_MW': 500},
        {'name': 'Small Battery (50MW/200MWh)', 'user_specified_battery_power_MW': 50, 'user_specified_battery_energy_MWh': 200},
        {'name': 'Medium Battery (100MW/400MWh)', 'user_specified_battery_power_MW': 100, 'user_specified_battery_energy_MWh': 400},
        {'name': 'Large Battery (200MW/800MWh)', 'user_specified_battery_power_MW': 200, 'user_specified_battery_energy_MWh': 800},
...
def generate_run_config():
    ...
    with open(os.path.join(SENSITIVITY_OUTPUT_DIR, f"run_config_{config_data['timestamp']}.json"), 'w') as f:
...
def modify_sys_data(original_file, temp_file, param_name, new_value):
    ...
    try:
        ...
        logging.error(
            f"Error modifying parameter '{param_name}' in {original_file}: {e}", exc_info=True)
        if os.path.exists(original_file) and not os.path.exists(temp_file):
            shutil.copyfile(original_file, temp_file)
        return False

def modify_hourly_data(input_dir, energy_price_multiplier=None, as_price_multiplier=None):
    """
    ...
    """
    iso_dirs = [d for d in os.listdir(input_dir) if os.path.isdir(os.path.join(input_dir, d)) 
            ...
            for iso in iso_dirs:
                ...
                if os.path.exists(price_file):
                    ...
                    if price_col in df.columns:
                        ...
                        logging.info(f"Applied energy price multiplier {energy_price_multiplier} to {price_file}")
            ...
            for iso in iso_dirs:
                ...
                if os.path.exists(ans_price_file):
                    ...
                    for col in df.columns:
                        ...
                        if col.lower() not in ['hour', 'timestamp', 'date', 'time', 'datetime']:
                            df[col] = df[col] * as_price_multiplier
                    df.to_csv(ans_price_file, index=False)
                    logging.info(f"Applied AS price multiplier {as_price_multiplier} to {ans_price_file}")
...
def run_single_optimization(input_dir, iso=None, solver_name="gurobi", config_override=None):
    """
        ...
        try:
            ...
            try:
                ...
                results_df, summary_results = extract_results(
                    model, target_iso, output_dir=os.path.join(input_dir, "results"))
                
                # Map summary results to our metrics if needed
                if 'H2 Production (kg)' not in result_dict or np.isnan(result_dict['H2 Production (kg)']):
                    result_dict['H2 Production (kg)'] = summary_results.get('Total_H2_Produced_kg', 0.0)
                
                if 'Electrolyzer Capacity Factor' not in result_dict or np.isnan(result_dict['Electrolyzer Capacity Factor']):
                    result_dict['Electrolyzer Capacity Factor'] = summary_results.get('Electrolyzer_Capacity_Factor_Actual', 0.0)
                
                # Add any other metrics we care about
                for key, value in summary_results.items():
                    if key not in result_dict and 'Total_' in key and isinstance(value, (int, float, np.number)):
...
def plot_sensitivity_results(results_df, output_dir):
        ...
        for metric in RESULT_METRICS:
            ...
            if change_col in summary_df.columns:
                ...
                top_params = summary_df.sort_values(
                    by=change_col, ascending=False).head(10)
                ...
                if not top_params.empty:
                    ...
                    plt.tight_layout()
                    plt.savefig(os.path.join(output_dir, f'top_parameters_{metric.replace(" ", "_")}.png'))
                    plt.close()
    ...
    for param in results_df['Parameter'].unique():
        ...
        for idx, metric in enumerate(RESULT_METRICS):
            ...
            ax.plot(param_data['Variation (%)'],
                    param_data[metric], 'b-', label='Absolute Value')

            # Plot percentage change if applicable
            if f'{metric}_Change_Pct' in param_data.columns:
                ax2 = ax.twinx()
                ax2.plot(param_data['Variation (%)'],
                         param_data[f'{metric}_Change_Pct'], 'r--', label='% Change')
        ...
        corr_data = param_data.filter(regex='^(?!.*_Change_Pct).*$')  # Exclude % change columns
        corr_data = corr_data.select_dtypes(include=[np.number])  # Keep only numeric columns
        ...
        if len(corr_data.columns) > 2:  # Need at least parameter value + 2 metrics
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, f'correlation_{param.replace(" ", "_")}.png'))
            plt.close()
...
def create_tornado_plots(results_df, output_dir):
    ...
    for metric in RESULT_METRICS:
        ...
        for param in results_df['Parameter'].unique():
            ...
            if change_col in param_data.columns:
                ...
                if not (np.isnan(max_increase) and np.isnan(max_decrease)):
                    impact_data.append({
                        'Parameter': param,
                        'Max Increase (%)': max_increase if not np.isnan(max_increase) else 0,
                        'Max Decrease (%)': max_decrease if not np.isnan(max_decrease) else 0
                    })
        ...
        if impact_data:
            ...
            impact_df['Total Impact'] = abs(impact_df['Max Increase (%)']) + abs(impact_df['Max Decrease (%)'])
            impact_df = impact_df.sort_values('Total Impact', ascending=False).head(15)  # Top 15
            ...
            plt.barh(y_pos, impact_df['Max Increase (%)'], height=0.8, 
                    color='green', alpha=0.6, label='Max Increase')
            plt.barh(y_pos, impact_df['Max Decrease (%)'], height=0.8,
                    color='red', alpha=0.6, label='Max Decrease')
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, f'tornado_{metric.replace(" ", "_")}.png'))
            plt.close()
...
def analyze_scenario_results(scenario_results, scenario_type, output_dir):
    ...
    for metric in metric_cols:
        ...
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'{scenario_type}_{metric.replace(" ", "_")}.png'))
        plt.close()
    ...
    plt.title(f'Normalized Metrics by {scenario_type.replace("_", " ").title()} Scenario')
    plt.tight_layout()
...
def analyze_size_combinations(output_dir=None):
    ...
    total_combinations = len(electrolyzer_sizes) * len(battery_power_sizes) * len(battery_energy_durations)
    logging.info(f"Starting size combination analysis with {total_combinations} combinations")
    ...
    for elec_size in electrolyzer_sizes:
        for batt_power in battery_power_sizes:
            for batt_duration in battery_energy_durations:
                ...
                # Update progress
                current_run += 1
                print(f"Running size combination {current_run}/{total_combinations}: "
                ...
                # Apply changes
                modify_sys_data(BASE_SYS_DATA_FILE, temp_file_1,
                               'user_specified_electrolyzer_capacity_MW', elec_size)
                modify_sys_data(temp_file_1, temp_file_2,
                               'user_specified_battery_power_MW', batt_power)
                modify_sys_data(temp_file_2, sys_data_file,
                               'user_specified_battery_energy_MWh', batt_energy)
                ...
                result_dict = {
                    ...
                    'E/B Power Ratio': elec_size / batt_power if batt_power > 0 else float('inf')
                }
    ...
    key_metrics = ['Total Profit', 'Energy Revenue', 'Hydrogen Revenue', 
                  'Ancillary Service Revenue', 'Total Cost', 'H2 Production (kg)']
        ...
        if metric in results_df.columns:
            for duration in results_df['Battery Duration (h)'].unique():
                ...
                if len(df_duration) > 0:
                    ...
                    # Save chart
                    metric_name = metric.replace(' ', '_').replace('(', '').replace(')', '')
                    plt.savefig(os.path.join(output_dir, f'heatmap_{metric_name}_duration_{duration}h.png'))
                    plt.close()
    
    # Create best size combination analysis
    try:
        # Sort by Total Profit to find the optimal combinations
        top_combinations = results_df.sort_values(by='Total Profit', ascending=False).head(10)
        ...
        plt.xticks(range(len(top_combinations)), 
                  [f"E{row['Electrolyzer Size (MW)']}|B{row['Battery Power (MW)']}|{row['Battery Duration (h)']}h" 
                   for _, row in top_combinations.iterrows()], rotation=45)
        ...
        top_combinations.to_csv(os.path.join(output_dir, 'top_size_combinations.csv'), index=False)
...
def sensitivity_analysis():
    ...
    for param_name, (min_pct, max_pct, steps) in PARAMETERS_TO_VARY.items():
        ...
        try:
            ...
            use_absolute = False
            if min_pct * max_pct >= 0 and min_pct >= 0:  # If both min & max are >= 0, likely absolute
                if param_name in ['hydrogen_subsidy_value_usd_per_kg', 'hydrogen_subsidy_duration_years']:
                    use_absolute = True
                    pct_range = np.linspace(min_pct, max_pct, steps) # Here pct_range is actually absolute values
                    logging.info(f"Using absolute values for {param_name} from {min_pct} to {max_pct}")
                else:
            ...
            for pct in pct_range:
                ...
                if use_absolute:
                    ...
                    display_pct = f"{pct}" 
                    if base_value != 0:
                        display_pct += f" ({(pct-base_value)/abs(base_value)*100:.1f}%) "
                    else:
                         display_pct += " (N/A %)"
                else:
                ...
                run_dir = os.path.join(
                    TEMP_RUN_DIR_BASE, f"{param_name.replace(' ', '_')}_{pct:.2f}")
                if os.path.exists(run_dir):
                    shutil.rmtree(run_dir)
                shutil.copytree(BASE_INPUT_DIR, run_dir)
                ...
                sys_data_file = os.path.join(run_dir, "sys_data_advanced.csv")
                modify_sys_data(BASE_SYS_DATA_FILE, sys_data_file,
                ...
                for metric in RESULT_METRICS:
                    ...
                    if not np.isnan(results[metric]) and not np.isnan(base_results[metric]) and base_results[metric] != 0:
                        result_dict[f'{metric}_Change_Pct'] = (
                            results[metric] - base_results[metric]) / abs(base_results[metric]) * 100
                    else:
    ...
    for scenario in SCENARIO_ANALYSES['technology_combinations']:
        ...
        print(f"Running technology scenario: {scenario_name} ({current_run+1}/{total_runs})...")
        
        run_dir = os.path.join(TEMP_RUN_DIR_BASE, f"tech_{scenario_name.replace(' ', '_')}")
        if os.path.exists(run_dir):
            shutil.rmtree(run_dir)
        shutil.copytree(BASE_INPUT_DIR, run_dir)
        
        # Run optimization with config override
        results = run_single_optimization(run_dir, iso=TARGET_ISO, config_override=config)
        current_run += 1
    ...
    for scenario in SCENARIO_ANALYSES['market_scenarios']:
        ...
        print(f"Running market scenario: {scenario_name} ({current_run+1}/{total_runs})...")
        
        run_dir = os.path.join(TEMP_RUN_DIR_BASE, f"market_{scenario_name.replace(' ', '_')}")
        if os.path.exists(run_dir):
            shutil.rmtree(run_dir)
        shutil.copytree(BASE_INPUT_DIR, run_dir)
    ...
    for scenario in SCENARIO_ANALYSES['h2_value_scenarios']:
        ...
        print(f"Running H2 value scenario: {scenario_name} ({current_run+1}/{total_runs})...")
        
        run_dir = os.path.join(TEMP_RUN_DIR_BASE, f"h2value_{scenario_name.replace(' ', '_')}")
        if os.path.exists(run_dir):
            shutil.rmtree(run_dir)
        shutil.copytree(BASE_INPUT_DIR, run_dir)
        ...
        modify_sys_data(BASE_SYS_DATA_FILE, temp_sys_data_1, 'H2_value_USD_per_kg', h2_value)
        modify_sys_data(temp_sys_data_1, temp_sys_data_2, 'hydrogen_subsidy_value_usd_per_kg', h2_subsidy)
        modify_sys_data(temp_sys_data_2, sys_data_file, 'hydrogen_subsidy_duration_years', h2_duration)
    ...
    for scenario in SCENARIO_ANALYSES['fixed_size_scenarios']:
        ...
        print(f"Running fixed size scenario: {scenario_name} ({current_run+1}/{total_runs})...")
        
        run_dir = os.path.join(TEMP_RUN_DIR_BASE, f"fixed_size_{scenario_name.replace(' ', '_')}")
        if os.path.exists(run_dir):
            shutil.rmtree(run_dir)
        shutil.copytree(BASE_INPUT_DIR, run_dir)
        ...
        for param, value in scenario.items():
            if param != 'name':
                param_display = param.replace('user_specified_', '').replace('_', ' ').title()
    ...
    pd.DataFrame(tech_scenario_results).to_csv(
        os.path.join(SENSITIVITY_OUTPUT_DIR, "technology_scenario_results.csv"), index=False)
    pd.DataFrame(market_scenario_results).to_csv(
        os.path.join(SENSITIVITY_OUTPUT_DIR, "market_scenario_results.csv"), index=False)
    pd.DataFrame(h2_scenario_results).to_csv(
        os.path.join(SENSITIVITY_OUTPUT_DIR, "h2_value_scenario_results.csv"), index=False)
    pd.DataFrame(fixed_size_results).to_csv(
        os.path.join(SENSITIVITY_OUTPUT_DIR, "fixed_size_scenario_results.csv"), index=False)
    ...
    analyze_scenario_results(
        tech_scenario_results, "technology", os.path.join(SENSITIVITY_OUTPUT_DIR, "technology_comparison"))
    
    logging.info("Analyzing market scenarios...")
    analyze_scenario_results(
        market_scenario_results, "market", os.path.join(SENSITIVITY_OUTPUT_DIR, "market_scenarios"))
    
    logging.info("Analyzing H2 value scenarios...")
    analyze_scenario_results(
        h2_scenario_results, "h2_value", os.path.join(SENSITIVITY_OUTPUT_DIR, "h2_value_scenarios"))
        
    logging.info("Analyzing fixed size scenarios...")
    analyze_scenario_results(
        fixed_size_results, "fixed_size", os.path.join(SENSITIVITY_OUTPUT_DIR, "fixed_size_scenarios"))
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(SENSITIVITY_OUTPUT_DIR, "correlation_analysis", "full_correlation_matrix.png"))
        plt.close()
        
        # Save correlation matrix to CSV
        correlation_matrix.to_csv(os.path.join(SENSITIVITY_OUTPUT_DIR, "correlation_analysis", "correlation_matrix.csv"))
    except Exception as e:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Run sensitivity analysis for nuclear-hydrogen optimization model')
    
    # Add command line arguments
    parser.add_argument('--all', action='store_true', help='Run all sensitivity analyses')
    parser.add_argument('--params', action='store_true', help='Run only parameter sensitivity analysis')
    parser.add_argument('--tech', action='store_true', help='Run only technology configuration analysis')
    parser.add_argument('--market', action='store_true', help='Run only market scenario analysis')
    parser.add_argument('--h2value', action='store_true', help='Run only H2 value scenario analysis')
    parser.add_argument('--sizes', action='store_true', help='Run only fixed size scenario analysis')
    parser.add_argument('--combinations', action='store_true', help='Run only size combination analysis (time-consuming)')
    parser.add_argument('--iso', type=str, default=TARGET_ISO, help=f'Specify ISO region (default: {TARGET_ISO})')
    parser.add_argument('--output', type=str, default=SENSITIVITY_OUTPUT_DIR, help=f'Specify output directory (default: {SENSITIVITY_OUTPUT_DIR})')
    ...
    if not any([args.all, args.params, args.tech, args.market, args.h2value, args.sizes, args.combinations]):
        print("No analysis type specified, running complete sensitivity analysis. Use --help to view options.")
        ...
        if args.all:
            ...
            try:
                ...
                BASE_INPUT_DIR_local = os.path.join(output_dir, os.path.basename(BASE_INPUT_DIR))
                SENSITIVITY_OUTPUT_CSV_local = os.path.join(output_dir, os.path.basename(SENSITIVITY_OUTPUT_CSV))
                TEMP_RUN_DIR_BASE_local = os.path.join(output_dir, os.path.basename(TEMP_RUN_DIR_BASE))
                
                # Run specified analysis
                sensitivity_analysis()
                if args.combinations:
                    analyze_size_combinations(output_dir=os.path.join(output_dir, "size_combination_analysis"))
            finally:
