...
def print_configuration(args):
    ...
    logger.info(
        f"Electrolyzer Degradation: {ENABLE_ELECTROLYZER_DEGRADATION_TRACKING}")
...
def main(argv: list[str] | None = None):
    ...
    # --- MODIFICATION: Catch exceptions during setup/solve ---
        ...
        logger.critical(
            f"An exception occurred during model setup or solver call: {e}", exc_info=True)
    ...
    try:
        if solver_status == SolverStatus.ok and term_cond in {TerminationCondition.optimal, TerminationCondition.feasible}:
            ...
            if args.debug_infeasibility:
                ...
                try:
                    ...
                    try:
                        model.write(str(infeasible_lp_file), io_options={
                        ...
                        logger.error(
                            f"Could not write infeasible model file: {write_e}", exc_info=True)
                        print(
                            f"ERROR: Could not write infeasible model file: {write_e}")

                except Exception as path_e:  # Catch potential Path issues
                    logger.error(
                        f"Error setting up path for infeasible model file: {path_e}", exc_info=True)
                    print(
            ...
            try:
                ...
                try:
                    model.write(str(unbounded_lp_file), io_options={
                                'symbolic_solver_labels': True})
                    logger.info(
                    ...
                    logger.error(
                        f"Could not write unbounded model file: {write_e}", exc_info=True)
                    print(
                        f"ERROR: Could not write unbounded model file: {write_e}")
            except Exception as path_e:
                logger.error(
                    f"Error setting up path for unbounded model file: {path_e}", exc_info=True)
                print(
            ...
            if hasattr(model, 'TotalProfit_Objective') and model.TotalProfit_Objective.value is not None:
                ...
                results_df, summary_results = extract_results(
                    model, target_iso=args.iso)  # Attempt extraction
            else:
                logger.warning(
        ...
        logger.error(
            f"An exception occurred during result processing or status checking: {e}", exc_info=True)
...
if __name__ == "__main__":
    try:
        ...
        logger.critical(
            f"--- main.py script encountered an unhandled top-level exception: {e} ---", exc_info=True)
        print("\n--- Traceback ---")
        traceback.print_exc()
        print("-----------------\n")
