...
def calculate_financial_metrics(
    ...
    if annual_h2_prod_kg > 0:
        ...
        for op_idx in range(project_lt - construction_p):
            year_idx = op_idx + construction_p
            pv_factor = (1 + discount_rt) ** year_idx
            pv_total_h2_production += annual_h2_prod_kg / pv_factor
...
def calculate_incremental_metrics(
    ...
    if h2_prod_annual > 0:  # LCOH for incremental H2 project
        ...
        pv_h2_prod_inc = sum(
            h2_prod_annual / ((1 + discount_rt) **
                              (op_idx + construction_period))
            for op_idx in range(project_lifetime - construction_period)
        )
