...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(
    __file__), '..', 'src')))  # Assuming execution from ../src
...
try:
    ...
    logging.warning(
        "Could not import from config/lcoh. Using default TARGET_ISO.")
...
BASE_INPUT_DIR_DEFAULT = SCRIPT_DIR.parent / \
    "input/hourly_data"  # Default input dir for sys_data
...
OM_COMPONENTS = {
    ...
    'Variable_OM': {
        # This is expected to be included in annual_metrics['Annual_Cost'] -> Annual_Profit
        'base_cost': 0,
        'size_dependent': True,
        'inflation_rate': 0.02
    },
    'Water_Cost': {
        # Expected to be included in annual_metrics['Annual_Cost'] -> Annual_Profit
        'base_cost': 0,
        'size_dependent': True,
...
REPLACEMENT_SCHEDULE = {
    'Electrolyzer_Stack': {
        'cost': 15_000_000,
        # Replacements in operational years 10 and 20
        'years': [10, 20],
        'size_dependent': True
    },
...
def load_tea_sys_params(sys_data_path):
    ...
    try:
        ...
        param_list = [
            ...
            # Baseline revenue for incremental analysis
        ...
        logging.error(
            f"System data file not found at {sys_data_path} for TEA.")
    except Exception as e:
        logging.error(
            f"Error loading system data file {sys_data_path} for TEA: {e}")
    return params
...
def load_hourly_results(filepath):
    ...
    try:
        ...
        if missing_required:
            logging.error(
                f"Missing required columns in results file: {missing_required}")
            return None

        # Warn if optional columns are missing
        missing_optional = [
            col for col in optional_cols if col not in df.columns]
        if missing_optional:
            logging.warning(
                f"Missing optional breakdown columns in results file: {missing_optional}")

        return df
    except Exception as e:
        logging.error(
            f"Error loading results file {filepath}: {e}", exc_info=True)
        return None
...
def calculate_annual_metrics(df):
    ...
    try:
        ...
        # Financial Metrics
        # This is Revenue - Opex - AnnualizedCapex (from model objective) if objective used
        ...
        metrics['Energy_Revenue'] = df['Revenue_Energy_USD'].sum(
        ) if 'Revenue_Energy_USD' in df.columns else 0
        metrics['AS_Revenue'] = df['Revenue_Ancillary_USD'].sum(
        ) if 'Revenue_Ancillary_USD' in df.columns else 0
        metrics['H2_Revenue'] = df['Revenue_Hydrogen_USD'].sum(
        ) if 'Revenue_Hydrogen_USD' in df.columns else 0
        metrics['H2_Subsidy_Revenue'] = df['Revenue_Hydrogen_Subsidy_USD'].sum(
        ) if 'Revenue_Hydrogen_Subsidy_USD' in df.columns else 0
        metrics['H2_Sales_Revenue'] = df['Revenue_Hydrogen_Sales_USD'].sum(
        ) if 'Revenue_Hydrogen_Sales_USD' in df.columns else 0

        # Cost Breakdown (check if columns exist)
        vom_cost = 0
        if 'Cost_VOM_Turbine_USD' in df.columns:
            vom_cost += df['Cost_VOM_Turbine_USD'].sum()
        if 'Cost_VOM_Electrolyzer_USD' in df.columns:
            vom_cost += df['Cost_VOM_Electrolyzer_USD'].sum()
        if 'Cost_VOM_Battery_USD' in df.columns:
            vom_cost += df['Cost_VOM_Battery_USD'].sum()
        metrics['VOM_Cost'] = vom_cost

        metrics['Startup_Cost'] = df['Cost_Startup_USD'].sum(
        ) if 'Cost_Startup_USD' in df.columns else 0
        metrics['Water_Cost'] = df['Cost_Water_USD'].sum(
        ) if 'Cost_Water_USD' in df.columns else 0
        metrics['Ramping_Cost'] = df['Cost_Ramping_USD'].sum(
        ) if 'Cost_Ramping_USD' in df.columns else 0
        metrics['Storage_Cycle_Cost'] = df['Cost_Storage_Cycle_USD'].sum(
        ) if 'Cost_Storage_Cycle_USD' in df.columns else 0

        # Operational Metrics
        metrics['H2_Production'] = df['mHydrogenProduced_kg_hr'].sum(
        ) * (8760.0 / num_hours) if num_hours > 0 else 0  # Scale to annual kg
        ...
        # Assuming this exists if turbine enabled
        turbine_cap_col = 'Turbine_Capacity_MW'

        if elec_cap_col in df.columns and not df[elec_cap_col].empty:
            # Assuming capacity is constant in the run
            elec_avg_cap_in_run = df[elec_cap_col].iloc[0]
            if elec_avg_cap_in_run > 1e-6:
                metrics['Electrolyzer_CF'] = (
                    df['pElectrolyzer_MW'].mean() / elec_avg_cap_in_run) * 100
            else:
                metrics['Electrolyzer_CF'] = 0
        else:
            metrics['Electrolyzer_CF'] = np.nan
        ...
        if 'pTurbine_MW' in df.columns:  # Check if turbine power column exists
            if turbine_cap_col in df.columns and not df[turbine_cap_col].empty:
                # Assuming constant
                turbine_avg_cap_in_run = df[turbine_cap_col].iloc[0]
                if turbine_avg_cap_in_run > 1e-6:
                    metrics['Turbine_CF'] = (
                        df['pTurbine_MW'].mean() / turbine_avg_cap_in_run) * 100
                else:
                    metrics['Turbine_CF'] = 0
            else:
                metrics['Turbine_CF'] = np.nan  # Capacity column missing
        else:
            metrics['Turbine_CF'] = 0  # Turbine not enabled or column missing
        ...
        if 'pElectrolyzer_MW' in df.columns:
            ...
            metrics['Annual_Electrolyzer_MWh'] = total_electrolyzer_power_mwh * \
                (8760.0 / num_hours) if num_hours > 0 else 0
            logging.info(
                f"Annual electrolyzer energy consumption: {metrics['Annual_Electrolyzer_MWh']:.2f} MWh")
        else:
            metrics['Annual_Electrolyzer_MWh'] = 0
            logging.warning(
                "Electrolyzer power data not found in results file.")
        ...
        if 'EnergyPrice_LMP_USDperMWh' in df.columns:
            ...
            # Default $40/MWh if not available
            metrics['Avg_Electricity_Price'] = 40.0
            metrics['Weighted_Avg_Electricity_Price'] = 40.0
            logging.warning(
                "Electricity price data not found in results file. Using default $40/MWh.")

    except KeyError as e:
        logging.error(
            f"Missing expected column in hourly results for annual metrics calculation: {e}")
        return None
    except Exception as e:
...
def calculate_cash_flows(annual_metrics, project_years,
    ...
    logging.info(
        f"Calculating cash flows for {project_years} years. Subsidy: {h2_subsidy_value_usd_per_kg:.2f}/kg for {h2_subsidy_duration_years} years. Electrolyzer Fixed: {electrolyzer_capacity_was_fixed}")
    ...
    for i in range(CONSTRUCTION_YEARS):
        ...
        logging.debug(
            f"Construction Year Index {i} (Project Year {year_for_lookup}): CAPEX = ${-cash_flows[i]:,.2f}")

    # Check if construction period has any capital expenditure
    if all(cash_flows[i] == 0 for i in range(CONSTRUCTION_YEARS)):
        logging.warning(
            "No CAPEX allocated during construction period! Redistributing costs...")
        # Find total CAPEX from all components
        total_redistributable_capex = sum(details.get(
            'base_cost', 0) for details in CAPEX_COMPONENTS.values())
        # Redistribute evenly across construction years
        for i in range(CONSTRUCTION_YEARS):
            cash_flows[i] = -total_redistributable_capex / CONSTRUCTION_YEARS
            logging.debug(
                f"Redistributed CAPEX to Construction Year {i}: ${-cash_flows[i]:,.2f}")
    ...
    if start_of_operation_capex > 0:
        ...
        logging.debug(
            f"Start of operation CAPEX (Year 0): ${start_of_operation_capex:,.2f}")
    ...
    base_annual_profit_component = annual_metrics.get(
        'Annual_Revenue', 0) - annual_metrics.get('Annual_Cost', 0)  # Revenue - Opex
    logging.info(
        f"Base annual profit component (Rev - Opex) from metrics: ${base_annual_profit_component:,.2f}")
    ...
    if 'H2_Production' in annual_metrics and annual_metrics['H2_Production'] > 0 and h2_subsidy_value_usd_per_kg > 0:
        annual_subsidy_component = annual_metrics['H2_Production'] * \
            h2_subsidy_value_usd_per_kg
        logging.info(
            f"Annual subsidy component calculated: ${annual_subsidy_component:,.2f}")

    # --- Operational Period Cash Flows ---
    # 0 for 1st op_year, 1 for 2nd...
    for op_year_idx in range(project_years - CONSTRUCTION_YEARS):
        ...
        if electrolyzer_capacity_was_fixed:
            if not subsidy_applicable_this_year and annual_subsidy_component > 0:
                ...
                logging.debug(
                    f"Op Year {operational_year_num}: Removed subsidy (${annual_subsidy_component:,.2f}) for fixed electrolyzer case.")

        # --- Calculate Fixed O&M costs with inflation ---
        fixed_om_cost = 0
        fixed_om_details = OM_COMPONENTS.get('Fixed_OM')
        if fixed_om_details:
            inflated_fixed_om = fixed_om_details.get(
                'base_cost', 0) * (1 + fixed_om_details.get('inflation_rate', 0))**op_year_idx
            fixed_om_cost = inflated_fixed_om
            annual_profit -= fixed_om_cost
            logging.debug(
                f"Op Year {operational_year_num}: Fixed O&M cost: ${fixed_om_cost:,.2f}")
        ...
        for component, details in REPLACEMENT_SCHEDULE.items():
            if operational_year_num in details.get('years', []):
                ...
                logging.debug(
                    f"Op Year {operational_year_num}: {component} replacement cost: ${component_replacement_cost:,.2f}")
        ...
        if taxable_income > 0:
            ...
            logging.debug(
                f"Op Year {operational_year_num}: Tax amount: ${tax_amount:,.2f}")
...
def calculate_financial_metrics(cash_flows, annual_metrics):
    ...
    try:
        ...
        logging.info(
            f"Cash flow pattern: {['Neg' if cf < 0 else 'Pos' if cf > 0 else 'Zero' for cf in cash_flows]}")
        logging.info(
            f"Cash flow values: {[f'${cf:,.2f}' for cf in cash_flows]}")

        # Calculate sign changes in cash flow (crucial for IRR calculation)
        sign_changes = sum(1 for i in range(1, len(cash_flows)) if np.sign(
            cash_flows[i]) != np.sign(cash_flows[i-1]) and cash_flows[i-1] != 0 and cash_flows[i] != 0)
        ...
        initial_investment = abs(
            sum(cf for cf in cash_flows[:CONSTRUCTION_YEARS] if cf < 0))
        ...
        try:
            ...
            logging.info(
                f"NPV calculated using all cash flows: ${metrics['NPV']:,.2f}")
        except Exception as npv_e:
            logging.warning(
                f"Standard NPV calculation failed: {npv_e}. Trying alternative method.")
            ...
            metrics['NPV'] = npv
            logging.info(
                f"NPV calculated using alternative method: ${metrics['NPV']:,.2f}")
        ...
        def manual_irr(cash_flows, min_rate=-0.999, max_rate=10.0, tolerance=1e-6, max_iterations=1000):
            ...
            if all(cf >= 0 for cf in cash_flows) or all(cf <= 0 for cf in cash_flows):
                logging.warning(
                    "All cash flows have the same sign, IRR calculation not possible.")
                return np.nan
            ...
            if npv_min * npv_max > 0:
                ...
                logging.warning(
                    f"IRR might not exist in range [{min_rate}, {max_rate}]")
        ...
        if any(cf > 0 for cf in cash_flows) and any(cf < 0 for cf in cash_flows):
            try:
                ...
                logging.info(
                    f"IRR calculated using numpy_financial: {metrics['IRR']*100:.2f}%")
            except Exception as irr_e:
                logging.warning(
                    f"Standard IRR calculation failed: {irr_e}. Trying manual calculation.")
                # Try manual IRR calculation
                metrics['IRR'] = manual_irr(cash_flows)
                if not math.isnan(metrics['IRR']):
                    logging.info(
                        f"IRR calculated manually: {metrics['IRR']*100:.2f}%")
                else:
                    logging.warning(
                        "Manual IRR calculation also failed. Setting IRR to NaN.")
        else:
            logging.warning(
                "Cash flow pattern does not allow IRR calculation (need both positive and negative values).")
            metrics['IRR'] = np.nan
        ...
        if len(positive_indices) > 0:
            ...
            if first_positive_idx >= CONSTRUCTION_YEARS:
                ...
                if first_positive_idx > 0 and cumulative_cash_flow[first_positive_idx-1] < 0:
                    ...
                    metrics['Payback_Period'] = payback_decimal - \
                        CONSTRUCTION_YEARS + 1
                else:
                    # Simple payback (first year of positive cumulative CF)
                    metrics['Payback_Period'] = first_positive_idx - \
                        CONSTRUCTION_YEARS + 1
            else:
                # Payback occurs during construction
                # Immediate payback in operational terms
                metrics['Payback_Period'] = 0
        else:
            # Never pays back
            metrics['Payback_Period'] = np.nan
            logging.warning(
                "Project never reaches payback based on provided cash flows.")
        ...
        if total_pv_h2_production_kg > 0:
            ...
            metrics['LCOH'] = np.nan
            logging.warning(
                "Could not calculate LCOH: No hydrogen production data available.")

    except Exception as e:
        logging.error(
            f"Error calculating financial metrics: {e}", exc_info=True)
        metrics['NPV'] = np.nan
        metrics['IRR'] = np.nan
        metrics['Payback_Period'] = np.nan
        metrics['LCOH'] = np.nan
...
def calculate_incremental_metrics(cash_flows, baseline_revenue, total_capex, annual_metrics, project_years, h2_subsidy_duration_years=0):
    ...
    """
    logging.info(
        f"Calculating incremental metrics compared to baseline (nuclear-only) scenario")
    incremental_metrics = {}
    ...
    try:
        ...
        for year in range(project_years):
            ...
            if year >= CONSTRUCTION_YEARS:
                baseline_cash_flows[year] = baseline_revenue - \
                    nuclear_annual_om
                # Apply tax
        ...
        logging.info(
            f"Baseline scenario annual cash flow: ${baseline_cash_flows[CONSTRUCTION_YEARS]:,.2f}")
        ...
        annual_electricity_to_electrolyzer = annual_metrics.get(
        ...
        if annual_electricity_to_electrolyzer == 0 and 'pElectrolyzer_MW' in annual_metrics:
            ...
            annual_electricity_to_electrolyzer = annual_metrics.get(
                'pElectrolyzer_MW', 0) * 8760
            logging.warning(
                f"Estimating electrolyzer consumption from average power: {annual_electricity_to_electrolyzer:.2f} MWh")
        ...
        if 'Weighted_Avg_Electricity_Price' in annual_metrics:
            ...
            logging.info(
                f"Using weighted average electricity price: ${avg_electricity_price:.2f}/MWh")
        elif 'Avg_Electricity_Price' in annual_metrics:
            avg_electricity_price = annual_metrics['Avg_Electricity_Price']
            logging.info(
                f"Using simple average electricity price: ${avg_electricity_price:.2f}/MWh")
        else:
            avg_electricity_price = 40.0  # Default value
            logging.warning(
                f"No electricity price data available. Using default: ${avg_electricity_price:.2f}/MWh")
        ...
        for i in range(CONSTRUCTION_YEARS):
            # Capital expenditures for H2/battery
            pure_incremental_cash_flows[i] = cash_flows[i]
        ...
        for year in range(CONSTRUCTION_YEARS, project_years):
            ...
            if fixed_om_details:
                ...
                incremental_om_cost = fixed_om_details.get(
                    'base_cost', 0) * (1 + fixed_om_details.get('inflation_rate', 0))**op_year_idx
            ...
            annual_profit = h2_revenue - h2_direct_costs - \
                incremental_om_cost - replacement_cost
        ...
        logging.info(
            f"Pure incremental cash flow pattern: {['Neg' if cf < 0 else 'Pos' if cf > 0 else 'Zero' for cf in pure_incremental_cash_flows]}")

        # Calculate NPV of incremental cash flows
        incremental_metrics['NPV'] = npf.npv(
            DISCOUNT_RATE, pure_incremental_cash_flows)
        ...
        if any(cf > 0 for cf in pure_incremental_cash_flows) and any(cf < 0 for cf in pure_incremental_cash_flows):
            try:
                incremental_metrics['IRR'] = npf.irr(
                    pure_incremental_cash_flows)
                logging.info(
                    f"Incremental IRR: {incremental_metrics['IRR']*100:.2f}%")
            except Exception as irr_e:
        ...
        total_incremental_returns = sum(
            cf for cf in pure_incremental_cash_flows if cf > 0)
        total_incremental_investment = abs(
            sum(cf for cf in pure_incremental_cash_flows if cf < 0))

        if total_incremental_investment > 0:
            incremental_metrics['ROI'] = (
                total_incremental_returns - total_incremental_investment) / total_incremental_investment
            logging.info(
                f"Incremental ROI: {incremental_metrics['ROI']*100:.2f}%")
        else:
        ...
        if annual_h2_production > 0:
            ...
            if total_pv_h2_production_kg > 0:
                incremental_metrics['LCOH'] = pv_incremental_costs / \
                    total_pv_h2_production_kg
                logging.info(
                    f"Incremental LCOH: ${incremental_metrics['LCOH']:.3f}/kg")
            else:
        ...
        if total_capex > 0:
            ...
            logging.info(
                f"Component contributions: Electrolyzer {electrolyzer_pct:.1f}%, Battery {battery_pct:.1f}%")

    except Exception as e:
        logging.error(
            f"Error calculating incremental metrics: {e}", exc_info=True)
        incremental_metrics = {
            'NPV': np.nan,
            'IRR': np.nan,
...
def plot_results(annual_metrics, financial_metrics, cash_flows, plot_output_dir, incremental_metrics=None):
    ...
    ax1 = plt.gca()
    annual_bars = ax1.bar(years, cash_flows, color='royalblue',
                          alpha=0.7, label='Annual Cash Flow')
    ...
    ax2 = ax1.twinx()
    ax2.plot(years, cumulative_cf, 'darkgreen',
    ...
    if incremental_metrics is not None and 'incremental_cash_flows' in incremental_metrics:
        ...
        # Create primary axis for annual cash flows
        ax1 = plt.gca()
        annual_bars = ax1.bar(years, incremental_cash_flows, color='mediumpurple',
                              alpha=0.7, label='Incremental Annual Cash Flow')
        ...
        # Create secondary axis for cumulative cash flow
        ax2 = ax1.twinx()
        ax2.plot(years, cumulative_icf, 'darkgreen', marker='o',
                 label='Cumulative Incremental Cash Flow')
        ...
        # Labels and title
        ax1.set_title(
            'Incremental Cash Flow Profile (H2/Battery System Only)', fontweight='bold')
        ax1.set_xlabel('Year')
        ...
        plt.tight_layout()
        plt.savefig(plot_output_dir / 'incremental_cash_flow_profile.png',
                    dpi=300, bbox_inches='tight')
        ...
        if 'traditional_incremental_cash_flows' in incremental_metrics:
            ...
            plt.title('Comparison of Incremental Cash Flow Approaches',
                      fontweight='bold')
            ...
            plt.tight_layout()
            plt.savefig(plot_output_dir / 'incremental_cash_flow_comparison.png',
                        dpi=300, bbox_inches='tight')
    ...
    if revenue_data_filtered:
        ...
        colors = plt.cm.Spectral(np.linspace(
            0.1, 0.9, len(revenue_data_filtered)))
        ...
        labels_with_values = [
            f"{k} (${v:,.0f})" for k, v in revenue_data_filtered.items()]
    ...
    if cost_data_filtered:
        ...
        # Add legend with values
        labels_with_values = [
            f"{k} (${v:,.0f})" for k, v in cost_data_filtered.items()]
        ...
        plt.axis('equal')  # Equal aspect ratio ensures pie is circular
        plt.tight_layout()
        plt.savefig(plot_output_dir / 'opex_cost_breakdown.png',
                    dpi=300, bbox_inches='tight')
        plt.close()
    else:
    ...
    metrics_to_plot = {
        # Convert to millions
        'NPV ($M)': financial_metrics.get('NPV', np.nan) / 1e6,
    ...
    metrics_plottable = {k: v for k,
                         v in metrics_to_plot.items() if not math.isnan(v)}

    if metrics_plottable:
        plt.figure()

        # Use different colors for different metrics
        colors = ['royalblue', 'forestgreen', 'darkred',
                  'darkorange'][:len(metrics_plottable)]

        # Create horizontal bar chart for better label visibility
        bars = plt.barh(list(metrics_plottable.keys()), list(
            metrics_plottable.values()), color=colors, alpha=0.7)
    ...
    if cf_data_plottable:
        ...
        # Create bar chart
        bars = plt.bar(list(cf_data_plottable.keys()), list(
            cf_data_plottable.values()), color=colors, alpha=0.8)
        ...
        plt.ylim(0, max(list(cf_data_plottable.values()))
                 * 1.2)  # Add space for labels
        plt.grid(True, axis='y', alpha=0.3)
        plt.tight_layout()
        plt.savefig(plot_output_dir / 'capacity_factors.png',
                    dpi=300, bbox_inches='tight')
        plt.close()
    ...
    if incremental_metrics is not None and any(not math.isnan(v) for v in incremental_metrics.values() if isinstance(v, (int, float)) and not isinstance(v, np.ndarray)):
        ...
        if comparison_metrics_plottable:
            ...
            plt.ylabel('Value')
            plt.title(
                'Comparison of Total vs Incremental Financial Metrics', fontweight='bold')
            ...
            for i, metric_key in enumerate(comparison_metrics_plottable.keys()):
                ...
                for j, value in enumerate(metric_values):
                    if not math.isnan(value):
                        ...
                        y_pos = value + \
                            (abs(value) * 0.02) if value > 0 else value - \
                            (abs(value) * 0.15)
                        plt.text(
                            x_pos, y_pos, f'{value:.2f}', ha='center', va='bottom', fontweight='bold')

            plt.grid(True, axis='y', alpha=0.3)
            plt.tight_layout()
            plt.savefig(plot_output_dir / 'incremental_metrics_comparison.png',
                        dpi=300, bbox_inches='tight')
            plt.close()
        ...
        if not math.isnan(incremental_metrics.get('ROI', np.nan)):
            ...
            roi_value = incremental_metrics.get(
                'ROI', 0) * 100  # Convert to percentage
            plt.bar(['Return on Incremental Investment'], [
                    roi_value], color='forestgreen', alpha=0.8)
            ...
            # Add value label
            plt.text(0, roi_value + 1, f'{roi_value:.2f}%',
                     ha='center', va='bottom', fontweight='bold')

            plt.tight_layout()
            plt.savefig(plot_output_dir / 'incremental_roi.png',
                        dpi=300, bbox_inches='tight')
            plt.close()
    ...
    try:
        with open(output_file, 'w') as f:
            ...
            if incremental_metrics:
                ...
                if 'Electrolyzer_CAPEX_Pct' in incremental_metrics and 'Battery_CAPEX_Pct' in incremental_metrics:
                    ...
                    if 'Electrolyzer_CAPEX' in incremental_metrics:
                        f.write(
                            f"  Electrolyzer CAPEX: ${incremental_metrics['Electrolyzer_CAPEX']:,.2f} ")
                        f.write(
                            f"({incremental_metrics['Electrolyzer_CAPEX_Pct']:.1f}% of total)\n")
                    if 'Battery_CAPEX' in incremental_metrics:
                        f.write(
                            f"  Battery CAPEX: ${incremental_metrics['Battery_CAPEX']:,.2f} ")
                        f.write(
                            f"({incremental_metrics['Battery_CAPEX_Pct']:.1f}% of total)\n")
                    f.write("\n")
                ...
                if 'Annual_Electricity_Opportunity_Cost' in incremental_metrics:
                    ...
                    f.write(
                        f"  Annual Electricity Opportunity Cost: ${opportunity_cost:,.2f}\n")
                    f.write(
            ...
            for component, details in sorted(CAPEX_COMPONENTS.items()):
                cost = details.get('base_cost', 0)
                # Add unit if needed
                unit = "/kW" if details.get('size_dependent') else ""
                year = details.get('year', 'Construction End')
                year_label = f" (Year {year})" if isinstance(
                    year, int) else f" ({year})"
            ...
            for component, details in sorted(OM_COMPONENTS.items()):
                ...
                f.write(
                    f"  {component}: {cost_label} (Inflation: {inflation:.1f}%/year)\n")
        ...
        logging.error(
            f"Failed to write report file {output_file}: {e}", exc_info=True)
...
def main():
    ...
    tea_output_file = tea_base_output_dir / \
        f"{current_target_iso}_TEA_Summary.txt"
    ...
    try:
        h2_subsidy_val = float(tea_params.get(
            'hydrogen_subsidy_value_usd_per_kg', 0.0))
        # Default to full lifetime if missing
        h2_subsidy_yrs = int(float(tea_params.get(
            'hydrogen_subsidy_duration_years', PROJECT_LIFETIME_YEARS)))

        # Determine if electrolyzer capacity was fixed
        elec_fixed_val_str = tea_params.get(
            'user_specified_electrolyzer_capacity_MW', None)
        ...
        if 'plant_lifetime_years' in tea_params and tea_params['plant_lifetime_years'] is not None:
            try:
                ...
                logging.warning(
                    f"Invalid plant_lifetime_years value: {tea_params['plant_lifetime_years']}. Using default: {PROJECT_LIFETIME_YEARS}")
        ...
        if 'baseline_nuclear_annual_revenue_USD' in tea_params and tea_params['baseline_nuclear_annual_revenue_USD'] is not None:
            try:
                baseline_revenue = float(
                    tea_params['baseline_nuclear_annual_revenue_USD'])
                logging.info(
                    f"Baseline nuclear annual revenue from sys_data: ${baseline_revenue:,.2f}")
            except (ValueError, TypeError):
                logging.warning(
                    f"Invalid baseline_nuclear_annual_revenue_USD value: {tea_params['baseline_nuclear_annual_revenue_USD']}. Will calculate from hourly results.")
                baseline_revenue = 0.0
        ...
        if 'enable_incremental_analysis' in tea_params and tea_params['enable_incremental_analysis'] is not None:
            try:
                run_incremental_analysis = bool(
                    int(float(tea_params['enable_incremental_analysis'])))
                logging.info(
                    f"Incremental analysis enabled: {run_incremental_analysis}")
            except (ValueError, TypeError):
                logging.warning(
                    f"Invalid enable_incremental_analysis value: {tea_params['enable_incremental_analysis']}. Defaulting to True.")
        ...
        logging.error(
            f"Invalid format for TEA parameters in sys_data: {e}. Using defaults.")
        h2_subsidy_val = 0.0
    ...
    logging.info(
        f"TEA Params - H2 Subsidy: {h2_subsidy_val}/kg, Duration: {h2_subsidy_yrs} yrs, Elec Fixed: {electrolyzer_was_fixed}, Lifetime: {project_lifetime} yrs")

    # Load optimization results
    # Ensure this path points to the correct output location of your main optimization
    results_file = SCRIPT_DIR.parent / \
        f"output/Results_Standardized/{current_target_iso}_Hourly_Results_Comprehensive.csv"

    # Check if file exists, if not try alternative locations
    if not results_file.exists():
        alternate_paths = [
            SCRIPT_DIR.parent /
            f"output/{current_target_iso}_Hourly_Results_Comprehensive.csv",
            SCRIPT_DIR.parent /
            f"output/Results/{current_target_iso}_Hourly_Results_Comprehensive.csv",
            SCRIPT_DIR.parent /
            f"output/Results_{current_target_iso}_Hourly_Results.csv"
        ]
    ...
    if hourly_results is None:
        ...
        print(
            f"Error: Could not find or load results file. Checked: {results_file}")
    ...
    if run_incremental_analysis:
        ...
        if baseline_revenue <= 0:
            try:
                ...
                # Default based on sys_data_advanced.csv
                turbine_max_capacity = float(
                    tea_params.get('pTurbine_max_MW', 380.0))
                logging.info(
                    f"Using turbine maximum capacity: {turbine_max_capacity} MW")
                ...
                if 'EnergyPrice_LMP_USDperMWh' in hourly_results.columns and 'pTurbine_MW' in hourly_results.columns:
                    ...
                    if 'Weighted_Avg_Electricity_Price' in annual_metrics:
                        ...
                        logging.info(
                            f"Using weighted average electricity price: ${avg_energy_price:.2f}/MWh")
                    else:
                        avg_energy_price = hourly_results['EnergyPrice_LMP_USDperMWh'].mean(
                        )
                        logging.info(
                            f"Using simple average electricity price: ${avg_energy_price:.2f}/MWh")

                    baseline_revenue_option1 = avg_energy_price * \
                        turbine_max_capacity * hours_in_year
                    ...
                    weighted_avg_price = (hourly_results['EnergyPrice_LMP_USDperMWh'] * hourly_results['pTurbine_MW']).sum(
                    ) / actual_turbine_output if actual_turbine_output > 0 else 0
                    ...
                    logging.info(
                        f"Calculated baseline revenue from full capacity operation: ${baseline_revenue:,.2f}")
                    logging.info(
                        f"Average electricity price: ${avg_energy_price:.2f}/MWh, Turbine capacity: {turbine_max_capacity} MW, Hours: {hours_in_year}")

                    # Add turbine power data to annual metrics if missing
                    if 'Annual_Turbine_MWh' not in annual_metrics:
                        annual_metrics['Annual_Turbine_MWh'] = turbine_max_capacity * \
                            hours_in_year
                        logging.info(
                            f"Added turbine annual generation: {annual_metrics['Annual_Turbine_MWh']:.2f} MWh")
                else:
                    logging.warning(
                        "Cannot calculate baseline revenue: required columns missing from hourly results.")
                    logging.warning(
                        "Using total energy revenue as baseline revenue.")
                    baseline_revenue = annual_metrics.get('Energy_Revenue', 0)
            except Exception as e:
                logging.error(f"Error calculating baseline revenue: {e}")
                baseline_revenue = annual_metrics.get('Energy_Revenue', 0)
                logging.warning(
                    f"Using total energy revenue as baseline revenue: ${baseline_revenue:,.2f}")

        # Sum up total CAPEX for H2/battery system
        total_capex = sum(details.get('base_cost', 0)
                          for details in CAPEX_COMPONENTS.values())
    ...
    plot_results(annual_metrics, financial_metrics, cash_flows,
                 plot_output_dir, incremental_metrics)

    # Generate report
    generate_report(annual_metrics, financial_metrics,
                    tea_output_file, incremental_metrics)
