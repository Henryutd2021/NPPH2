...
def main(
    ...
    enable_plotting_override: bool = None
):
    """
    ...
    # Prepare configuration overrides - disable plotting by default for all TEA analysis
    # User can explicitly enable plotting with enable_plotting_override=True
    disable_plotting_default = True if enable_plotting_override is None else not enable_plotting_override

    config_overrides = {
        'disable_plotting': disable_plotting_default,  # Disable plotting by default
        'analysis_mode': 'standard_tea'  # Mark as standard TEA analysis
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument('--enable-plotting', action='store_true',
                        help='Enable plotting functionality (plotting is disabled by default)')
    parser.add_argument('--disable-plotting', action='store_true',
                        help='Explicitly disable plotting functionality (default behavior)')
    ...
    enable_plotting_override = None
    if args.enable_plotting and args.disable_plotting:
        print("Error: Cannot both enable and disable plotting. Choose one.")
        sys.exit(1)
    elif args.enable_plotting:
        enable_plotting_override = True
    elif args.disable_plotting:
        # Explicitly disable (though this is already default)
        enable_plotting_override = False
    ...
    try:
        main_success = main(
            ...
            enable_plotting_override=enable_plotting_override
        )
        if not main_success:
