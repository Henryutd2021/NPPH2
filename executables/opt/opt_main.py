...
def _load_imports():
    ...
    from src.logger_utils.logging_setup import logger as _logger
...
def main(argv: list[str] | None = None):
    ...
    if term_cond == TerminationCondition.infeasible and args.debug_infeasibility:
        # Use absolute path to project root's output/logs directory
        infeasible_lp_file = Path(__file__).resolve(
        ).parent.parent.parent / "output" / "logs" / f"{args.iso}_infeasible_model.lp"
        print(f"Infeasible model saved to {infeasible_lp_file}")
    elif term_cond == TerminationCondition.unbounded:
        # Use absolute path to project root's output/logs directory
        unbounded_lp_file = Path(__file__).resolve(
        ).parent.parent.parent / "output" / "logs" / f"{args.iso}_unbounded_model.lp"
