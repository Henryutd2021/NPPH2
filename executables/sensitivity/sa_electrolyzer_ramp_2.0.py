#!/usr/bin/env python3
"""
Sensitivity Analysis Script for Electrolyzer Ramp Rate = 2.0
This script tests the optimization framework with Electrolyzer HTE ramp rate set to 2.0 (both up and down)
"""
...
OUTPUT_DIR = "../../output/sensitivity_analysis/electrolyzer_ramp_2.0"
TEMP_DIR = "../../temp_sensitivity_runs/electrolyzer_ramp_2.0"

# Parameters to modify
ELECTROLYZER_RAMP_UP_RATE = 2.0
ELECTROLYZER_RAMP_DOWN_RATE = 2.0
...
def setup_logging():
    ...
    log_filename = os.path.join(
        OUTPUT_DIR, f"electrolyzer_ramp_2.0_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    logging.basicConfig(
        level=logging.INFO,
    ...
    logging.info(
        "=== Electrolyzer Ramp Rate 2.0 Sensitivity Analysis Started ===")
...
def run_optimization():
    ...
    try:
        ...
        results_filename = os.path.join(
            OUTPUT_DIR, "electrolyzer_ramp_2.0_results.csv")
        summary_filename = os.path.join(
            OUTPUT_DIR, "electrolyzer_ramp_2.0_summary.json")
...
def main():
    ...
    try:
        ...
        logging.info(
            "=== Electrolyzer Ramp Rate 2.0 Sensitivity Analysis Completed ===")
        return 0
