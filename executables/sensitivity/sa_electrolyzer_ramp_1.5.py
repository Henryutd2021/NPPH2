#!/usr/bin/env python3
"""
Sensitivity Analysis Script for Electrolyzer Ramp Rate = 1.5
This script tests the optimization framework with Electrolyzer HTE ramp rate set to 1.5 (both up and down)
"""
...
OUTPUT_DIR = "../output/sensitivity_analysis/electrolyzer_ramp_1.5"
TEMP_DIR = "../temp_sensitivity_runs/electrolyzer_ramp_1.5"

# Parameters to modify
ELECTROLYZER_RAMP_UP_RATE = 1.5
ELECTROLYZER_RAMP_DOWN_RATE = 1.5
...
def setup_logging():
    ...
    log_filename = os.path.join(
        OUTPUT_DIR, f"electrolyzer_ramp_1.5_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    logging.basicConfig(
        level=logging.INFO,
    ...
    logging.info(
        "=== Electrolyzer Ramp Rate 1.5 Sensitivity Analysis Started ===")
...
def run_optimization():
    ...
    try:
        ...
        results_filename = os.path.join(
            OUTPUT_DIR, "electrolyzer_ramp_1.5_results.csv")
        summary_filename = os.path.join(
            OUTPUT_DIR, "electrolyzer_ramp_1.5_summary.json")
...
def main():
    ...
    try:
        ...
        logging.info(
            "=== Electrolyzer Ramp Rate 1.5 Sensitivity Analysis Completed ===")
        return 0
