#!/usr/bin/env python3
"""
Sensitivity Analysis Script for Turbine Ramp Rate = 1.5
This script tests the optimization framework with Turbine ramp rate set to 1.5 (both up and down)
"""
...
OUTPUT_DIR = "../../output/sensitivity_analysis/turbine_ramp_1.5"
TEMP_DIR = "../../temp_sensitivity_runs/turbine_ramp_1.5"

# Parameters to modify
TURBINE_RAMP_UP_RATE = 1.5
TURBINE_RAMP_DOWN_RATE = 1.5
...
def setup_logging():
    ...
    log_filename = os.path.join(
        OUTPUT_DIR, f"turbine_ramp_1.5_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    logging.basicConfig(
        level=logging.INFO,
    ...
    logging.info("=== Turbine Ramp Rate 1.5 Sensitivity Analysis Started ===")
...
def run_optimization():
    ...
    try:
        ...
        results_filename = os.path.join(
            OUTPUT_DIR, "turbine_ramp_1.5_results.csv")
        summary_filename = os.path.join(
            OUTPUT_DIR, "turbine_ramp_1.5_summary.json")
...
def main():
    ...
    try:
        ...
        logging.info(
            "=== Turbine Ramp Rate 1.5 Sensitivity Analysis Completed ===")
        return 0
