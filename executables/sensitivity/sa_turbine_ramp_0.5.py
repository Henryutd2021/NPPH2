#!/usr/bin/env python3
"""
Sensitivity Analysis Script for Turbine Ramp Rate = 0.5
This script tests the optimization framework with Turbine ramp rate set to 0.5 (both up and down)
"""
...
OUTPUT_DIR = "../output/sensitivity_analysis/turbine_ramp_0.5"
TEMP_DIR = "../temp_sensitivity_runs/turbine_ramp_0.5"

# Parameters to modify
TURBINE_RAMP_UP_RATE = 0.5
TURBINE_RAMP_DOWN_RATE = 0.5
...
def setup_logging():
    ...
    log_filename = os.path.join(
        OUTPUT_DIR, f"turbine_ramp_0.5_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    logging.basicConfig(
        level=logging.INFO,
    ...
    logging.info("=== Turbine Ramp Rate 0.5 Sensitivity Analysis Started ===")
...
def run_optimization():
    ...
    try:
        ...
        # Modify turbine ramp rates
        success1 = modify_sys_data(BASE_SYS_DATA_FILE, temp_sys_file,
                                   "Turbine_RampUp_Rate_Percent_per_Min", TURBINE_RAMP_UP_RATE)
        success2 = modify_sys_data(temp_sys_file, temp_sys_file,
                                   "Turbine_RampDown_Rate_Percent_per_Min", TURBINE_RAMP_DOWN_RATE)
        ...
        results_filename = os.path.join(
            OUTPUT_DIR, "turbine_ramp_0.5_results.csv")
        summary_filename = os.path.join(
            OUTPUT_DIR, "turbine_ramp_0.5_summary.json")
...
def main():
    ...
    try:
        ...
        # Run optimization
        logging.info(
            f"Starting optimization with Turbine RampUp Rate = {TURBINE_RAMP_UP_RATE}, RampDown Rate = {TURBINE_RAMP_DOWN_RATE}")
        ...
        logging.info(
            "=== Turbine Ramp Rate 0.5 Sensitivity Analysis Completed ===")
        return 0
