#!/usr/bin/env python3
"""
Sensitivity Analysis Script for Turbine Ramp Rate = 2.0
"""
...
OUTPUT_DIR = "../output/sensitivity_analysis/turbine_ramp_2.0"
TEMP_DIR = "../temp_sensitivity_runs/turbine_ramp_2.0"

# Parameters to modify
TURBINE_RAMP_UP_RATE = 2.0
TURBINE_RAMP_DOWN_RATE = 2.0
...
def setup_logging():
    ...
    log_filename = os.path.join(
        OUTPUT_DIR, f"turbine_ramp_2.0_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    logging.basicConfig(
        level=logging.INFO,
    ...
    logging.info("=== Turbine Ramp Rate 2.0 Sensitivity Analysis Started ===")
...
def run_optimization():
    ...
    try:
        ...
        results_filename = os.path.join(
            OUTPUT_DIR, "turbine_ramp_2.0_results.csv")
        summary_filename = os.path.join(
            OUTPUT_DIR, "turbine_ramp_2.0_summary.json")
...
def main():
    ...
    try:
        ...
        # Log summary
        if result['summary_results'] is not None:
        ...
        logging.info(
            "=== Turbine Ramp Rate 2.0 Sensitivity Analysis Completed Successfully ===")
        return 0
