#!/usr/bin/env python3
"""
Sensitivity Analysis Script for Electrolyzer Capacity = 250MW
This script tests the optimization framework with Electrolyzer capacity upper bound set to 250MW
"""
...
def setup_logging():
    ...
    log_filename = os.path.join(OUTPUT_DIR, "electrolyzer_capacity_250.log")
    logging.basicConfig(
        level=logging.INFO,
    ...
    logging.info("=== Electrolyzer Capacity 250MW Sensitivity Analysis Started ===")
...
def prepare_directories():
    ...
    if os.path.exists(BASE_INPUT_DIR):
        ...
        raise FileNotFoundError(f"Base input directory not found: {BASE_INPUT_DIR}")
...
def modify_sys_data(original_file, temp_file, param_name, new_value):
    ...
    try:
        ...
        if param_name not in df.index:
            logging.warning(f"Parameter '{param_name}' not found in {original_file}")
            return False

        old_value = df.loc[param_name, 'Value']
        logging.info(f"Modifying '{param_name}' from {old_value} to {new_value}")
...
def run_optimization():
    ...
    try:
        ...
        model = create_model(data, TARGET_ISO, simulate_dispatch=SIMULATE_AS_DISPATCH_EXECUTION)
        if model is None:
        ...
        if not solver.available():
            logging.warning("Gurobi not available, trying alternative solvers...")
        ...
        results_df, summary_results = extract_results(model, TARGET_ISO, output_dir=OUTPUT_DIR)

        # Save results
        results_filename = os.path.join(OUTPUT_DIR, "electrolyzer_capacity_250_results.csv")
        summary_filename = os.path.join(OUTPUT_DIR, "electrolyzer_capacity_250_summary.json")
...
def main():
    ...
    try:
        ...
        # Run optimization
        logging.info(f"Starting optimization with Electrolyzer Capacity = {ELECTROLYZER_CAPACITY_MW} MW")
        ...
        logging.info(f"Optimization completed successfully in {elapsed_time:.2f} seconds")
        ...
        logging.info("=== Electrolyzer Capacity 250MW Sensitivity Analysis Completed ===")
        return 0
