...
from utils import (
    # Used for fixed values like H2_value, costs, market params etc.
    get_param,
...
def EnergyRevenue_rule(m):
    ...
    try:
        if not hasattr(m, 'pIES') or not hasattr(m, 'energy_price') or not hasattr(m, 'delT_minutes'):
            logger.error(
                "Missing pIES, energy_price, or delT_minutes for EnergyRevenue_rule definition.")
            return 0.0
        ...
        logger.critical(
            f"CRITICAL Error defining EnergyRevenue_rule expression: {e}", exc_info=True)
        raise
...
def HydrogenRevenue_rule(m):
    ...
    try:
        ...
        logger.critical(
            f"CRITICAL Missing var/param for HydrogenRevenue rule: {e}.", exc_info=True)
        raise e
    except Exception as e:
        logger.critical(
            f"CRITICAL Error defining HydrogenRevenue rule: {e}", exc_info=True)
        raise
...
def _get_total_deployed_sum_for_service(m, t, internal_service):
    ...
    if enable_electrolyzer:
        total_deployed += get_symbolic_as_deployed_sum(
            m, t, [internal_service], 'Electrolyzer')
    if enable_battery:
        total_deployed += get_symbolic_as_deployed_sum(
            m, t, [internal_service], 'Battery')
    if enable_npp and (enable_electrolyzer or enable_battery):  # Turbine AS condition
        total_deployed += get_symbolic_as_deployed_sum(
            m, t, [internal_service], 'Turbine')
...
def _symbolic_hourly_spp_revenue_expr(m, t):
    ...
    if bid_var is not None and bid_var.is_indexed() and t in bid_var.index_set():
        ...
        if simulate_dispatch:
            deployed_amount_expr = _get_total_deployed_sum_for_service(
                m, t, internal_service)
            energy_perf_payment = deployed_amount_expr * lmp
        else:  # Bidding mode
            # Example factor name
            mileage = get_param(
                m, f'mileage_factor_{service_iso}', t, default=1.0)
            # Example factor name
            perf = get_param(
                m, f'performance_factor_{service_iso}', t, default=1.0)
...
def _symbolic_hourly_caiso_revenue_expr(m, t):
    ...
    if bid_var is not None and bid_var.is_indexed() and t in bid_var.index_set():
        ...
        mileage_price = get_param(
            m, 'p_RegMileage', t, default=0.0)  # Assumed name
        # Factor specific to RegU
        mileage_factor = get_param(
            m, f'mileage_factor_{service_iso}', t, default=1.0)
        mileage_payment = 0.0
        if simulate_dispatch:
    ...
    # Reserves
    reserve_map = {'Spin': 'SR', 'NSpin': 'NSR', 'RMU': 'RampUp',
                   'RMD': 'RampDown'}  # CAISO specific mapping
    for service_iso, internal_service in reserve_map.items():
        bid_var = getattr(m, f'Total_{internal_service}', None)
        if bid_var is not None and bid_var.is_indexed() and t in bid_var.index_set():
            bid = bid_var[t]
            ...
            if simulate_dispatch:
                deployed_amount_expr = _get_total_deployed_sum_for_service(
                    m, t, internal_service)
                # Simplified, likely incorrect for CAISO DA
                energy_payment = deployed_amount_expr * lmp
            else:  # Bidding mode
...
def _symbolic_hourly_pjm_revenue_expr(m, t):
    ...
    if bid_var is not None and bid_var.is_indexed() and t in bid_var.index_set():
        ...
        # Performance price
        mcp_perf = get_param(m, 'p_RegPerf', t, default=0.0)
        # Locational adder? Unlikely for PJM Reg.
        adder = get_param(m, f'loc_{service_iso}', t, default=0.0)
        win_rate = get_param(m, f'winning_rate_{service_iso}', t, default=1.0)
        mileage_ratio = get_param(
            m, 'mileage_ratio', t, default=1.0)  # PJM specific factor
        perf_score = get_param(m, 'performance_score', t,
                               default=1.0)  # PJM specific factor
        ...
        if simulate_dispatch:
            ...
            perf_payment = reg_bid * perf_score * \
                mileage_ratio * mcp_perf  # Estimate based on bid
    ...
    reserve_map = {'Syn': 'SR', 'Rse': 'NSR',
                   'TMR': 'ThirtyMin'}  # PJM specific mapping
    for service_iso, internal_service in reserve_map.items():
        bid_var = getattr(m, f'Total_{internal_service}', None)
        if bid_var is not None and bid_var.is_indexed() and t in bid_var.index_set():
            bid = bid_var[t]
...
def _symbolic_hourly_nyiso_revenue_expr(m, t):
    ...
    if bid_var is not None and bid_var.is_indexed() and t in bid_var.index_set():
        ...
        # Capacity price (p_RegC_NYISO)
        mcp_cap = get_param(m, f'p_{service_iso}', t, default=0.0)
        adder = get_param(m, f'loc_{service_iso}',
                          t, default=0.0)  # Locational adder?
        win_rate = get_param(m, f'winning_rate_{service_iso}', t, default=1.0)
        cap_payment = reg_bid * win_rate * mcp_cap
    ...
    reserve_map = {'Spin10': 'SR', 'NSpin10': 'NSR',
                   'Res30': 'ThirtyMin'}  # NYISO specific mapping
    for service_iso, internal_service in reserve_map.items():
        bid_var = getattr(m, f'Total_{internal_service}', None)
        if bid_var is not None and bid_var.is_indexed() and t in bid_var.index_set():
            bid = bid_var[t]
...
def _symbolic_hourly_isone_revenue_expr(m, t):
    ...
    # ISONE specific mapping (using internal names)
    ...
    for service_iso, internal_service in reserve_map.items():  # service_iso is key used for param lookup
        ...
        if bid_var is not None and bid_var.is_indexed() and t in bid_var.index_set():
            bid = bid_var[t]
            mcp = get_param(m, f'p_{service_iso}', t, default=0.0)
            # ISONE has Reserve Constraint Penalty Factors (RCPFs) - complex
            adder = get_param(m, f'loc_{service_iso}', t, default=0.0)
            win_rate = get_param(
                m, f'winning_rate_{service_iso}', t, default=1.0)
            deploy_factor = get_param(
...
def _symbolic_hourly_miso_revenue_expr(m, t):
    ...
    service_iso = 'Reg'
    internal_service_up = 'RegUp'
    internal_service_down = 'RegDown'
    ...
    if (bid_up_var is not None and bid_up_var.is_indexed() and t in bid_up_var.index_set() and
        ...
        # Assumes single p_Reg_MISO price
        mcp_cap = get_param(m, f'p_{service_iso}', t, default=0.0)
        adder = get_param(m, f'loc_{service_iso}', t, default=0.0)
        win_rate = get_param(m, f'winning_rate_{service_iso}', t, default=1.0)
        ...
        energy_perf_payment = 0.0
        if simulate_dispatch:
            deployed_up_expr = _get_total_deployed_sum_for_service(
                m, t, internal_service_up)
            deployed_down_expr = _get_total_deployed_sum_for_service(
                m, t, internal_service_down)
            # Highly simplified proxy
    ...
    reserve_map = {'Spin': 'SR', 'Sup': 'NSR', 'STR': 'ThirtyMin',
                   'RamU': 'RampUp', 'RamD': 'RampDown'}  # MISO specific mapping
    for service_iso, internal_service in reserve_map.items():
        bid_var = getattr(m, f'Total_{internal_service}', None)
        if bid_var is not None and bid_var.is_indexed() and t in bid_var.index_set():
            bid = bid_var[t]
...
AncillaryRevenue_SPP_rule = AncillaryRevenue_rule_factory(
    _symbolic_hourly_spp_revenue_expr)
AncillaryRevenue_CAISO_rule = AncillaryRevenue_rule_factory(
    _symbolic_hourly_caiso_revenue_expr)
AncillaryRevenue_ERCOT_rule = AncillaryRevenue_rule_factory(
    _symbolic_hourly_ercot_revenue_expr)
AncillaryRevenue_PJM_rule = AncillaryRevenue_rule_factory(
    _symbolic_hourly_pjm_revenue_expr)
AncillaryRevenue_NYISO_rule = AncillaryRevenue_rule_factory(
    _symbolic_hourly_nyiso_revenue_expr)
AncillaryRevenue_ISONE_rule = AncillaryRevenue_rule_factory(
    _symbolic_hourly_isone_revenue_expr)
AncillaryRevenue_MISO_rule = AncillaryRevenue_rule_factory(
    _symbolic_hourly_miso_revenue_expr)
    ...
    try:
        ...
        if enable_electrolyzer and hasattr(m, 'vom_electrolyzer') and hasattr(m, 'pElectrolyzer'):
            cost_vom_electrolyzer_expr = sum(
                m.vom_electrolyzer * m.pElectrolyzer[t] * time_factor for t in m.TimePeriods)  # Based on actual power
        ...
        logger.critical(
            f"CRITICAL Missing parameter/variable for OpexCost rule: {e}", exc_info=True)
        raise e
    except Exception as e:
        logger.critical(
