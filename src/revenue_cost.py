...
def energy_revenue_rule(m):
    ...
    try:
        ...
        logger.critical(
            f"CRITICAL Error defining EnergyRevenue_rule expression: {e}",
            exc_info=True,
        )
        raise
...
def hydrogen_revenue_rule(m):
    ...
    try:
        ...
        logger.critical(
            f"CRITICAL Missing var/param for HydrogenRevenue rule: {e}.",
            exc_info=True,
        )
        raise
    except Exception as e:
...
def _get_total_deployed_sum_for_service(m, t, internal_service_name):
    ...
    if enable_electrolyzer:
        total_deployed += get_symbolic_as_deployed_sum(
            m, t, [internal_service_name], "Electrolyzer"
        )
    if enable_battery:
        total_deployed += get_symbolic_as_deployed_sum(
            m, t, [internal_service_name], "Battery"
        )
    if enable_npp and (enable_electrolyzer or enable_battery):
        total_deployed += get_symbolic_as_deployed_sum(
            m, t, [internal_service_name], "Turbine"
        )
    return total_deployed


def _calculate_standard_regulation_revenue(
    m, t, iso_service_name, internal_service_name, lmp
):
    """
...
def _symbolic_hourly_spp_revenue_expr(m, t):
    ...
    hourly_revenue_rate_expr += _calculate_standard_regulation_revenue(
        m, t, "RegU", "RegUp", lmp
    )
    # Regulation Down (using standard calculation)
    hourly_revenue_rate_expr += _calculate_standard_regulation_revenue(
        m, t, "RegD", "RegDown", lmp
    )
...
def ancillary_revenue_rule_factory(iso_hourly_revenue_func):
    ...
    def _rule(m):
        ...
        try:
            ...
            logger.critical(
                f"CRITICAL Error defining AncillaryRevenue rule using {iso_hourly_revenue_func.__name__}: {e}",
                exc_info=True,
            )
            raise
...
AncillaryRevenue_SPP_rule = ancillary_revenue_rule_factory(
    _symbolic_hourly_spp_revenue_expr
)
AncillaryRevenue_CAISO_rule = ancillary_revenue_rule_factory(
    _symbolic_hourly_caiso_revenue_expr
)
AncillaryRevenue_ERCOT_rule = ancillary_revenue_rule_factory(
    _symbolic_hourly_ercot_revenue_expr
)
AncillaryRevenue_PJM_rule = ancillary_revenue_rule_factory(
    _symbolic_hourly_pjm_revenue_expr
)
AncillaryRevenue_NYISO_rule = ancillary_revenue_rule_factory(
    _symbolic_hourly_nyiso_revenue_expr
)
AncillaryRevenue_ISONE_rule = ancillary_revenue_rule_factory(
    _symbolic_hourly_isone_revenue_expr
)
AncillaryRevenue_MISO_rule = ancillary_revenue_rule_factory(
...
def opex_cost_rule(m):
    ...
    try:
        ...
        if (
            enable_electrolyzer
            and hasattr(m, "vom_electrolyzer")
            and hasattr(m, "pElectrolyzer")
        ):
            cost_vom_electrolyzer_expr = sum(
                m.vom_electrolyzer * m.pElectrolyzer[t] * time_factor
                for t in m.TimePeriods
            )
        ...
        if (
            enable_battery
            and hasattr(m, "vom_battery_per_mwh_cycled")
            and hasattr(m, "BatteryCharge")
            and hasattr(m, "BatteryDischarge")
        ):
            cost_vom_battery_expr = sum(
                m.vom_battery_per_mwh_cycled
                * (m.BatteryCharge[t] + m.BatteryDischarge[t])
                / 2.0
                * time_factor
                for t in m.TimePeriods
            )
        ...
        if (
            enable_electrolyzer
            and hasattr(m, "cost_water_per_kg_h2")
            and hasattr(m, "mHydrogenProduced")
        ):
            cost_water_expr = sum(
                m.cost_water_per_kg_h2 * m.mHydrogenProduced[t] * time_factor
                for t in m.TimePeriods
            )
        ...
        if (
            enable_electrolyzer
            and hasattr(m, "cost_electrolyzer_ramping")
            and hasattr(m, "pElectrolyzerRampPos")
            and hasattr(m, "pElectrolyzerRampNeg")
        ):
            cost_ramping_expr = sum(
                m.cost_electrolyzer_ramping
        ...
        if (
            enable_h2_storage
            and hasattr(m, "vom_storage_cycle")
            and hasattr(m, "H2_to_storage")
            and hasattr(m, "H2_from_storage")
        ):
            cost_storage_cycle_expr = sum(
                m.vom_storage_cycle
                * (m.H2_to_storage[t] + m.H2_from_storage[t])
                * time_factor
                for t in m.TimePeriods
            )
        ...
        if (
            ...
            and hasattr(m, "cost_startup_electrolyzer")
            and hasattr(m, "vElectrolyzerStartup")
        ):
            cost_startup_expr = sum(
                m.cost_startup_electrolyzer * m.vElectrolyzerStartup[t]
                for t in m.TimePeriods
            )
        ...
        logger.critical(
            f"CRITICAL Missing parameter/variable for OpexCost rule: {e}",
            exc_info=True,
        )
        raise
    except Exception as e:
