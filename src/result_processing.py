...
try:
    ...
    def get_as_components_helper(m, t): # Placeholder using imported utils
        ...
        for service in up_services:
            ...
            if total_var is not None: up_bids += get_var_value(total_var, t, default=0.0) # Use util
        for service in down_services:
            total_var = getattr(m, f'Total_{service}', None)
            if total_var is not None: down_bids += get_var_value(total_var, t, default=0.0) # Use util
...
def calculate_hourly_as_revenue(m: pyo.ConcreteModel, t: int) -> float:
    ...
    try:
        ...
        if iso_suffix == 'SPP':
            ...
            bid_var = getattr(m, f'Total_{internal_service}', None); bid = get_var_value(bid_var, t, default=0.0) # Use util
            mcp_cap = get_param(m, f'p_{service_iso}', t, default=0.0); adder = get_param(m, f'loc_{service_iso}', t, default=0.0); win_rate = get_param(m, f'winning_rate_{service_iso}', t, default=1.0) # Use util
            cap_payment = bid * win_rate * mcp_cap
            energy_perf_payment = 0.0
            if simulate_dispatch:
                deployed_amount = get_total_deployed_as(m, t, internal_service) # Use util
            ...
            bid_var = getattr(m, f'Total_{internal_service}', None); bid = get_var_value(bid_var, t, default=0.0)
            mcp_cap = get_param(m, f'p_{service_iso}', t, default=0.0); adder = get_param(m, f'loc_{service_iso}', t, default=0.0); win_rate = get_param(m, f'winning_rate_{service_iso}', t, default=1.0)
            cap_payment = bid * win_rate * mcp_cap
            energy_perf_payment = 0.0
            if simulate_dispatch:
            ...
            for service_iso, internal_service in reserve_map.items():
                ...
                mcp = get_param(m, f'p_{service_iso}', t, default=0.0); adder = get_param(m, f'loc_{service_iso}', t, default=0.0); win_rate = get_param(m, f'winning_rate_{service_iso}', t, default=1.0)
...
def extract_results(model: pyo.ConcreteModel, target_iso: str, output_dir: str = '../output/Results_Standardized'):
    ...
    if not hasattr(model, 'TARGET_ISO') or model.TARGET_ISO != target_iso:
         logger.warning(f"Model TARGET_ISO mismatches function arg ('{target_iso}'). Using model attribute if available, else function arg.")
    ...
    can_provide_as_local = getattr(model, 'CAN_PROVIDE_ANCILLARY_SERVICES', False) # Use flag from model if set
    ...
    time_factor = get_param(model, 'delT_minutes', default=60.0) / 60.0 # Use util
    if time_factor <= 0:
        logger.error("Invalid time_factor (<=0) obtained. Cannot proceed with result extraction.")
        return pd.DataFrame(), {}
    ...
    enable_lte = getattr(model, 'LTE_MODE', ENABLE_LOW_TEMP_ELECTROLYZER) # Use LTE_MODE if available
    ...
    var_extract_list = [
        ('pIES', 'pIES_MW', True, 0.0), # (Internal Var Name, Output Col Name, Always Extract?, Default if Disabled)
        ...
        ('uElectrolyzer', 'Electrolyzer_Status(0=off,1=on)', enable_startup_shutdown, (1.0 if enable_electrolyzer else 0.0)), # Default depends if elec enabled
        ...
        ('pElectrolyzerRampPos', 'pElectrolyzerRampPos_MW', enable_electrolyzer, 0.0), # Check hasattr later
        ('pElectrolyzerRampNeg', 'pElectrolyzerRampNeg_MW', enable_electrolyzer, 0.0), # Check hasattr later
        ('qSteamElectrolyzerRampPos', 'qSteamElectrolyzerRampPos_MWth', enable_electrolyzer and not enable_lte, 0.0), # Check hasattr later
        ('qSteamElectrolyzerRampNeg', 'qSteamElectrolyzerRampNeg_MWth', enable_electrolyzer and not enable_lte, 0.0), # Check hasattr later
    ...
    for var_name, col_name, is_enabled, default_val in var_extract_list:
        if is_enabled:
            var_component = getattr(model, var_name, None) # Use getattr to handle missing optional vars
            if var_component is not None:
                 hourly_data[col_name] = [get_var_value(var_component, t, default=default_val) for t in hours] # Use util
            # elif not is_enabled: # This condition is redundant with the outer 'if is_enabled'
                 ...
                 logger.warning(f"Variable '{var_name}' expected but not found on model. Filling '{col_name}' with default {default_val}.")
                 hourly_data[col_name] = [default_val] * len(hours)
    ...
    all_as_components_labels = ['Electrolyzer', 'Battery', 'Turbine', 'Total'] # Include Total
        ...
        for service in as_service_list:
            ...
            if can_provide_as_local and hasattr(model, base_name):
                ...
                if isinstance(var_comp, pyo.Var):
                    hourly_data[col_name] = [get_var_value(var_comp, t, default=0.0) for t in hours] # Use util
                else: # It's a Param (likely fixed to 0)
                    hourly_data[col_name] = [get_param(model, base_name, t, default=0.0) for t in hours] # Use get_param for Param
        ...
        for comp in components_providing_as: # Use list of components actually providing AS
            for service in as_service_list:
                ...
                if hasattr(model, deployed_var_name):
                    hourly_data[col_name] = [get_var_value(getattr(model, deployed_var_name), t, default=0.0) for t in hours] # Use util
    ...
    hourly_data['EnergyPrice_LMP_USDperMWh'] = [get_param(model, 'energy_price', t, default=0.0) for t in hours] # Use util
    if can_provide_as_local:
        ...
        if target_iso_local in iso_service_map:
            for service_iso_key in iso_service_map[target_iso_local]: # Use the key from the map (e.g., 'RegU', 'Syn')

                # Define expected param types based on the key
                param_types_to_extract = []
                is_factor_type_param = any(f in service_iso_key.lower() for f in ['factor', 'score', 'ratio'])

                if not is_factor_type_param: # Add standard price/adder params if not a factor key
                     param_types_to_extract.extend([('p', 0.0), ('loc', 0.0), ('winning_rate', 1.0), ('deploy_factor', 0.0)])
                ...
                if target_iso_local == 'PJM':
                     ...
                     if service_iso_key == 'performance_score': param_types_to_extract.append(('performance_score', 1.0))
                     if service_iso_key == 'mileage_ratio': param_types_to_extract.append(('mileage_ratio', 1.0))
                ...
                for param_prefix, default_val in param_types_to_extract:
                     ...
                     if param_prefix in ['performance_score', 'mileage_ratio'] and target_iso_local == 'PJM':
                          ...
                          param_base_name_on_model = f"{param_prefix}" # e.g., 'performance_score'
                          output_col_name = f"{param_prefix}_{target_iso_local}" # e.g., performance_score_PJM
                     else:
                          # Standard case: param name includes service_iso_key
                          param_base_name_on_model = f"{param_prefix}_{service_iso_key}" # e.g., p_RegU, loc_Spin
                          output_col_name = f"{param_prefix}_{service_iso_key}_{target_iso_local}" # e.g., p_RegU_SPP
    ...
    try:
        ...
        for k, v in hourly_data.items():
             logger.debug(f"Length of '{k}': {len(v) if isinstance(v, list) else 'N/A'}")
    ...
    cost_calc_list = [
        ...
        ('Cost_VOM_Battery_USD', enable_battery, 'vom_battery_per_mwh_cycled', ['Battery_Charge_MW', 'Battery_Discharge_MW']), # Special case: needs sum + division?
        ('Cost_Water_USD', enable_electrolyzer, 'cost_water_per_kg_h2', 'mHydrogenProduced_kg_hr'), # Needs time factor? No, rate is per kg
        ('Cost_Ramping_USD', enable_electrolyzer, 'cost_electrolyzer_ramping', ['pElectrolyzerRampPos_MW', 'pElectrolyzerRampNeg_MW']), # Special case: no time factor
        ('Cost_Storage_Cycle_USD', enable_h2_storage, 'vom_storage_cycle', ['H2_to_Storage_Input_kg_hr', 'H2_from_Storage_kg_hr']), # Special case: needs time factor? No, rate is per kg
        ('Cost_Startup_USD', enable_startup_shutdown, 'cost_startup_electrolyzer', 'Electrolyzer_Startup(0=no,1=yes)') # Special case: no time factor
    ]

    cost_cols_for_total = []
    for cost_col, is_enabled, param_name, source_col_or_list in cost_calc_list:
        if is_enabled:
            cost_rate_param = get_param(model, param_name, default=0.0) # Use util
            if cost_rate_param > 1e-9: # Only calculate if cost rate is non-zero
                if isinstance(source_col_or_list, list): # Multiple source columns
                    if all(col in results_df.columns for col in source_col_or_list):
                         ...
                         if cost_col == 'Cost_VOM_Battery_USD': # Rate is per MWh cycled, sum(charge+discharge)/2 ? Assume rate is per MWh thruput (charge OR discharge)
                             results_df[cost_col] = source_sum * cost_rate_param * time_factor / 2.0 # Divide by 2 if rate is per round trip MWh
                         elif cost_col == 'Cost_Ramping_USD' or cost_col == 'Cost_Startup_USD': # Costs applied per event/ramp, not per hour duration
                              results_df[cost_col] = source_sum * cost_rate_param
                         elif cost_col == 'Cost_Storage_Cycle_USD': # Rate is $/kg cycled
                              results_df[cost_col] = source_sum * cost_rate_param # Sum is kg/hr, rate is $/kg -> $/hr. Multiply by time_factor
                              results_df[cost_col] = results_df[cost_col] * time_factor
                    ...
                    if source_col in results_df.columns:
                        if cost_col == 'Cost_Water_USD': # Rate is $/kg, source is kg/hr
                             results_df[cost_col] = results_df[source_col] * cost_rate_param * time_factor # Total kg * $/kg
                        else: # VOMs: MW * $/MWh * hr -> $
                             results_df[cost_col] = results_df[source_col] * cost_rate_param * time_factor
                        cost_cols_for_total.append(cost_col)
                    else: results_df[cost_col] = 0.0
            else: results_df[cost_col] = 0.0 # Cost rate is zero
        else: results_df[cost_col] = 0.0 # Feature disabled
    ...
    results_df['Cost_HourlyOpex_Total_USD'] = results_df[cost_cols_for_total].sum(axis=1)
    ...
    if enable_electrolyzer:
        cost_elec_cap_param = get_param(model, 'cost_electrolyzer_capacity', default=0.0) # Use util
    ...
    if enable_battery:
        cost_batt_cap_mwh_yr = get_param(model, 'BatteryCapex_USD_per_MWh_year', default=0.0) # Use util
        cost_batt_pow_mw_yr = get_param(model, 'BatteryCapex_USD_per_MW_year', default=0.0) # Use util
        cost_batt_fom_mwh_yr = get_param(model, 'BatteryFixedOM_USD_per_MWh_year', default=0.0) # Use util
    ...
    summary_results['Objective_Value_USD'] = get_var_value(getattr(model, 'TotalProfit_Objective', None), default=None) # Use util
    ...
    if elec_capacity_val > 1e-6 and 'pElectrolyzer_MW' in results_df.columns:
        ...
        capacity_factor_actual = avg_elec_power_actual / elec_capacity_val if elec_capacity_val > 0 else 0.0
    ...
    if 'Electrolyzer_Startup(0=no,1=yes)' in hourly_data: # Check original extracted list
    ...
    if not summary_results:
        ...
        try:
            ...
            with open(output_summary_path, 'w') as f:
                ...
                key_order = [k for k in summary_results.keys() if k not in ['Simulation_Mode', 'Target_ISO']]
                ...
                if obj_val_key in key_order: key_order.remove(obj_val_key)
                if calc_prof_key in key_order: key_order.remove(calc_prof_key)
                ...
                if obj_val is not None: f.write(f"{obj_val_key}: {obj_val:,.4f}\n")
                else: f.write(f"{obj_val_key}: N/A (Solver Failed?)\n")
                if calc_prof is not None: f.write(f"{calc_prof_key}: {calc_prof:,.4f}\n")
                else: f.write(f"{calc_prof_key}: N/A (Calculation Error?)\n")
