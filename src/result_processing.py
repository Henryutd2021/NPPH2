...
from config import (
    ENABLE_NUCLEAR_GENERATOR, ENABLE_ELECTROLYZER, ENABLE_BATTERY,
    ENABLE_H2_STORAGE, ENABLE_STARTUP_SHUTDOWN,
    ENABLE_ELECTROLYZER_DEGRADATION_TRACKING
)


def _get_regulation_revenue_component_results(m: pyo.ConcreteModel, t: int, lmp: float,
                                              iso_service_name: str, internal_service_name: str,
                                              simulate_dispatch: bool) -> float:
    """
    Calculates revenue for a single regulation service component (Up or Down) post-solve.
    ...
    # In dispatch simulation, even if bid was zero (e.g. fixed), deployed could be non-zero if forced by other constraints,
    # but typically awarded capacity (bid*win_rate) would be zero, leading to zero capacity payment.
    ...
    win_rate_val = get_param(m, f'winning_rate_{iso_service_name}', t, default=1.0)
    ...
    if simulate_dispatch:
        ...
        deployed_amount_val = get_total_deployed_as(m, t, internal_service_name)
        energy_perf_payment = deployed_amount_val * lmp
    else:
        # Standardized calculation for all ISOs.
        # Assumes model.py loaded parameters like 'mileage_factor_RegUp_PJM', 'performance_factor_RegUp_PJM' etc.
        mileage_val = get_param(m, f'mileage_factor_{iso_service_name}', t, default=1.0)
        perf_val = get_param(m, f'performance_factor_{iso_service_name}', t, default=1.0)
...
def calculate_hourly_as_revenue(m: pyo.ConcreteModel, t: int) -> float:
    """
    Calculates hourly AS revenue rate ($/hr) using ISO-specific logic and numerical results.
    ...
    try:
        ...
        # SPP, CAISO, ERCOT use 'RegU'/'RegD' as iso_service_name for params
            ...
            total_hourly_as_revenue_rate += _get_regulation_revenue_component_results(
                m, t, lmp_val, 'RegU', 'RegUp', simulate_dispatch # internal_service_name is always RegUp/RegDown
            )
            total_hourly_as_revenue_rate += _get_regulation_revenue_component_results(
                m, t, lmp_val, 'RegD', 'RegDown', simulate_dispatch
            )
        
        # Reserve Revenue Calculation (remains largely the same, uses deploy_factor or deployed amount)
        reserve_map = {}
        if iso_suffix == 'SPP':
        ...
        for service_iso, internal_service in reserve_map.items():
            ...
            win_rate_val = get_param(m, f'winning_rate_{service_iso}', t, default=1.0)
            
            cap_payment = bid_val * win_rate_val * mcp_val
            energy_payment = 0.0
            if simulate_dispatch:
                deployed_amount_val = get_total_deployed_as(m, t, internal_service)
                energy_payment = deployed_amount_val * lmp_val
            else:
                deploy_factor_val = get_param(m, f'deploy_factor_{service_iso}', t, default=0.0)
        ...
        logger.error(f"Error during hourly AS revenue calculation for t={t}, ISO={iso_suffix}: {e}", exc_info=True)
        return 0.0
...
def extract_results(model: pyo.ConcreteModel, target_iso: str, output_dir: str = '../output/Results_Standardized'):
    ...
    model.TARGET_ISO = getattr(model, 'TARGET_ISO', target_iso) # Ensure model object has it
    ...
    enable_electrolyzer = getattr(model, 'ENABLE_ELECTROLYZER', ENABLE_ELECTROLYZER)
    enable_battery = getattr(model, 'ENABLE_BATTERY', ENABLE_BATTERY)
    enable_npp = getattr(model, 'ENABLE_NUCLEAR_GENERATOR', ENABLE_NUCLEAR_GENERATOR)
    enable_lte = getattr(model, 'LTE_MODE', False) # LTE_MODE is set on model during creation
    enable_h2_storage = getattr(model, 'ENABLE_H2_STORAGE', ENABLE_H2_STORAGE)
    enable_startup_shutdown = getattr(model, 'ENABLE_STARTUP_SHUTDOWN', ENABLE_STARTUP_SHUTDOWN)
    enable_degradation = getattr(model, 'ENABLE_ELECTROLYZER_DEGRADATION_TRACKING', ENABLE_ELECTROLYZER_DEGRADATION_TRACKING)
    ...
    if enable_battery:
        ...
        if isinstance(batt_cap_component, pyo.Param): # Fixed capacity
            ...
            batt_power_val = pyo.value(batt_pow_component) if batt_pow_component is not None else 0.0
            ...
            batt_capacity_val = get_param(model, 'BatteryCapacity_max_param', default=0.0) # Using _param to indicate it's the bound
    ...
    var_extract_list = [
        ('pIES', 'pIES_MW', True, 0.0), ('pTurbine', 'pTurbine_MW', enable_npp, 0.0),
        ...
        ('qSteam_Electrolyzer', 'qSteam_Electrolyzer_MWth', enable_electrolyzer and not enable_lte, 0.0),
        ...
        ('uElectrolyzer', 'Electrolyzer_Status(0=off,1=on)', enable_startup_shutdown, (1.0 if enable_electrolyzer else 0.0)),
        ('vElectrolyzerStartup', 'Electrolyzer_Startup(0=no,1=yes)', enable_startup_shutdown, 0.0),
        ('wElectrolyzerShutdown', 'Electrolyzer_Shutdown(0=no,1=yes)', enable_startup_shutdown, 0.0),
        ('DegradationState', 'DegradationState_Units', enable_degradation, 0.0),
        ('pElectrolyzerRampPos', 'pElectrolyzerRampPos_MW', enable_electrolyzer, 0.0),
        ('pElectrolyzerRampNeg', 'pElectrolyzerRampNeg_MW', enable_electrolyzer, 0.0),
        ('qSteamElectrolyzerRampPos', 'qSteamElectrolyzerRampPos_MWth', enable_electrolyzer and not enable_lte, 0.0),
        ('qSteamElectrolyzerRampNeg', 'qSteamElectrolyzerRampNeg_MWth', enable_electrolyzer and not enable_lte, 0.0),
    ...
    for var_name, col_name, is_enabled_flag, default_val in var_extract_list:
        if is_enabled_flag:
            ...
            if var_component is not None:
                hourly_data[col_name] = [get_var_value(var_component, t, default=default_val) for t in hours]
            else:
                logger.warning(f"Variable '{var_name}' expected but not found. Filling '{col_name}' with {default_val}.")
    ...
    if enable_npp and (enable_electrolyzer or enable_battery): components_providing_as.append('Turbine')
    ...
    for comp_label in all_as_components_labels:
        for service_internal in as_service_list_internal:
            ...
            base_name = f"Total_{service_internal}" if is_total else f"{service_internal}_{comp_label}"
            col_name = f"{base_name}_Bid_MW"
            if can_provide_as_local and hasattr(model, base_name):
                var_comp = getattr(model, base_name)
                hourly_data[col_name] = [get_var_value(var_comp, t, default=0.0) for t in hours]
            else:
        ...
        for comp in components_providing_as:
            for service_internal in as_service_list_internal:
                ...
                if hasattr(model, deployed_var_name):
                    hourly_data[col_name] = [get_var_value(getattr(model, deployed_var_name), t, default=0.0) for t in hours]
    ...
    hourly_data['EnergyPrice_LMP_USDperMWh'] = [get_param(model, 'energy_price', t, default=0.0) for t in hours]
    if can_provide_as_local:
        ...
        if target_iso_local in iso_service_map_for_inputs:
            for service_key in iso_service_map_for_inputs[target_iso_local]:
                ...
                param_configs = [
                    ('p', 0.0), ('loc', 0.0), ('winning_rate', 1.0)
                ]
                # Add factors based on service type (regulation vs reserve)
                is_regulation_service = "RegU" in service_key or "RegD" in service_key or "RegUp" in service_key or "RegDown" in service_key
                if is_regulation_service:
                    param_configs.extend([('mileage_factor', 1.0), ('performance_factor', 1.0)])
                ...
                for param_prefix, default_val in param_configs:
                    ...
                    # Here, param_base_name_on_model is like 'p_RegUp', 'mileage_factor_RegDown'
                    param_base_name_on_model = f"{param_prefix}_{service_key}"
                    output_col_name = f"{param_prefix}_{service_key}_{target_iso_local}" # For CSV column
                    
                    hourly_data[output_col_name] = [get_param(model, param_base_name_on_model, t, default=default_val) for t in hours]
    ...
    try:
        ...
        logger.error(f"Error creating DataFrame, likely due to inconsistent array lengths: {ve}")
        for k, v_list in hourly_data.items(): logger.debug(f"Length of '{k}': {len(v_list) if isinstance(v_list, list) else 'N/A'}")
        return pd.DataFrame(), {}
    ...
    results_df['Revenue_Total_USD'] = results_df[['Revenue_Energy_USD', 'Revenue_Hydrogen_USD', 'Revenue_Ancillary_USD']].sum(axis=1)

    logger.info("Calculating hourly costs...")
    cost_calc_list = [
        ('Cost_VOM_Turbine_USD', enable_npp, 'vom_turbine', 'pTurbine_MW'),
        ('Cost_VOM_Electrolyzer_USD', enable_electrolyzer, 'vom_electrolyzer', 'pElectrolyzer_MW'),
        ('Cost_VOM_Battery_USD', enable_battery, 'vom_battery_per_mwh_cycled', ['Battery_Charge_MW', 'Battery_Discharge_MW']),
        ('Cost_Water_USD', enable_electrolyzer, 'cost_water_per_kg_h2', 'mHydrogenProduced_kg_hr'),
        ('Cost_Ramping_USD', enable_electrolyzer, 'cost_electrolyzer_ramping', ['pElectrolyzerRampPos_MW', 'pElectrolyzerRampNeg_MW']),
        ('Cost_Storage_Cycle_USD', enable_h2_storage, 'vom_storage_cycle', ['H2_to_Storage_Input_kg_hr', 'H2_from_Storage_kg_hr']),
        ('Cost_Startup_USD', enable_startup_shutdown, 'cost_startup_electrolyzer', 'Electrolyzer_Startup(0=no,1=yes)')
    ]
    cost_cols_for_total = []
            ...
            if cost_rate_param > 1e-9:
                if isinstance(source_col_or_list, list):
                    if all(col in results_df.columns for col in source_col_or_list):
                        ...
                        if cost_col == 'Cost_VOM_Battery_USD': results_df[cost_col] = source_sum * cost_rate_param * time_factor / 2.0
                        elif cost_col in ['Cost_Ramping_USD', 'Cost_Startup_USD']: results_df[cost_col] = source_sum * cost_rate_param
                        elif cost_col == 'Cost_Storage_Cycle_USD': results_df[cost_col] = source_sum * cost_rate_param * time_factor
                        else: results_df[cost_col] = 0.0
                    ...
                    if source_col in results_df.columns:
                        if cost_col == 'Cost_Water_USD': results_df[cost_col] = results_df[source_col] * cost_rate_param * time_factor
                        else: results_df[cost_col] = results_df[source_col] * cost_rate_param * time_factor
                        cost_cols_for_total.append(cost_col)
                    else: results_df[cost_col] = 0.0
            else: results_df[cost_col] = 0.0
        else: results_df[cost_col] = 0.0
    ...
    results_df['Cost_HourlyOpex_Total_USD'] = results_df[cost_cols_for_total].sum(axis=1) if cost_cols_for_total else 0.0
    ...
    if 'Revenue_Hydrogen_Sales_USD' in results_df.columns: summary_results['Total_Hydrogen_Sales_Revenue_USD'] = results_df['Revenue_Hydrogen_Sales_USD'].sum()
    if 'Revenue_Hydrogen_Subsidy_USD' in results_df.columns: summary_results['Total_Hydrogen_Subsidy_Revenue_USD'] = results_df['Revenue_Hydrogen_Subsidy_USD'].sum()
    ...
    summary_results['Total_VOM_Cost_USD'] = sum(results_df[col].sum() for col in ['Cost_VOM_Turbine_USD', 'Cost_VOM_Electrolyzer_USD', 'Cost_VOM_Battery_USD'] if col in results_df.columns)
    summary_results['Total_Water_Cost_USD'] = results_df['Cost_Water_USD'].sum() if 'Cost_Water_USD' in results_df.columns else 0.0
    summary_results['Total_Ramping_Cost_USD'] = results_df['Cost_Ramping_USD'].sum() if 'Cost_Ramping_USD' in results_df.columns else 0.0
    summary_results['Total_Storage_Cycle_Cost_USD'] = results_df['Cost_Storage_Cycle_USD'].sum() if 'Cost_Storage_Cycle_USD' in results_df.columns else 0.0
    summary_results['Total_Startup_Cost_USD'] = results_df['Cost_Startup_USD'].sum() if 'Cost_Startup_USD' in results_df.columns else 0.0

    total_annualized_capex = 0.0; electrolyzer_annual_capex = 0.0; battery_annual_capex = 0.0
    total_hours_sim = len(hours) * time_factor
    scaling_factor = total_hours_sim / get_param(model, 'HOURS_IN_YEAR', default=8760.0) if total_hours_sim > 0 and get_param(model, 'HOURS_IN_YEAR', default=8760.0) > 0 else 0.0
    if enable_electrolyzer:
    ...
    if enable_battery:
        ...
        battery_annual_capex = (batt_capacity_val * cost_batt_cap_mwh_yr + batt_power_val * cost_batt_pow_mw_yr + batt_capacity_val * cost_batt_fom_mwh_yr) * scaling_factor
    ...
    summary_results['Total_Profit_Calculated_USD'] = summary_results['Total_Revenue_USD'] - summary_results['Total_Hourly_Opex_USD'] - summary_results['Total_Annualized_Capex_USD']
    summary_results['Objective_Value_USD'] = get_var_value(getattr(model, 'TotalProfit_Objective', None), default=None)

    if 'mHydrogenProduced_kg_hr' in results_df.columns: summary_results['Total_H2_Produced_kg'] = results_df['mHydrogenProduced_kg_hr'].sum() * time_factor
    else: summary_results['Total_H2_Produced_kg'] = 0.0
    if enable_h2_storage:
        summary_results['Total_H2_to_Market_Direct_kg'] = results_df['H2_to_Market_kg_hr'].sum() * time_factor if 'H2_to_Market_kg_hr' in results_df.columns else 0.0
        summary_results['Total_H2_from_Storage_kg'] = results_df['H2_from_Storage_kg_hr'].sum() * time_factor if 'H2_from_Storage_kg_hr' in results_df.columns else 0.0
        if 'H2_Storage_Level_kg' in results_df.columns and not results_df.empty: summary_results['Final_H2_Storage_Level_kg'] = results_df['H2_Storage_Level_kg'].iloc[-1]
    ...
    if elec_capacity_val > 1e-6 and 'pElectrolyzer_MW' in results_df.columns:
        ...
        summary_results['Electrolyzer_Capacity_Factor_Actual'] = avg_elec_power_actual / elec_capacity_val if elec_capacity_val > 0 else 0.0
    else: summary_results['Electrolyzer_Capacity_Factor_Actual'] = 0.0
    if 'Electrolyzer_Startup(0=no,1=yes)' in hourly_data: summary_results['Total_Electrolyzer_Startups'] = int(np.sum(hourly_data['Electrolyzer_Startup(0=no,1=yes)']))
    if 'DegradationState_Units' in hourly_data and hourly_data['DegradationState_Units']:
        ...
        summary_results['Final_DegradationState_Units'] = last_state if pd.notna(last_state) else 0.0

    if simulate_dispatch_mode and can_provide_as_local:
        logger.info("Calculating total deployed AS amounts...")
        for service_internal in as_service_list_internal: # Use internal names for deployed vars
            total_deployed_mwh = 0.0; temp_sum_mw = 0.0
            for comp in components_providing_as:
                comp_col_name = f"{service_internal}_{comp}_Deployed_MW"
                if comp_col_name in results_df.columns: temp_sum_mw += results_df[comp_col_name].sum()
            total_deployed_mwh = temp_sum_mw * time_factor
            summary_results[f'Total_Deployed_{service_internal}_MWh'] = total_deployed_mwh

    if not summary_results: logger.warning("Summary results dictionary is empty.")
    else:
        output_summary_path = results_dir / f'{target_iso_local}_Summary_Results.txt'
        try:
            ...
            with open(output_summary_path, 'w') as f:
                ...
                key_order = [k for k in summary_results.keys() if k not in ['Simulation_Mode', 'Target_ISO', 'Objective_Value_USD', 'Total_Profit_Calculated_USD']]
                for key in key_order:
                    ...
                    try:
                        ...
                        logger.error(f"Error writing summary key '{key}' value '{value}': {write_err}")
                        f.write(f"{key}: ERROR_WRITING_VALUE\n")
                f.write("\n--- Profitability ---\n")
                obj_val = summary_results.get('Objective_Value_USD'); calc_prof = summary_results.get('Total_Profit_Calculated_USD')
                f.write(f"Objective_Value_USD: {obj_val:,.4f}\n" if obj_val is not None else "Objective_Value_USD: N/A (Solver Failed?)\n")
                f.write(f"Total_Profit_Calculated_USD: {calc_prof:,.4f}\n" if calc_prof is not None else "Total_Profit_Calculated_USD: N/A (Calculation Error?)\n")
                if isinstance(obj_val, (int, float, np.number)) and isinstance(calc_prof, (int, float, np.number)):
                    diff = calc_prof - obj_val
                    f.write(f"Objective vs Calculated Profit Diff: {diff:,.4f}\n")
                    if abs(diff) > 1.0: f.write("WARNING: Significant difference between objective and calculated profit!\n"); logger.warning("Significant difference: objective vs calculated profit.")
    ...
    if not results_df.empty:
        ...
        output_csv_path = results_dir / f'{target_iso_local}_Hourly_Results_Comprehensive.csv'
        try:
    ...
    else: logger.warning("Hourly results DataFrame is empty. Skipping CSV save.")
    ...
    if summary_results:
        ...
        calc_prof_console = summary_results.get('Total_Profit_Calculated_USD', 'N/A')
        print(f"Objective_Value_USD: {obj_val_console:,.4f}" if isinstance(obj_val_console, (int, float, np.number)) else f"Objective_Value_USD: {obj_val_console}")
        print(f"Total_Profit_Calculated_USD: {calc_prof_console:,.4f}" if isinstance(calc_prof_console, (int, float, np.number)) else f"Total_Profit_Calculated_USD: {calc_prof_console}")
        print(f"Total_H2_Produced_kg: {summary_results.get('Total_H2_Produced_kg', 0.0):,.2f}")
        print("---------------------------------")
