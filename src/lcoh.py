...
def calculate_hydrogen_system_lcoh(
    ...
    # $/kW (DOE Baseline Avg 2022$) [Untitled.md, Ref 1]
    default_pem_capex: float = 2000.0,
    # $/kW (Estimate based on sources) [Untitled.md, Ref 27; CostAnalysis.md, Ref 16]
    default_awe_capex: float = 1800.0,
    # $/kW (Estimate based on sources) [CostAnalysis.md, Ref 4, 52]
    ...
    hydrogen_outlet_pressure_bar: float = 350.0,
    # Required H2 storage capacity in kg (set > 0 to include)
    required_storage_kg: float = 0.0,
    # CAPEX estimates for Compression/Storage (Highly variable, provide if known)
    # Optional: Estimate compressor cost relative to electrolyzer size
    compressor_capex_usd_per_kw_electrolyzer: float = None,
    # Rough default based on $1M for ~500 kg/day (~1 MW system) -> ~$1000/kW? Very rough.
    # Alt: Estimate as fraction of Elec. CAPEX (e.g., 30%)
    ...
    system_efficiency_kwh_per_kg: float = None,
    default_pem_efficiency: float = 57.0,  # kWh/kg [Untitled.md, Ref 1]
    # kWh/kg [CostAnalysis.md, Ref 4, 18]
    default_awe_efficiency: float = 57.0,
    # kWh/kg (Includes heat credit) [CostAnalysis.md, Ref 18]
    ...
    default_soec_stack_life_hrs: int = 20000,
    default_aem_stack_life_hrs: int = 30000,  # Hours [CostAnalysis.md, Ref 10]
    # % of INITIAL ELECTROLYZER CAPEX (e.g., if stack=30% of initial sys cost)
    stack_replacement_cost_percent_electrolyzer_capex: float = 0.30,
    ...
    if verbose:
        ...
        for component, value in lcoh_components.items():
            if component != 'lcoh':
                ...
                total_check += value
        # Sanity check
        print(f"  {'Calculated Component Sum':<30}: ${total_check:.3f}")
...
if __name__ == "__main__":
    ...
    lcoh_results_pem_full = calculate_hydrogen_system_lcoh(
        ...
        # Example: Stack is 30% of initial Elec cost
    ...
    lcoh_results_pem_elec_only = calculate_hydrogen_system_lcoh(
        ...
        # Note: This % now applies only to Electrolyzer CAPEX
        fixed_om_percent_total_capex_per_year=0.02,
