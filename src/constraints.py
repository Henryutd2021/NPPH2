"""Constraints module for optimization model.
...
def build_piecewise_constraints(model: pyo.ConcreteModel, *, component_prefix: str,
    ...
    logger.info(
        "Building piece-wise constraints for %s using SOS2 (constraints.py version)", component_prefix)
...
def steam_balance_rule(m, t):
    ...
    try:
        ...
        hte_steam = m.qSteam_Electrolyzer[t] if enable_electrolyzer and not lte_mode and hasattr(
            m, 'qSteam_Electrolyzer') else 0
...
def power_balance_rule(m, t):
    ...
    enable_electrolyzer = getattr(m, 'ENABLE_ELECTROLYZER', False)
    try:
        turbine_power = m.pTurbine[t] if enable_npp and hasattr(
            m, 'pTurbine') else 0
        battery_discharge = m.BatteryDischarge[t] if enable_battery and hasattr(
            m, 'BatteryDischarge') else 0
        electrolyzer_power = m.pElectrolyzer[t] if enable_electrolyzer and hasattr(
            m, 'pElectrolyzer') else 0  # Actual power
        battery_charge = m.BatteryCharge[t] if enable_battery and hasattr(
            m, 'BatteryCharge') else 0
...
def constant_turbine_power_rule(m, t):
    ...
    enable_electrolyzer = getattr(m, 'ENABLE_ELECTROLYZER', False)
    lte_mode = getattr(m, 'LTE_MODE', False)
    if not (enable_npp and enable_electrolyzer and lte_mode):
        return pyo.Constraint.Skip
    try:
        if hasattr(m, 'pTurbine') and hasattr(m, 'pTurbine_LTE_setpoint'):
            ...
            logger.warning(
                f"Skipping constant_turbine_power rule @t={t}: Missing components.")
            return pyo.Constraint.Skip
    except Exception as e:
        logger.error(f"Error in constant_turbine_power rule @t={t}: {e}")
        raise
...
def link_setpoint_to_actual_power_if_not_simulating_dispatch_rule(m, t):
    ...
    enable_electrolyzer = getattr(m, 'ENABLE_ELECTROLYZER', False)
    if not enable_electrolyzer or simulate_dispatch:
        return pyo.Constraint.Skip
    try:
...
def electrolyzer_setpoint_min_power_rule(m, t):
    ...
    enable_electrolyzer = getattr(m, 'ENABLE_ELECTROLYZER', False)
    if not enable_electrolyzer or not hasattr(m, 'pElectrolyzerSetpoint'):
        return pyo.Constraint.Skip
    try:
...
def h2_storage_balance_adj_rule(m, t):
    ...
    if not enable_h2_storage:
        return pyo.Constraint.Skip
    try:
        discharge_eff = pyo.value(
            m.storage_discharge_eff)  # Efficiency is param
        ...
        logger.error(f"Error in h2_storage_balance rule @t={t}: {e}")
        raise
...
def h2_prod_dispatch_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in h2_prod_dispatch rule @t={t}: {e}")
        raise
...
def h2_storage_charge_limit_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in h2_charge_limit rule @t={t}: {e}")
        raise
...
def h2_storage_discharge_limit_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in h2_discharge_limit rule @t={t}: {e}")
        raise
...
def h2_storage_level_max_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in h2_level_max rule @t={t}: {e}")
        raise
...
def h2_storage_level_min_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in h2_level_min rule @t={t}: {e}")
        raise
...
def Electrolyzer_RampUp_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in Electrolyzer_RampUp rule @t={t}: {e}")
        raise
...
def Electrolyzer_RampDown_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in Electrolyzer_RampDown rule @t={t}: {e}")
        raise
...
def Turbine_RampUp_rule(m, t):
    ...
    if not enable_npp:
        return pyo.Constraint.Skip
    if lte_mode and enable_electrolyzer:
        return pyo.Constraint.Skip  # Skip if turbine fixed
    if t == m.TimePeriods.first():
        return pyo.Constraint.Skip
    try:
        time_factor = pyo.value(m.delT_minutes) / 60.0
        ...
        logger.error(f"Error in Turbine_RampUp rule @t={t}: {e}")
        raise
...
def Turbine_RampDown_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in Turbine_RampDown rule @t={t}: {e}")
        raise
...
def Steam_Electrolyzer_Ramp_rule(m, t):
    enable_electrolyzer = getattr(m, 'ENABLE_ELECTROLYZER', False)
    lte_mode = getattr(m, 'LTE_MODE', False)
    if not (enable_electrolyzer and not lte_mode):
    ...
    try:
        ...
        logger.error(f"Error in Steam_Electrolyzer_Ramp rule @t={t}: {e}")
        raise
...
def h2_CapacityFactor_rule(m):
    ...
    enable_electrolyzer = getattr(m, 'ENABLE_ELECTROLYZER', False)
    if not enable_h2_cap_factor or not enable_electrolyzer:
        return pyo.Constraint.Skip
    try:
        ...
        if not hasattr(m, 'pElectrolyzer_efficiency_breakpoints') or not list(m.pElectrolyzer_efficiency_breakpoints):
            logger.warning(
                "H2 Cap Factor: Missing or empty electrolyzer efficiency breakpoints.")
            return pyo.Constraint.Skip
        ...
        if not hasattr(m, 'ke_H2_inv_values') or max_power_bp not in m.ke_H2_inv_values:
            logger.warning(
                "H2 Cap Factor: Missing inverse efficiency values (ke_H2_inv_values).")
            return pyo.Constraint.Skip
        max_h2_rate_kg_per_mwh = pyo.value(m.ke_H2_inv_values[max_power_bp])
        if max_h2_rate_kg_per_mwh < 1e-9:
            logger.warning(
                "H2 Cap Factor: Near-zero efficiency at max breakpoint.")
            return pyo.Constraint.Skip
        ...
        total_actual_production_kg = sum(
            m.mHydrogenProduced[t] * (pyo.value(m.delT_minutes) / 60.0) for t in m.TimePeriods)
        ...
        logger.error(f"Error in h2_CapacityFactor rule: {e}")
        raise
...
def electrolyzer_on_off_logic_rule(m, t):
    enable_startup_shutdown = getattr(m, 'ENABLE_STARTUP_SHUTDOWN', False)
    enable_electrolyzer = getattr(m, 'ENABLE_ELECTROLYZER', False)
    if not enable_startup_shutdown or not enable_electrolyzer:
        return pyo.Constraint.Skip
    try:
        if t == m.TimePeriods.first():
        ...
        logger.error(f"Error in SU/SD logic rule @t={t}: {e}")
        raise
...
def electrolyzer_min_power_when_on_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in SU/SD min power rule @t={t}: {e}")
        raise
...
def electrolyzer_max_power_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in SU/SD max power rule @t={t}: {e}")
        raise
...
def electrolyzer_min_power_sds_disabled_rule(m, t):
    ...
    try:
        # Only apply if min power is meaningfully positive
        if pyo.value(m.pElectrolyzer_min) > 1e-6:
            return m.pElectrolyzer[t] >= m.pElectrolyzer_min
        return pyo.Constraint.Skip
    except Exception as e:
        logger.error(f"Error in min_power_sds_disabled rule @t={t}: {e}")
        raise
...
def electrolyzer_startup_shutdown_exclusivity_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in SU/SD exclusivity rule @t={t}: {e}")
        raise
...
def electrolyzer_min_uptime_rule(m, t):
    ...
    try:
        ...
        if not all(i in m.TimePeriods for i in range(start_idx, t + 1)):
            logger.warning(
                f"Min uptime constraint skipped at t={t} due to index range issues relative to TimePeriods.")
            return pyo.Constraint.Skip
        return sum(m.uElectrolyzer[i] for i in range(start_idx, t + 1)) >= min_uptime * m.vElectrolyzerStartup[t]
    except Exception as e:
        logger.error(f"Error in min uptime rule @t={t}: {e}")
        raise
...
def electrolyzer_min_downtime_rule(m, t):
    ...
    try:
        ...
        if not all(i in m.TimePeriods for i in range(start_idx, t + 1)):
            logger.warning(
                f"Min downtime constraint skipped at t={t} due to index range issues relative to TimePeriods.")
            return pyo.Constraint.Skip
        return sum((1 - m.uElectrolyzer[i]) for i in range(start_idx, t + 1)) >= min_downtime * m.wElectrolyzerShutdown[t]
    except Exception as e:
        logger.error(f"Error in min downtime rule @t={t}: {e}")
        raise
...
def electrolyzer_degradation_rule(m, t):
    enable_degradation = getattr(
        m, 'ENABLE_ELECTROLYZER_DEGRADATION_TRACKING', False)
    enable_electrolyzer = getattr(m, 'ENABLE_ELECTROLYZER', False)
    if not enable_degradation or not enable_electrolyzer:
        return pyo.Constraint.Skip
    try:
        ...
        if t == m.TimePeriods.first():
            ...
            if (t-1) not in m.TimePeriods:
                logger.warning(
                    f"Skipping degradation constraint at t={t} because t-1 is not in TimePeriods.")
                return pyo.Constraint.Skip
        ...
        logger.error(
            f"Error defining electrolyzer_degradation rule @t={t}: {e}", exc_info=True)
        raise
...
def battery_soc_balance_rule(m, t):
    ...
    if not enable_battery:
        return pyo.Constraint.Skip
    try:
        initial_soc = m.BatterySOC_initial_fraction * \
            m.BatteryCapacity_MWh  # Symbolic expression using Var
        time_factor = pyo.value(m.delT_minutes) / 60.0
        ...
        logger.error(f"Error in battery_soc_balance rule @t={t}: {e}")
        raise
...
def battery_charge_limit_rule(m, t):
    ...
    if not enable_battery:
        return pyo.Constraint.Skip
    try:
        # Links to Var BatteryPower_MW
        return m.BatteryCharge[t] <= m.BatteryPower_MW * m.BatteryBinaryCharge[t]
    except Exception as e:
        logger.error(f"Error in battery_charge_limit rule @t={t}: {e}")
        raise
...
def battery_discharge_limit_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in battery_discharge_limit rule @t={t}: {e}")
        raise
...
def battery_binary_exclusivity_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in battery_binary_exclusivity rule @t={t}: {e}")
        raise
...
def battery_soc_max_rule(m, t):
    ...
    if not enable_battery:
        return pyo.Constraint.Skip
    try:
        # Compares with Var BatteryCapacity_MWh
        return m.BatterySOC[t] <= m.BatteryCapacity_MWh
    except Exception as e:
        logger.error(f"Error in battery_soc_max rule @t={t}: {e}")
        raise
...
def battery_soc_min_rule(m, t):
    ...
    if not enable_battery:
        return pyo.Constraint.Skip
    try:
        # Compares with expression using Var BatteryCapacity_MWh
        return m.BatterySOC[t] >= m.BatterySOC_min_fraction * m.BatteryCapacity_MWh
    except Exception as e:
        logger.error(f"Error in battery_soc_min rule @t={t}: {e}")
        raise
...
def battery_ramp_up_rule(m, t):  # Charge ramp
    enable_battery = getattr(m, 'ENABLE_BATTERY', False)
    if not enable_battery or t == m.TimePeriods.first():
        return pyo.Constraint.Skip
    try:
        time_factor = pyo.value(m.delT_minutes) / 60.0
        if time_factor <= 1e-6:
            return pyo.Constraint.Skip
        # Corrected: uses symbolic Var BatteryCapacity_MWh * time_factor
        ...
        logger.error(f"Error in battery_ramp_up rule @t={t}: {e}")
        raise
...
def battery_ramp_down_rule(m, t):  # Charge ramp
    ...
    try:
        ...
        if time_factor <= 1e-6:
            return pyo.Constraint.Skip
        ramp_limit_mw = m.BatteryRampRate * \
            m.BatteryCapacity_MWh * time_factor  # Corrected
        return m.BatteryCharge[t-1] - m.BatteryCharge[t] <= ramp_limit_mw
    except Exception as e:
        logger.error(f"Error in battery_ramp_down rule @t={t}: {e}")
        raise
...
def battery_discharge_ramp_up_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in battery_discharge_ramp_up rule @t={t}: {e}")
        raise
...
def battery_discharge_ramp_down_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in battery_discharge_ramp_down rule @t={t}: {e}")
        raise


def battery_cyclic_soc_lower_rule(m):
    enable_battery = getattr(m, 'ENABLE_BATTERY', False)
    if not enable_battery or not pyo.value(m.BatteryRequireCyclicSOC):
        return pyo.Constraint.Skip
    try:
        last_t = m.TimePeriods.last()
        initial_soc_expr = m.BatterySOC_initial_fraction * \
            m.BatteryCapacity_MWh  # Symbolic
        tolerance = 0.01  # MWh tolerance - fixed parameter
        return m.BatterySOC[last_t] >= initial_soc_expr - tolerance
    except Exception as e:
        logger.error(f"Error in battery_cyclic_soc_lower rule: {e}")
        raise
...
def battery_cyclic_soc_upper_rule(m):
    ...
    try:
        ...
        logger.error(f"Error in battery_cyclic_soc_upper rule: {e}")
        raise
...
def battery_min_cap_rule(m):
    ...
    try:
        ...
        logger.error(f"Error in battery_min_cap rule: {e}")
        raise
...
def Turbine_AS_Zero_rule(m, t):
    ...
    enable_electrolyzer = getattr(m, 'ENABLE_ELECTROLYZER', False)
    lte_mode = getattr(m, 'LTE_MODE', False)
    turbine_as_disabled = not (can_provide_as and enable_npp) or (
        lte_mode and enable_electrolyzer)
    if turbine_as_disabled:
        all_services = ['RegUp', 'RegDown', 'SR', 'NSR',
                        'ECRS', 'ThirtyMin', 'RampUp', 'RampDown', 'UncU']
        zero_sum_expr = get_symbolic_as_bid_sum(
            m, t, all_services, 'Turbine')  # Use imported helper
...
def Turbine_AS_Pmax_rule(m, t):  # Upward capability
    ...
    enable_electrolyzer = getattr(m, 'ENABLE_ELECTROLYZER', False)
    lte_mode = getattr(m, 'LTE_MODE', False)
    if not (can_provide_as and enable_npp) or (lte_mode and enable_electrolyzer):
        return pyo.Constraint.Skip
    try:
        up_services = ['RegUp', 'SR', 'NSR',
                       'RampUp', 'UncU', 'ThirtyMin', 'ECRS']
        up_reserve_bids_turbine = get_symbolic_as_bid_sum(
            m, t, up_services, 'Turbine')  # Use imported helper
        return m.pTurbine[t] + up_reserve_bids_turbine <= m.pTurbine_max
    except Exception as e:
        logger.error(f"Error in Turbine_AS_Pmax rule @t={t}: {e}")
        raise
...
def Turbine_AS_Pmin_rule(m, t):  # Downward capability
    ...
    try:
        ...
        down_reserve_bids_turbine = get_symbolic_as_bid_sum(
            m, t, down_services, 'Turbine')  # Use imported helper
        return m.pTurbine[t] - down_reserve_bids_turbine >= m.pTurbine_min
    except Exception as e:
        logger.error(f"Error in Turbine_AS_Pmin rule @t={t}: {e}")
        raise
...
def Turbine_AS_RU_rule(m, t):  # Ramp-Up capability check
    ...
    try:
        ...
        logger.error(f"Error in Turbine_AS_RU rule @t={t}: {e}")
        raise
...
def Turbine_AS_RD_rule(m, t):  # Ramp-Down capability check
    ...
    try:
        ...
        logger.error(f"Error in Turbine_AS_RD rule @t={t}: {e}")
        raise


# Capability to increase load (Down-reserve)
def Electrolyzer_AS_Pmax_rule(m, t):
    can_provide_as = getattr(m, 'CAN_PROVIDE_ANCILLARY_SERVICES', False)
    enable_electrolyzer = getattr(m, 'ENABLE_ELECTROLYZER', False)
    enable_startup_shutdown = getattr(m, 'ENABLE_STARTUP_SHUTDOWN', False)
    if not (enable_electrolyzer and can_provide_as):
        return pyo.Constraint.Skip
    try:
        down_services = ['RegDown', 'RampDown']
        down_reserve_bids_h2 = get_symbolic_as_bid_sum(
            m, t, down_services, 'Electrolyzer')  # Use imported helper
        ...
        logger.error(f"Error in Electrolyzer_AS_Pmax rule @t={t}: {e}")
        raise


# Capability to decrease load (Up-reserve)
def Electrolyzer_AS_Pmin_rule(m, t):
    can_provide_as = getattr(m, 'CAN_PROVIDE_ANCILLARY_SERVICES', False)
    enable_electrolyzer = getattr(m, 'ENABLE_ELECTROLYZER', False)
    enable_startup_shutdown = getattr(m, 'ENABLE_STARTUP_SHUTDOWN', False)
    if not (enable_electrolyzer and can_provide_as):
    ...
    try:
        up_services = ['RegUp', 'SR', 'NSR',
                       'RampUp', 'UncU', 'ThirtyMin', 'ECRS']
        up_reserve_bids_h2 = get_symbolic_as_bid_sum(
            m, t, up_services, 'Electrolyzer')  # Use imported helper
        ...
        logger.error(f"Error in Electrolyzer_AS_Pmin rule @t={t}: {e}")
        raise


# Ramp capability for Down-reserve (Increasing Load based on Setpoint)
def Electrolyzer_AS_RU_rule(m, t):
    can_provide_as = getattr(m, 'CAN_PROVIDE_ANCILLARY_SERVICES', False)
    enable_electrolyzer = getattr(m, 'ENABLE_ELECTROLYZER', False)
    if not (enable_electrolyzer and can_provide_as):
        return pyo.Constraint.Skip
    ...
    try:
        ...
        down_reserve_bids_h2 = get_symbolic_as_bid_sum(
            m, t, down_services, 'Electrolyzer')  # Use imported helper
        time_factor = pyo.value(m.delT_minutes) / 60.0
        ramp_limit_mw = m.RU_Electrolyzer_percent_hourly * \
            m.pElectrolyzer_max * time_factor
        return (m.pElectrolyzerSetpoint[t] + down_reserve_bids_h2) - m.pElectrolyzer[t-1] <= ramp_limit_mw
    except Exception as e:
        logger.error(f"Error in Electrolyzer_AS_RU rule @t={t}: {e}")
        raise


# Ramp capability for Up-reserve (Decreasing Load based on Setpoint)
def Electrolyzer_AS_RD_rule(m, t):
    can_provide_as = getattr(m, 'CAN_PROVIDE_ANCILLARY_SERVICES', False)
    enable_electrolyzer = getattr(m, 'ENABLE_ELECTROLYZER', False)
    if not (enable_electrolyzer and can_provide_as):
        return pyo.Constraint.Skip
    ...
    try:
        ...
        up_reserve_bids_h2 = get_symbolic_as_bid_sum(
            m, t, up_services, 'Electrolyzer')  # Use imported helper
        time_factor = pyo.value(m.delT_minutes) / 60.0
        ramp_limit_mw = m.RD_Electrolyzer_percent_hourly * \
            m.pElectrolyzer_max * time_factor
        return m.pElectrolyzer[t-1] - (m.pElectrolyzerSetpoint[t] - up_reserve_bids_h2) <= ramp_limit_mw
    except Exception as e:
        logger.error(f"Error in Electrolyzer_AS_RD rule @t={t}: {e}")
        raise
...
def Battery_AS_Pmax_rule(m, t):
    can_provide_as = getattr(m, 'CAN_PROVIDE_ANCILLARY_SERVICES', False)
    enable_battery = getattr(m, 'ENABLE_BATTERY', False)
    if not (enable_battery and can_provide_as):
        return pyo.Constraint.Skip
    try:
        down_services = ['RegDown', 'RampDown']
        down_reserve_bids_battery = get_symbolic_as_bid_sum(
            m, t, down_services, 'Battery')  # Use imported helper
        available_charge_headroom = m.BatteryPower_MW - m.BatteryCharge[t]
        return down_reserve_bids_battery <= available_charge_headroom
    except Exception as e:
        logger.error(f"Error in Battery_AS_Pmax rule @t={t}: {e}")
        raise
...
def Battery_AS_Pmin_rule(m, t):
    ...
    try:
        up_services = ['RegUp', 'SR', 'NSR',
                       'RampUp', 'UncU', 'ThirtyMin', 'ECRS']
        up_reserve_bids_battery = get_symbolic_as_bid_sum(
            m, t, up_services, 'Battery')  # Use imported helper
        available_discharge_headroom = m.BatteryPower_MW - \
            m.BatteryDischarge[t]
        return up_reserve_bids_battery <= available_discharge_headroom
    except Exception as e:
        logger.error(f"Error in Battery_AS_Pmin rule @t={t}: {e}")
        raise
...
def Battery_AS_SOC_Up_rule(m, t):
    ...
    try:
        ...
        energy_needed_expr = up_reserve_bids_battery * \
        ...
        logger.error(f"Error in Battery_AS_SOC_Up rule @t={t}: {e}")
        raise
...
def Battery_AS_SOC_Down_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in Battery_AS_SOC_Down rule @t={t}: {e}")
        raise
...
def Battery_AS_RU_rule(m, t):  # Ramp capability for Down-reg (Increasing Charge)
    can_provide_as = getattr(m, 'CAN_PROVIDE_ANCILLARY_SERVICES', False)
    enable_battery = getattr(m, 'ENABLE_BATTERY', False)
    if not (enable_battery and can_provide_as) or t == m.TimePeriods.first():
        return pyo.Constraint.Skip
    try:
        down_services = ['RegDown', 'RampDown']
        down_reserve_bids_battery = get_symbolic_as_bid_sum(
        ...
        logger.error(f"Error in Battery_AS_RU rule @t={t}: {e}")
        raise
...
def Battery_AS_RD_rule(m, t):  # Ramp capability for Up-reg (Increasing Discharge)
    ...
    try:
        ...
        logger.error(f"Error in Battery_AS_RD rule @t={t}: {e}")
        raise
...
def link_total_as_rule(m, t, service_name):
    ...
    try:
        ...
        if enable_npp and (enable_electrolyzer or enable_battery):
            turbine_bid_expr = get_symbolic_as_bid_sum(
                m, t, [service_name], 'Turbine')  # Use imported helper

        electro_bid_expr = 0.0
        if enable_electrolyzer:
            electro_bid_expr = get_symbolic_as_bid_sum(
                m, t, [service_name], 'Electrolyzer')  # Use imported helper

        battery_bid_expr = 0.0
        if enable_battery:
            battery_bid_expr = get_symbolic_as_bid_sum(
                m, t, [service_name], 'Battery')  # Use imported helper

        return total_var[t] == turbine_bid_expr + electro_bid_expr + battery_bid_expr
    except AttributeError as e:
        logger.debug(
            f"Attribute error linking service {service_name} at time {t}: {e}")
        return pyo.Constraint.Skip
    except Exception as e:
        logger.error(
...
def link_deployed_to_bid_rule(m, t, service_name, component_name):
    ...
    try:
        ...
        if not (hasattr(m, win_rate_param_name_on_model) and hasattr(m, deploy_factor_param_name_on_model)):
            return pyo.Constraint.Skip
        ...
        logger.error(
            f"Error in link_deployed_to_bid_rule for {service_name} {component_name} @t={t}: {e}")
        raise
