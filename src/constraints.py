...
def steam_balance_rule(m, t):
    ...
    try:
        ...
        hte_steam = 0
        if getattr(m, 'ENABLE_ELECTROLYZER', False) and not getattr(m, 'LTE_MODE', False) and hasattr(m, 'qSteam_Electrolyzer'):
...
def power_balance_rule(m, t):
    ...
    try:
        turbine_power = m.pTurbine[t] if getattr(m, 'ENABLE_NUCLEAR_GENERATOR', False) and hasattr(m, 'pTurbine') else 0
        battery_discharge = m.BatteryDischarge[t] if getattr(m, 'ENABLE_BATTERY', False) and hasattr(m, 'BatteryDischarge') else 0
        ...
        battery_charge = m.BatteryCharge[t] if getattr(m, 'ENABLE_BATTERY', False) and hasattr(m, 'BatteryCharge') else 0
        auxiliary_power = m.pAuxiliary[t] if hasattr(m, 'pAuxiliary') else 0  # Exists if aux_power_consumption_per_kg_h2 > 0
...
def constant_turbine_power_rule(m, t):
    ...
    if not (getattr(m, 'ENABLE_NUCLEAR_GENERATOR', False) and \
            getattr(m, 'ENABLE_ELECTROLYZER', False) and \
            getattr(m, 'LTE_MODE', False)):
        return pyo.Constraint.Skip
    try:
...
def link_auxiliary_power_rule(m, t):
    ...
    if not hasattr(m, 'pAuxiliary') or not getattr(m, 'ENABLE_ELECTROLYZER', False):  # pAuxiliary only exists if param > 0
        return pyo.Constraint.Skip
    try:
...
def link_setpoint_to_actual_power_if_not_simulating_dispatch_rule(m, t):
    ...
    if not getattr(m, 'ENABLE_ELECTROLYZER', False) or getattr(m, 'SIMULATE_AS_DISPATCH_EXECUTION', False):
        return pyo.Constraint.Skip
    try:
        ...
        logger.error(f"Error in link_setpoint_to_actual_power rule @t={t}: {e}", exc_info=True)
        raise


def electrolyzer_setpoint_min_power_rule(m, t):
    """Ensures the electrolyzer setpoint respects minimum turn-down if operational."""
    if not getattr(m, 'ENABLE_ELECTROLYZER', False) or not hasattr(m, 'pElectrolyzerSetpoint'):
        return pyo.Constraint.Skip
    try:
        min_power_param = getattr(m, 'pElectrolyzer_min', None)
        if min_power_param is None:
            logger.warning(f"Skipping setpoint_min_power @t={t}: pElectrolyzer_min not found.")
            return pyo.Constraint.Skip
        ...
        if getattr(m, 'ENABLE_STARTUP_SHUTDOWN', False) and hasattr(m, 'uElectrolyzer'):
            ...
            return m.pElectrolyzerSetpoint[t] >= min_power_param if pyo.value(min_power_param) > 1e-6 else pyo.Constraint.Skip
    except Exception as e:
        logger.error(f"Error in electrolyzer_setpoint_min_power rule @t={t}: {e}", exc_info=True)
        raise

# --- H2 STORAGE RULES ---
def h2_storage_balance_adj_rule(m, t):
    if not getattr(m, 'ENABLE_H2_STORAGE', False): return pyo.Constraint.Skip
    try:
        discharge_eff = pyo.value(m.storage_discharge_eff); charge_eff = pyo.value(m.storage_charge_eff)
        discharge_term = (m.H2_from_storage[t] / discharge_eff if discharge_eff > 1e-9 else 0)
        charge_term = m.H2_to_storage[t] * charge_eff
        if t == m.TimePeriods.first(): return m.H2_storage_level[t] == m.H2_storage_level_initial + charge_term - discharge_term
        else: return m.H2_storage_level[t] == m.H2_storage_level[t - 1] + charge_term - discharge_term
    except Exception as e: logger.error(f"H2 Storage Balance Error @t={t}: {e}", exc_info=True); raise

def h2_prod_dispatch_rule(m, t):
    if not getattr(m, 'ENABLE_H2_STORAGE', False): return pyo.Constraint.Skip
    try: return m.mHydrogenProduced[t] == m.H2_to_market[t] + m.H2_to_storage[t]
    except Exception as e: logger.error(f"H2 Prod Dispatch Error @t={t}: {e}", exc_info=True); raise

def h2_storage_charge_limit_rule(m, t):
    if not getattr(m, 'ENABLE_H2_STORAGE', False): return pyo.Constraint.Skip
    try: return m.H2_to_storage[t] <= m.H2_storage_charge_rate_max
    except Exception as e: logger.error(f"H2 Charge Limit Error @t={t}: {e}", exc_info=True); raise

def h2_storage_discharge_limit_rule(m, t):
    if not getattr(m, 'ENABLE_H2_STORAGE', False): return pyo.Constraint.Skip
    try: return m.H2_from_storage[t] <= m.H2_storage_discharge_rate_max
    except Exception as e: logger.error(f"H2 Discharge Limit Error @t={t}: {e}", exc_info=True); raise

def h2_storage_level_max_rule(m, t):
    if not getattr(m, 'ENABLE_H2_STORAGE', False): return pyo.Constraint.Skip
    try: return m.H2_storage_level[t] <= m.H2_storage_capacity_max
    except Exception as e: logger.error(f"H2 Level Max Error @t={t}: {e}", exc_info=True); raise

def h2_storage_level_min_rule(m, t):
    if not getattr(m, 'ENABLE_H2_STORAGE', False): return pyo.Constraint.Skip
    try: return m.H2_storage_level[t] >= m.H2_storage_capacity_min
    except Exception as e: logger.error(f"H2 Level Min Error @t={t}: {e}", exc_info=True); raise

# --- RAMP RATE RULES ---
def Electrolyzer_RampUp_rule(m, t):
    if not getattr(m, 'ENABLE_ELECTROLYZER', False) or t == m.TimePeriods.first(): return pyo.Constraint.Skip
    try:
    ...
    except Exception as e: logger.error(f"Elec RampUp Error @t={t}: {e}", exc_info=True); raise
...
def Electrolyzer_RampDown_rule(m, t):
    ...
    except Exception as e: logger.error(f"Elec RampDown Error @t={t}: {e}", exc_info=True); raise

def Turbine_RampUp_rule(m, t):
    if not getattr(m, 'ENABLE_NUCLEAR_GENERATOR', False) or \
       (getattr(m, 'LTE_MODE', False) and getattr(m, 'ENABLE_ELECTROLYZER', False)) or \
       t == m.TimePeriods.first(): return pyo.Constraint.Skip
    try:
        time_factor = pyo.value(m.delT_minutes) / 60.0
        return m.pTurbine[t] - m.pTurbine[t - 1] <= m.RU_Turbine_hourly * time_factor
    except Exception as e: logger.error(f"Turbine RampUp Error @t={t}: {e}", exc_info=True); raise
...
def Turbine_RampDown_rule(m, t):
    ...
    except Exception as e: logger.error(f"Turbine RampDown Error @t={t}: {e}", exc_info=True); raise

def Steam_Electrolyzer_Ramp_rule(m, t):
    if not (getattr(m, 'ENABLE_ELECTROLYZER', False) and not getattr(m, 'LTE_MODE', False)) or \
       t == m.TimePeriods.first(): return pyo.Constraint.Skip  # HTE only
    try:
        ...
        return pyo.Constraint.Skip
    except Exception as e: logger.error(f"Steam Elec Ramp Error @t={t}: {e}", exc_info=True); raise

# --- H2 CAPACITY FACTOR RULE ---
def h2_CapacityFactor_rule(m):
    if not getattr(m, 'ENABLE_H2_CAP_FACTOR', False) or not getattr(m, 'ENABLE_ELECTROLYZER', False):
        return pyo.Constraint.Skip
    try:
        total_hours_sim = len(m.TimePeriods) * (pyo.value(m.delT_minutes) / 60.0)
        if not hasattr(m, 'pElectrolyzer_max') or pyo.value(m.pElectrolyzer_max) <= 1e-6:
            if isinstance(m.pElectrolyzer_max, pyo.Param) and pyo.value(m.pElectrolyzer_max) <= 1e-6:
                return pyo.Constraint.Skip
            if hasattr(m, 'pElectrolyzer_max_upper_bound') and pyo.value(m.pElectrolyzer_max_upper_bound) <= 1e-6:
                return pyo.Constraint.Skip
            max_elec_power_limit = pyo.value(m.pElectrolyzer_max_upper_bound) if hasattr(m, 'pElectrolyzer_max_upper_bound') else pyo.value(m.pElectrolyzer_max)
        else:
        ...
        if max_h2_rate_kg_per_mwh < 1e-9: return pyo.Constraint.Skip
    ...
    except Exception as e: logger.error(f"H2 Cap Factor Error: {e}", exc_info=True); raise

# --- STARTUP/SHUTDOWN RULES ---
def electrolyzer_on_off_logic_rule(m, t):
    if not getattr(m, 'ENABLE_STARTUP_SHUTDOWN', False) or not getattr(m, 'ENABLE_ELECTROLYZER', False): return pyo.Constraint.Skip
    try:
        if t == m.TimePeriods.first(): return m.uElectrolyzer[t] - m.uElectrolyzer_initial == m.vElectrolyzerStartup[t] - m.wElectrolyzerShutdown[t]
        else: return m.uElectrolyzer[t] - m.uElectrolyzer[t - 1] == m.vElectrolyzerStartup[t] - m.wElectrolyzerShutdown[t]
    except Exception as e: logger.error(f"SU/SD Logic Error @t={t}: {e}", exc_info=True); raise
...
def electrolyzer_min_power_when_on_rule(m, t):  # Applies to actual physical power
    ...
    except Exception as e: logger.error(f"SU/SD Min Power Error @t={t}: {e}", exc_info=True); raise
...
def electrolyzer_max_power_rule(m, t):  # Applies to actual physical power
    ...
    except Exception as e: logger.error(f"SU/SD Max Power Error @t={t}: {e}", exc_info=True); raise

def electrolyzer_min_power_sds_disabled_rule(m, t):  # Applies to actual physical power
    if getattr(m, 'ENABLE_STARTUP_SHUTDOWN', False) or not getattr(m, 'ENABLE_ELECTROLYZER', False): return pyo.Constraint.Skip
    try:
        return m.pElectrolyzer[t] >= m.pElectrolyzer_min if pyo.value(m.pElectrolyzer_min) > 1e-6 else pyo.Constraint.Skip
    except Exception as e: logger.error(f"Min Power (SDS Disabled) Error @t={t}: {e}", exc_info=True); raise

def electrolyzer_startup_shutdown_exclusivity_rule(m, t):
    if not getattr(m, 'ENABLE_STARTUP_SHUTDOWN', False) or not getattr(m, 'ENABLE_ELECTROLYZER', False): return pyo.Constraint.Skip
    try: return m.vElectrolyzerStartup[t] + m.wElectrolyzerShutdown[t] <= 1
    except Exception as e: logger.error(f"SU/SD Exclusivity Error @t={t}: {e}", exc_info=True); raise
...
def electrolyzer_min_uptime_rule(m, t):
    ...
    try:
        ...
        if not all(i in m.TimePeriods for i in range(start_idx, t + 1)): return pyo.Constraint.Skip
        return sum(m.uElectrolyzer[i] for i in range(start_idx, t + 1)) >= min_uptime * m.vElectrolyzerStartup[t]
    except Exception as e: logger.error(f"Min Uptime Error @t={t}: {e}", exc_info=True); raise
...
def electrolyzer_min_downtime_rule(m, t):
    ...
    except Exception as e: logger.error(f"Min Downtime Error @t={t}: {e}", exc_info=True); raise

# --- ELECTROLYZER DEGRADATION RULE ---
def electrolyzer_degradation_rule(m, t):  # Based on actual power m.pElectrolyzer
    if not getattr(m, 'ENABLE_ELECTROLYZER_DEGRADATION_TRACKING', False) or not getattr(m, 'ENABLE_ELECTROLYZER', False): return pyo.Constraint.Skip
    try:
        relative_load_expr = 0; max_cap_var = m.pElectrolyzer_max; epsilon = 1e-6
        # Ensure pElectrolyzer_max is treated as a value if it's a Param, or use the variable directly
        current_max_cap = pyo.value(max_cap_var) if isinstance(max_cap_var, pyo.Param) else max_cap_var

        relative_load_expr = m.pElectrolyzer[t] / (current_max_cap + epsilon)
        time_factor = pyo.value(m.delT_minutes) / 60.0
        op_factor = pyo.value(m.DegradationFactorOperation); startup_factor = pyo.value(m.DegradationFactorStartup)
        degradation_increase_op = op_factor * relative_load_expr * time_factor; degradation_increase_su = 0.0
        if getattr(m, 'ENABLE_STARTUP_SHUTDOWN', False) and hasattr(m, 'vElectrolyzerStartup'):
        ...
        if t == m.TimePeriods.first(): return m.DegradationState[t] == m.DegradationStateInitial + total_degradation_increase
        else:
            if (t - 1) not in m.TimePeriods or not hasattr(m, 'DegradationState'): return pyo.Constraint.Skip
            return m.DegradationState[t] == m.DegradationState[t - 1] + total_degradation_increase
    except Exception as e: logger.error(f"Elec Degradation Error @t={t}: {e}", exc_info=True); raise

# --- BATTERY STORAGE RULES ---
def battery_soc_balance_rule(m, t):
    if not getattr(m, 'ENABLE_BATTERY', False): return pyo.Constraint.Skip
    try:
        initial_soc = m.BatterySOC_initial_fraction * m.BatteryCapacity_MWh; time_factor = pyo.value(m.delT_minutes) / 60.0
        charge_eff = pyo.value(m.BatteryChargeEff); discharge_eff = pyo.value(m.BatteryDischargeEff)
        discharge_term = (m.BatteryDischarge[t] / discharge_eff if discharge_eff > 1e-9 else 0)
        charge_term = m.BatteryCharge[t] * charge_eff
        if t == m.TimePeriods.first(): return m.BatterySOC[t] == initial_soc + (charge_term - discharge_term) * time_factor
        else: return m.BatterySOC[t] == m.BatterySOC[t - 1] + (charge_term - discharge_term) * time_factor
    except Exception as e: logger.error(f"Battery SOC Balance Error @t={t}: {e}", exc_info=True); raise

def battery_charge_limit_rule(m, t):
    if not getattr(m, 'ENABLE_BATTERY', False): return pyo.Constraint.Skip
    try: return m.BatteryCharge[t] <= m.BatteryPower_MW * m.BatteryBinaryCharge[t]
    except Exception as e: logger.error(f"Battery Charge Limit Error @t={t}: {e}", exc_info=True); raise

def battery_discharge_limit_rule(m, t):
    if not getattr(m, 'ENABLE_BATTERY', False): return pyo.Constraint.Skip
    try: return m.BatteryDischarge[t] <= m.BatteryPower_MW * m.BatteryBinaryDischarge[t]
    except Exception as e: logger.error(f"Battery Discharge Limit Error @t={t}: {e}", exc_info=True); raise

def battery_binary_exclusivity_rule(m, t):
    if not getattr(m, 'ENABLE_BATTERY', False): return pyo.Constraint.Skip
    try: return m.BatteryBinaryCharge[t] + m.BatteryBinaryDischarge[t] <= 1
    except Exception as e: logger.error(f"Battery Binary Exclusivity Error @t={t}: {e}", exc_info=True); raise

def battery_soc_max_rule(m, t):
    if not getattr(m, 'ENABLE_BATTERY', False): return pyo.Constraint.Skip
    try: return m.BatterySOC[t] <= m.BatteryCapacity_MWh
    except Exception as e: logger.error(f"Battery SOC Max Error @t={t}: {e}", exc_info=True); raise

def battery_soc_min_rule(m, t):
    if not getattr(m, 'ENABLE_BATTERY', False): return pyo.Constraint.Skip
    try: return m.BatterySOC[t] >= m.BatterySOC_min_fraction * m.BatteryCapacity_MWh
    except Exception as e: logger.error(f"Battery SOC Min Error @t={t}: {e}", exc_info=True); raise

def battery_ramp_up_rule(m, t):  # Charge ramp
    if not getattr(m, 'ENABLE_BATTERY', False) or t == m.TimePeriods.first(): return pyo.Constraint.Skip
    try:
        time_factor = pyo.value(m.delT_minutes) / 60.0;
    ...
    except Exception as e: logger.error(f"Battery RampUp Error @t={t}: {e}", exc_info=True); raise
...
def battery_ramp_down_rule(m, t):  # Charge ramp
    ...
    except Exception as e: logger.error(f"Battery RampDown Error @t={t}: {e}", exc_info=True); raise
...
def battery_discharge_ramp_up_rule(m, t):
    ...
    except Exception as e: logger.error(f"Battery Discharge RampUp Error @t={t}: {e}", exc_info=True); raise
...
def battery_discharge_ramp_down_rule(m, t):
    ...
    except Exception as e: logger.error(f"Battery Discharge RampDown Error @t={t}: {e}", exc_info=True); raise

def battery_cyclic_soc_lower_rule(m):
    if not getattr(m, 'ENABLE_BATTERY', False) or not pyo.value(m.BatteryRequireCyclicSOC): return pyo.Constraint.Skip
    try:
        last_t = m.TimePeriods.last(); initial_soc_expr = m.BatterySOC_initial_fraction * m.BatteryCapacity_MWh
        tolerance = 0.01  # MWh
        return m.BatterySOC[last_t] >= initial_soc_expr - tolerance
    except Exception as e: logger.error(f"Battery Cyclic SOC Lower Error: {e}", exc_info=True); raise
...
def battery_cyclic_soc_upper_rule(m):
    ...
    try:
        ...
        tolerance = 0.01  # MWh
        return m.BatterySOC[last_t] <= initial_soc_expr + tolerance
    except Exception as e: logger.error(f"Battery Cyclic SOC Upper Error: {e}", exc_info=True); raise

def battery_power_capacity_link_rule(m):
    if not getattr(m, 'ENABLE_BATTERY', False) or isinstance(getattr(m, 'BatteryCapacity_MWh', None), pyo.Param):
        return pyo.Constraint.Skip  # Only if optimizing capacity
    try: return m.BatteryPower_MW == m.BatteryCapacity_MWh * m.BatteryPowerRatio
    except Exception as e: logger.error(f"Battery Power-Capacity Link Error: {e}", exc_info=True); raise

def battery_min_cap_rule(m):
    if not getattr(m, 'ENABLE_BATTERY', False): return pyo.Constraint.Skip
    try:
        # In model.py, BatteryCapacity_min_param is used for the bound when optimizing
        if hasattr(m, 'BatteryCapacity_min_param') and pyo.value(m.BatteryCapacity_min_param) > 1e-6:
            return m.BatteryCapacity_MWh >= m.BatteryCapacity_min_param
        return pyo.Constraint.Skip
    except Exception as e: logger.error(f"Battery Min Cap Error: {e}", exc_info=True); raise

# --- ANCILLARY SERVICE CAPABILITY RULES ---
def Turbine_AS_Zero_rule(m, t):
    turbine_as_disabled = not (getattr(m, 'CAN_PROVIDE_ANCILLARY_SERVICES', False) and getattr(m, 'ENABLE_NUCLEAR_GENERATOR', False)) or \
                          (getattr(m, 'LTE_MODE', False) and getattr(m, 'ENABLE_ELECTROLYZER', False))
...
def Turbine_AS_Pmax_rule(m, t):  # Upward capability
    if not (getattr(m, 'CAN_PROVIDE_ANCILLARY_SERVICES', False) and getattr(m, 'ENABLE_NUCLEAR_GENERATOR', False)) or \
       (getattr(m, 'LTE_MODE', False) and getattr(m, 'ENABLE_ELECTROLYZER', False)): return pyo.Constraint.Skip
    try:
    ...
    except Exception as e: logger.error(f"Turbine AS Pmax Error @t={t}: {e}", exc_info=True); raise
...
def Turbine_AS_Pmin_rule(m, t):  # Downward capability
    ...
    except Exception as e: logger.error(f"Turbine AS Pmin Error @t={t}: {e}", exc_info=True); raise
...
def Turbine_AS_RU_rule(m, t):
    ...
    try:
        ...
        up_bids = get_symbolic_as_bid_sum(m, t, up_services, 'Turbine')
        time_factor = pyo.value(m.delT_minutes) / 60.0
        return (m.pTurbine[t] + up_bids) - m.pTurbine[t - 1] <= m.RU_Turbine_hourly * time_factor
    except Exception as e: logger.error(f"Turbine AS RU Error @t={t}: {e}", exc_info=True); raise
...
def Turbine_AS_RD_rule(m, t):
    ...
    try:
        ...
        down_bids = get_symbolic_as_bid_sum(m, t, down_services, 'Turbine')
        time_factor = pyo.value(m.delT_minutes) / 60.0
        return m.pTurbine[t - 1] - (m.pTurbine[t] - down_bids) <= m.RD_Turbine_hourly * time_factor
    except Exception as e: logger.error(f"Turbine AS RD Error @t={t}: {e}", exc_info=True); raise

def Electrolyzer_AS_Pmax_rule(m, t):  # Capability to increase load (Down-reserve from setpoint)
    if not (getattr(m, 'ENABLE_ELECTROLYZER', False) and getattr(m, 'CAN_PROVIDE_ANCILLARY_SERVICES', False)): return pyo.Constraint.Skip
    try:
        down_services = ['RegDown', 'RampDown']
    ...
    except Exception as e: logger.error(f"Elec AS Pmax Error @t={t}: {e}", exc_info=True); raise

def Electrolyzer_AS_Pmin_rule(m, t):  # Capability to decrease load (Up-reserve from setpoint)
    if not (getattr(m, 'ENABLE_ELECTROLYZER', False) and getattr(m, 'CAN_PROVIDE_ANCILLARY_SERVICES', False)): return pyo.Constraint.Skip
    try:
        up_services = ['RegUp', 'SR', 'NSR', 'RampUp', 'UncU', 'ThirtyMin', 'ECRS']
    ...
    except Exception as e: logger.error(f"Elec AS Pmin Error @t={t}: {e}", exc_info=True); raise

def Electrolyzer_AS_RU_rule(m, t):  # Ramp for Down-reserve (Increasing Load from actual previous power)
    if not (getattr(m, 'ENABLE_ELECTROLYZER', False) and getattr(m, 'CAN_PROVIDE_ANCILLARY_SERVICES', False)) or \
       t == m.TimePeriods.first(): return pyo.Constraint.Skip
    try:
        down_services = ['RegDown', 'RampDown']
        down_bids = get_symbolic_as_bid_sum(m, t, down_services, 'Electrolyzer')
        time_factor = pyo.value(m.delT_minutes) / 60.0
        ramp_limit = m.RU_Electrolyzer_percent_hourly * m.pElectrolyzer_max * time_factor
        # Compare (Setpoint + Down Bids) with previous actual power m.pElectrolyzer[t-1]
        return (m.pElectrolyzerSetpoint[t] + down_bids) - m.pElectrolyzer[t - 1] <= ramp_limit
    except Exception as e: logger.error(f"Elec AS RU Error @t={t}: {e}", exc_info=True); raise

def Electrolyzer_AS_RD_rule(m, t):  # Ramp for Up-reserve (Decreasing Load from actual previous power)
    if not (getattr(m, 'ENABLE_ELECTROLYZER', False) and getattr(m, 'CAN_PROVIDE_ANCILLARY_SERVICES', False)) or \
       t == m.TimePeriods.first(): return pyo.Constraint.Skip
    try:
        up_services = ['RegUp', 'SR', 'NSR', 'RampUp', 'UncU', 'ThirtyMin', 'ECRS']
        up_bids = get_symbolic_as_bid_sum(m, t, up_services, 'Electrolyzer')
        time_factor = pyo.value(m.delT_minutes) / 60.0
        ramp_limit = m.RD_Electrolyzer_percent_hourly * m.pElectrolyzer_max * time_factor
        # Compare (Setpoint - Up Bids) with previous actual power m.pElectrolyzer[t-1]
        return m.pElectrolyzer[t - 1] - (m.pElectrolyzerSetpoint[t] - up_bids) <= ramp_limit
    except Exception as e: logger.error(f"Elec AS RD Error @t={t}: {e}", exc_info=True); raise

def Battery_AS_Pmax_rule(m, t):  # Down-reserve (charge capability)
    if not (getattr(m, 'ENABLE_BATTERY', False) and getattr(m, 'CAN_PROVIDE_ANCILLARY_SERVICES', False)): return pyo.Constraint.Skip
    try:
        down_services = ['RegDown', 'RampDown']
        ...
        return down_bids <= m.BatteryPower_MW * m.BatteryBinaryCharge[t] - m.BatteryCharge[t]
    except Exception as e: logger.error(f"Battery AS Pmax Error @t={t}: {e}", exc_info=True); raise
...
def Battery_AS_Pmin_rule(m, t):  # Up-reserve (discharge capability)
    ...
    try:
        ...
        return up_bids <= m.BatteryPower_MW * m.BatteryBinaryDischarge[t] - m.BatteryDischarge[t]
    except Exception as e: logger.error(f"Battery AS Pmin Error @t={t}: {e}", exc_info=True); raise
...
def Battery_AS_SOC_Up_rule(m, t):  # Energy for Up-reserve
    ...
    try:
        up_services = ['RegUp', 'SR', 'NSR', 'RampUp', 'UncU', 'ThirtyMin', 'ECRS']
        up_bids = get_symbolic_as_bid_sum(m, t, up_services, 'Battery')
        discharge_eff = pyo.value(m.BatteryDischargeEff); as_duration = pyo.value(m.AS_Duration)
        energy_needed = up_bids * (as_duration / discharge_eff if discharge_eff > 1e-9 else float('inf'))
        min_soc = m.BatterySOC_min_fraction * m.BatteryCapacity_MWh
        return m.BatterySOC[t] - energy_needed >= min_soc
    except Exception as e: logger.error(f"Battery AS SOC Up Error @t={t}: {e}", exc_info=True); raise
...
def Battery_AS_SOC_Down_rule(m, t):  # Energy for Down-reserve
    ...
    try:
        down_services = ['RegDown', 'RampDown']
        down_bids = get_symbolic_as_bid_sum(m, t, down_services, 'Battery')
        charge_eff = pyo.value(m.BatteryChargeEff); as_duration = pyo.value(m.AS_Duration)
        energy_absorbed = down_bids * as_duration * charge_eff
        return m.BatterySOC[t] + energy_absorbed <= m.BatteryCapacity_MWh
    except Exception as e: logger.error(f"Battery AS SOC Down Error @t={t}: {e}", exc_info=True); raise

def Battery_AS_RU_rule(m, t):  # Ramp for Down-reg (Increasing Charge)
    if not (getattr(m, 'ENABLE_BATTERY', False) and getattr(m, 'CAN_PROVIDE_ANCILLARY_SERVICES', False)) or \
       t == m.TimePeriods.first(): return pyo.Constraint.Skip
    try:
        down_services = ['RegDown', 'RampDown']
        down_bids = get_symbolic_as_bid_sum(m, t, down_services, 'Battery')
    ...
    except Exception as e: logger.error(f"Battery AS RU Error @t={t}: {e}", exc_info=True); raise
...
def Battery_AS_RD_rule(m, t):  # Ramp for Up-reg (Increasing Discharge)
    ...
    try:
        ...
        if time_factor <= 1e-6: return pyo.Constraint.Skip
        ramp_limit = m.BatteryRampRate * m.BatteryCapacity_MWh * time_factor
        return (m.BatteryDischarge[t] + up_bids) - m.BatteryDischarge[t - 1] <= ramp_limit
    except Exception as e: logger.error(f"Battery AS RD Error @t={t}: {e}", exc_info=True); raise
...
def link_total_as_rule(m, t, service_name):
    ...
    if not getattr(m, 'CAN_PROVIDE_ANCILLARY_SERVICES', False): return pyo.Constraint.Skip
    try:
        ...
        logger.debug(f"Attribute error linking total for service {service_name} at time {t}: {e}.")
        return pyo.Constraint.Skip 
    except Exception as e:
        logger.error(f"Error in link_total_as_rule for {service_name} @t={t}: {e}", exc_info=True)
        raise
...
def link_deployed_to_bid_rule(m, t, internal_service_name, component_name):
    ...
    try:
        ...
        if not (hasattr(m, deployed_var_name) and hasattr(m, bid_var_name)):
            logger.debug(f"Skipping link_deployed for {deployed_var_name} or {bid_var_name}: base variable(s) not found on model for {component_name} at t={t}.")
        ...
        if target_iso == 'SPP':
            if internal_service_name == 'RegUp': iso_specific_service_key = 'RegU'
            elif internal_service_name == 'RegDown': iso_specific_service_key = 'RegD'
            elif internal_service_name == 'SR': iso_specific_service_key = 'Spin'
            elif internal_service_name == 'NSR': iso_specific_service_key = 'Sup'
        elif target_iso == 'CAISO':
            if internal_service_name == 'RegUp': iso_specific_service_key = 'RegU'
            elif internal_service_name == 'RegDown': iso_specific_service_key = 'RegD'
            elif internal_service_name == 'SR': iso_specific_service_key = 'Spin'
            elif internal_service_name == 'NSR': iso_specific_service_key = 'NSpin'
            elif internal_service_name == 'RampUp': iso_specific_service_key = 'RMU'
            elif internal_service_name == 'RampDown': iso_specific_service_key = 'RMD'
            ...
            if internal_service_name == 'SR': iso_specific_service_key = 'Syn'
            elif internal_service_name == 'NSR': iso_specific_service_key = 'Rse'
            elif internal_service_name == 'ThirtyMin': iso_specific_service_key = 'TMR'
        elif target_iso == 'NYISO': 
            if internal_service_name == 'RegUp': iso_specific_service_key = 'RegU'
            elif internal_service_name == 'RegDown': iso_specific_service_key = 'RegD'
            if internal_service_name == 'SR': iso_specific_service_key = 'Spin10'
            elif internal_service_name == 'NSR': iso_specific_service_key = 'NSpin10'
            elif internal_service_name == 'ThirtyMin': iso_specific_service_key = 'Res30'
        elif target_iso == 'ISONE':
            if internal_service_name == 'SR': iso_specific_service_key = 'Spin10'
            elif internal_service_name == 'NSR': iso_specific_service_key = 'NSpin10'
            elif internal_service_name == 'ThirtyMin': iso_specific_service_key = 'OR30'
        elif target_iso == 'MISO':
            if internal_service_name == 'SR': iso_specific_service_key = 'Spin'
            elif internal_service_name == 'NSR': iso_specific_service_key = 'Sup'
            elif internal_service_name == 'ThirtyMin': iso_specific_service_key = 'STR'

        win_rate_param_name = f"winning_rate_{iso_specific_service_key}_{target_iso}"
        deploy_factor_param_name = f"deploy_factor_{iso_specific_service_key}_{target_iso}"

        is_service_actually_provided = False
        if target_iso in ACTUAL_ISO_SERVICES_PROVIDED and \
           iso_specific_service_key in ACTUAL_ISO_SERVICES_PROVIDED[target_iso]:
            is_service_actually_provided = True
        
        # Determine if the service is regulation or reserve for parameter checking
        is_regulation_service_flag = "RegU" in internal_service_name or \
                                     "RegD" in internal_service_name or \
                                     "RegUp" in internal_service_name or \
                                     "RegDown" in internal_service_name
        ...
        if is_regulation_service_flag:
            ...
            if hasattr(m, win_rate_param_name) and hasattr(m, deploy_factor_param_name):
                ...
                missing = []
                if not hasattr(m, win_rate_param_name): missing.append(f"'{win_rate_param_name}'")
                if not hasattr(m, deploy_factor_param_name): missing.append(f"'{deploy_factor_param_name}'")
...
def define_actual_electrolyzer_power_rule(m, t):
    ...
    try:
        ...
        logger.error(f"Error in define_actual_electrolyzer_power_rule @t={t}: {e}", exc_info=True)
        raise
