...
def format_aligned_section(items: dict, min_width: int = 50, indent: str = "  ") -> str:
    ...
    for name, value in items.items():
        result_lines.append(format_aligned_line(str(name), str(value), effective_width, indent))
...
def _calculate_case1_lcoe(annual_perf: dict, plant_params: dict, financial_metrics: dict) -> Optional[float]:
    ...
    try:
        ...
        remaining_lifetime = plant_params.get('remaining_plant_life_years') or financial_metrics.get('project_lifetime_years')
        ...
        if total_generation_pv > 0:
            ...
            logger.info(f"Case 1 LCOE calculated: ${lcoe:.2f}/MWh (OPEX-only over {remaining_lifetime} years)")
            return lcoe
        else:
            logger.warning("Cannot calculate LCOE: total generation PV is zero")
            return None
...
def generate_comprehensive_tea_summary_report(
    ...
    """
    logger.info(f"Generating comprehensive TEA summary report: {output_file_path}")
    ...
    try:
        ...
        with open(output_file_path, "w", encoding="utf-8") as f:
            ...
            f.write("=" * (len("Techno-Economic Analysis Report - ") + len(report_title)) + "\n\n")
            ...
            nuclear_baseline_analysis_rpt = annual_metrics_rpt.get("nuclear_baseline_analysis", {})
            _write_existing_reactor_baseline_section(
                f, nuclear_baseline_analysis_rpt 
            )
            
            nuclear_integrated_analysis_rpt = annual_metrics_rpt.get("nuclear_integrated_analysis", {})
            lcoh_breakdown_analysis_rpt = annual_metrics_rpt.get("lcoh_breakdown_analysis", {})
            ...
            greenfield_60yr_tax_analysis_main = annual_metrics_rpt.get("comprehensive_tax_incentive_analysis", {})
            _write_greenfield_60yr_analysis_section(
                f, greenfield_60yr_tax_analysis_main 
            )
            
            lifecycle_comparison_rpt = annual_metrics_rpt.get("lifecycle_comparison_analysis", {})
        ...
        logger.info(f"Comprehensive TEA summary report saved to {output_file_path}")
        return True
        
    except Exception as e:
        logger.error(f"Error generating comprehensive TEA summary report: {str(e)}", exc_info=True)
        return False
...
def _write_project_overview_section(
    ...
    nuclear_baseline_analysis_data = annual_metrics_rpt.get("nuclear_baseline_analysis", {})
    nuclear_baseline_fin_metrics = nuclear_baseline_analysis_data.get("financial_metrics", {})
    nuclear_baseline_plant_params = nuclear_baseline_analysis_data.get("plant_parameters", {})

    specific_lifetime_fm = nuclear_baseline_fin_metrics.get("project_lifetime_years")
    specific_lifetime_pp = nuclear_baseline_plant_params.get('remaining_plant_life_years') 
    ...
    nuclear_capacity_mw = nuclear_baseline_plant_params.get("turbine_capacity_mw")
    if nuclear_capacity_mw is None: 
        nuclear_capacity_mw = annual_metrics_rpt.get("Turbine_Capacity_MW")
    ...
    if nuclear_capacity_mw is not None and isinstance(nuclear_capacity_mw, (int, float)) and nuclear_capacity_mw > 0:
        overview_items["Nuclear Unit Capacity (MW)"] = _format_number(nuclear_capacity_mw, 2)
    
    if thermal_capacity_mwt is not None and isinstance(thermal_capacity_mwt, (int, float)) and thermal_capacity_mwt > 0:
        overview_items["Nuclear Unit Thermal Capacity (MWt)"] = _format_number(thermal_capacity_mwt, 2)
...
def _write_existing_reactor_baseline_section(f, nuclear_baseline_analysis_rpt: dict):
    ...
    f.write("    Analysis of an existing nuclear reactor's baseline operations without\n")
    f.write("    modifications for hydrogen production. This case evaluates the financial\n")
    f.write("    performance of the nuclear plant operating as-is, considering its\n")
    f.write("    remaining lifetime and typical operational parameters. It serves as a\n")
    f.write("    benchmark for comparison with retrofit or integrated system scenarios.\n\n")

    if not nuclear_baseline_analysis_rpt or not nuclear_baseline_analysis_rpt.get("plant_parameters"):
        f.write("  Status: Existing Reactor Baseline Analysis data not available or incomplete.\n\n")
        return
    ...
    scenario_no_45u = nuclear_baseline_analysis_rpt.get("scenario_without_45u", {})
    ...
    benefits_45u = nuclear_baseline_analysis_rpt.get("nuclear_45u_benefits", {})
    financial_metrics_no_45u = nuclear_baseline_analysis_rpt.get("financial_metrics", scenario_no_45u)


    f.write("  Key Assumptions:\n")
    remaining_lifetime_val = plant_params.get('remaining_plant_life_years', financial_metrics_no_45u.get('project_lifetime_years'))
    assumptions = {
    ...
    f.write(format_aligned_section(fin_metrics_no_45u_items, min_width=25, indent="    "))
    ...
    lcoe_value = _calculate_case1_lcoe(annual_perf, plant_params, financial_metrics_no_45u)
    lcoe_items = {
    ...
    if benefits_45u and benefits_45u.get('total_45u_credits', 0) > 0 :
        ...
        f.write(format_aligned_section(fin_metrics_45u_items, min_width=25, indent="      "))
        ...
        f.write(format_aligned_section(npv_improvement_items, min_width=25, indent="      "))
    else:
...
def _write_existing_reactor_retrofit_section(
    ...
    f.write("    production and battery storage systems. This case evaluates the combined\n")
    f.write("    financial and operational performance of the nuclear plant augmented with\n")
    ...
    f.write(format_aligned_section(fin_metrics_integrated_no_45U, min_width=30, indent="    "))

    if nuclear_integrated_analysis_rpt and nuclear_integrated_analysis_rpt.get("includes_45u_analysis"):
        scenario_with_45u = nuclear_integrated_analysis_rpt.get("scenario_with_45u", {})
        ...
        f.write(format_aligned_section(fin_metrics_integrated_with_45U, min_width=30, indent="    "))
    else:
    ...
    if lcoh_breakdown_analysis_rpt and "total_lcoh_usd_per_kg" in lcoh_breakdown_analysis_rpt:
        ...
        lcoh_breakdown = lcoh_breakdown_analysis_rpt.get("lcoh_breakdown_usd_per_kg", {})
        lcoh_percentages = lcoh_breakdown_analysis_rpt.get("lcoh_percentages", {})
        if lcoh_breakdown:
            sorted_components = sorted(lcoh_breakdown.items(), key=lambda item: item[1] if isinstance(item[1], (int,float)) else 0, reverse=True)
            for comp, cost_val in sorted_components:
                display_comp_name = comp.replace("CAPEX_", "").replace("_", " ").title()
                if isinstance(cost_val, (int,float)) and cost_val > 0.0001: 
                    f.write(f"      {display_comp_name:<40}: {_format_lcoh(cost_val)} ({_format_percentage(lcoh_percentages.get(comp,0))})\n")
    else:
    ...
    f.write(format_aligned_section(op_data_retrofit, min_width=35, indent="    "))
    f.write("\n")
...
def _write_lifecycle_cash_flow_analysis_section(f, lifecycle_comparison_rpt: dict, project_lifetime_years: int):
    ...
    if results_60yr and results_80yr:
        ...
        f.write(format_aligned_section(comparison_items, min_width=35, indent="    "))
        ...
        if cf_60yr and cf_80yr:
            ...
            f.write(format_aligned_section(cf_characteristics, min_width=35, indent="    "))

    # Compare with existing plant scenarios
    f.write(f"\n  C. Existing Plant vs New Construction Comparison:\n")
    f.write(f"    Existing Plant Remaining Life: {project_lifetime_years} years\n")
    ...
    f.write(f"    • 80-year lifecycle provides additional 20 years of revenue generation\n")
    f.write(f"    • Lifecycle extension significantly improves long-term financial metrics\n")
...
def _write_incremental_system_analysis_section(f, incremental_metrics_rpt: dict, annual_metrics_rpt: dict, tax_rate_rpt: float):
    ...
    f.write("    and battery systems, assuming they are added to an existing operational\n")
    f.write("    nuclear plant. This evaluates the standalone economics of the H2/battery\n")
    f.write("    project, considering its own CAPEX, revenues, and opportunity costs.\n\n")

    if not incremental_metrics_rpt:
        f.write("  Status: Incremental System Financial Analysis data not available.\n\n")
        return

    f.write("  Incremental Investment CAPEX:\n")
    inc_capex_total = incremental_metrics_rpt.get("Total_Incremental_CAPEX_Learned_USD", 0)
    f.write(f"    Total Incremental CAPEX: {_format_currency(inc_capex_total)}\n")
    inc_capex_breakdown = incremental_metrics_rpt.get("incremental_capex_breakdown", {})
    if inc_capex_breakdown:
        f.write("    CAPEX Breakdown:\n")
        sorted_capex_breakdown = sorted(inc_capex_breakdown.items(), key=lambda item: item[1] if isinstance(item[1], (int,float)) else 0, reverse=True)
        for comp, cost_val in sorted_capex_breakdown:
            ...
            if isinstance(cost_val, (int,float)) and cost_val > 0:
                 f.write(f"      {display_comp_name:<35}: {_format_currency(cost_val)} ({_format_percentage((cost_val / inc_capex_total * 100) if inc_capex_total else 0)})\n")
    ...
    f.write(format_aligned_section(inc_fin_metrics, min_width=30, indent="    "))

    f.write("\n  MACRS Depreciation Impact on Incremental Project:\n")
    inc_macrs_total_dep_arr = incremental_metrics_rpt.get("incremental_macrs_total_depreciation") 
    ...
    if inc_macrs_total_dep_arr is not None and isinstance(inc_macrs_total_dep_arr, (list, np.ndarray)) and len(inc_macrs_total_dep_arr)>0 :
        ...
        f.write(format_aligned_section(macrs_impact, min_width=45, indent="    ")) 
    else:
        f.write("    MACRS depreciation data not available or not applicable for incremental project.\n")
...
def _write_greenfield_60yr_analysis_section(f, greenfield_60yr_tax_analysis_main_dict: dict):
    ...
    f.write("-" * 90 + "\n")
    f.write("  Objective & Description:\n")
    f.write("    Analysis of a new, purpose-built nuclear-hydrogen integrated system with a\n")
    ...
    tax_analysis_content = greenfield_60yr_tax_analysis_main_dict.get("tax_incentive_analysis", {})
    ...
    system_config = greenfield_60yr_tax_analysis_main_dict.get("system_configuration", tax_analysis_content.get("system_configuration", {}))
    ...
    baseline_scenario_fin_metrics = scenarios.get("baseline", {}).get("financial_metrics", {}) 
    ...
    for scenario_key, scenario_name in [("baseline", "Baseline (No Incentives)"), 
        ...
        scenario_fin_metrics = scenarios.get(scenario_key, {}).get("financial_metrics", {})
        ...
        metrics_to_display["LCOH (Hydrogen)"] = _format_lcoh(scenario_fin_metrics.get("lcoh_usd_per_kg"))
        ...
        f.write(format_aligned_section(metrics_to_display, min_width=30, indent="      "))
        
        if scenario_key == "ptc":
            ptc_details = scenarios.get("ptc", {}).get("analysis", {}).get("tax_benefits", {}).get("ptc", {})
            ptc_value = ptc_details.get("total_ptc_value_usd", 0)
            f.write(f"        PTC Value Assessment: {_format_currency(ptc_value)} (Total over 10 years)\n")
        elif scenario_key == "itc":
            itc_details = scenarios.get("itc", {}).get("analysis", {}).get("tax_benefits", {}).get("itc", {})
            itc_credit = itc_details.get("itc_credit_amount_usd", 0)
            dep_reduction = itc_details.get("depreciation_basis_reduction_usd", 0)
            f.write(f"        ITC Value Assessment: {_format_currency(itc_credit)} (Upfront credit)\n")
            f.write(f"        ITC Impact on Depreciation Basis: -{_format_currency(dep_reduction)}\n")
        f.write("\n")
    ...
    best_scenario_name_map = {"baseline": "Baseline (No Incentives)", "ptc": "45Y PTC", "itc": "48E ITC"}
    f.write(f"    Recommended Scenario: {best_scenario_name_map.get(best_scenario_key, best_scenario_key)}\n")
    ...
    if best_scenario_key == "ptc" and ptc_npv_comp > baseline_npv_comp:
        findings.append(f"45Y PTC significantly improves NPV to {_format_currency(ptc_npv_comp)} from {_format_currency(baseline_npv_comp)} (Baseline).")
    elif best_scenario_key == "itc" and itc_npv_comp > baseline_npv_comp:
         findings.append(f"48E ITC significantly improves NPV to {_format_currency(itc_npv_comp)} from {_format_currency(baseline_npv_comp)} (Baseline).")
    elif best_scenario_key == "baseline" and baseline_npv_comp >= ptc_npv_comp and baseline_npv_comp >= itc_npv_comp:
        findings.append(f"Baseline scenario (NPV: {_format_currency(baseline_npv_comp)}) is currently the most favorable or equal to tax incentive scenarios.")
    else: 
        findings.append(f"Review NPVs: Baseline ({_format_currency(baseline_npv_comp)}), PTC ({_format_currency(ptc_npv_comp)}), ITC ({_format_currency(itc_npv_comp)}).")
...
def _write_greenfield_lifecycle_comparison_section(f, lifecycle_comparison_rpt: dict):
    ...
    f.write("    of federal tax incentives (Baseline, 45Y PTC, 48E ITC) on the financial\n")
    ...
    if data_missing_60yr or data_missing_80yr:
        ...
        if data_missing_60yr:
            f.write("          - Missing or incomplete data for 60-year lifecycle results.\n")
        if data_missing_80yr:
            f.write("          - Missing or incomplete data for 80-year lifecycle results.\n")
        f.write("          (Expected under 'lifecycle_comparison_analysis' -> '[60/80]_year_results' -> \n")
    ...
    header_parts = ["Lifecycle", "Tax Scenario", "NPV ($M)", "IRR (%)", "Payback (yrs)", "LCOH ($/kg)", "LCOE ($/MWh)", "LCOS ($/MWh)"]
    col_widths = [9, 12, 10, 7, 13, 11, 12, 12] 
    header_format = " | ".join([f"{{:<{w}}}" if i < 2 else f"{{:>{w}}}" for i, w in enumerate(col_widths)])
    ...
    for life_name, life_data_main in scenarios_data_map.items():
        ...
        baseline_greenfield_res_life = life_data_main.get("baseline_greenfield_results", {})
        # tax_scenarios_overall_life contains financial_metrics for baseline, ptc, itc tax cases
        tax_scenarios_overall_life = life_data_main.get("tax_incentive_analysis", {}).get("scenarios", {})
        ...
        for tax_key, tax_name in tax_scenario_map.items():
            ...
            current_scenario_fin_metrics = tax_scenarios_overall_life.get(tax_key, {}).get("financial_metrics", {})
            
            if not current_scenario_fin_metrics: continue 
            ...
            lcoh = current_scenario_fin_metrics.get("lcoh_usd_per_kg") # LCOH should be in each tax scenario's metrics
            ...
            row_values = [
                ...
                _format_number(payback, 1, default_na=True) if payback is not None else "N/A",
                _format_lcoh(lcoh) if lcoh is not None else "N/A",
                _format_lcoe_lcos(lcoe) if isinstance(lcoe, (int,float)) else lcoe,
                _format_lcoe_lcos(lcos) if isinstance(lcos, (int,float)) else lcos
            ]
...
def _write_detailed_performance_cost_section(f, annual_metrics_rpt: dict, tax_rate_rpt: float):
    ...
    if not annual_metrics_rpt:
        ...
        perf_indicator_lines = [
            ...
            ("  Total Nuclear Generation", f"{_format_number(annual_metrics_rpt.get('Annual_Nuclear_Generation_MWh'), 0)} MWh"),
            ("  Annual H2 Production", f"{_format_number(annual_metrics_rpt.get('H2_Production_kg_annual'), 0)} kg"),
            ("  Battery Annual Throughput", f"{_format_number(annual_metrics_rpt.get('Annual_Battery_Throughput_MWh', annual_metrics_rpt.get('Annual_Battery_Charge_MWh',0) + annual_metrics_rpt.get('Annual_Battery_Discharge_MWh',0)), 0)} MWh"),
            ("Capacity Factors", None),
            ("  Electrolyzer Capacity Factor", _format_percentage(annual_metrics_rpt.get("Electrolyzer_CF_percent"))),
            ("  Turbine Capacity Factor", _format_percentage(annual_metrics_rpt.get("Turbine_CF_percent"))),
            ("  Battery Capacity Factor", _format_percentage(annual_metrics_rpt.get("Battery_CF_percent"))),
            ("Average Prices", None),
            ("  Average Electricity Price", f"${_format_number(annual_metrics_rpt.get('Avg_Electricity_Price_USD_per_MWh'), 2)}/MWh"),
            ("  Average Hydrogen Price (if set)", f"${_format_number(annual_metrics_rpt.get('Avg_H2_Price_USD_per_kg'), 2)}/kg"),
            ("Ancillary Services", None),
            ("  Total AS Revenue", _format_currency(annual_metrics_rpt.get("AS_Revenue"))),
            ("Thermal Energy Utilization", None),
            ("  HTE Thermal Efficiency", _format_number(annual_metrics_rpt.get("thermal_efficiency", annual_metrics_rpt.get("Thermal_Efficiency")), 4)),
            ("  HTE Heat Opportunity Cost", _format_currency(annual_metrics_rpt.get("HTE_Heat_Opportunity_Cost_Annual_USD")))
        ]
        for name, value_str in perf_indicator_lines:
            if value_str is None: 
                 f.write(f"    {name}\n")
            else: 
                f.write(format_aligned_line(name.strip(), value_str, min_width=45, indent="    "))
    ...
    if total_capex_val is None and capex_breakdown: 
        total_capex_val = sum(v for v in capex_breakdown.values() if isinstance(v, (int,float)))
    ...
    if capex_breakdown and total_capex_val is not None and isinstance(total_capex_val, (int,float)) and total_capex_val > 0:
        sorted_capex_breakdown = sorted(capex_breakdown.items(), key=lambda item: item[1] if isinstance(item[1], (int,float)) else 0, reverse=True)
        for comp, cost_val in sorted_capex_breakdown:
            display_comp_name = comp.replace("_System", "").replace("_", " ")
            if isinstance(cost_val, (int, float)) and cost_val > 0:
                f.write(f"      {display_comp_name:<40}: {_format_currency(cost_val)} ({_format_percentage(cost_val / total_capex_val * 100)})\n")
        f.write(f"      {'Total CAPEX':<40}: {_format_currency(total_capex_val)}\n")
    else:
    ...
    total_annual_opex_calc = sum(v for v in opex_items_detail.values() if isinstance(v, (int, float)) and not np.isnan(v) and v is not None)
    ...
    for item, value in opex_items_detail.items():
        if value is not None and isinstance(value, (int, float)) and not np.isnan(value) and value != 0:
            ...
            has_opex_data = True
    if has_opex_data:
         f.write(f"      {'Calculated Total Annual OPEX':<40}: {_format_currency(total_annual_opex_calc)}\n")
         reported_total_opex = annual_metrics_rpt.get("Total_System_OPEX_Annual_USD", annual_metrics_rpt.get("Annual_Opex_Cost_from_Opt"))
         if reported_total_opex is not None:
             f.write(f"      {'Reported Total System OPEX':<40}: {_format_currency(reported_total_opex)}\n")
    else:
    ...
    if lcoh_analysis and "total_lcoh_usd_per_kg" in lcoh_analysis:
        ...
        lcoh_breakdown_detail = lcoh_analysis.get("lcoh_breakdown_usd_per_kg", {})
        lcoh_percentages_detail = lcoh_analysis.get("lcoh_percentages", {})
        if lcoh_breakdown_detail:
            sorted_lcoh_components = sorted(lcoh_breakdown_detail.items(), key=lambda item: item[1] if isinstance(item[1], (int,float)) else 0, reverse=True)
            for comp, cost_val in sorted_lcoh_components:
                display_comp_name = comp.replace("CAPEX_", "").replace("_", " ").title()
                if isinstance(cost_val, (int,float)) and cost_val > 0.0001:
                     f.write(f"        {display_comp_name:<40}: {_format_lcoh(cost_val)} ({_format_percentage(lcoh_percentages_detail.get(comp,0))})\n")
    else:
    ...
    if macrs_enabled:
        total_depreciation_arr = annual_metrics_rpt.get("macrs_total_depreciation", np.array([]))
        total_depreciation_sum = np.sum(total_depreciation_arr) if isinstance(total_depreciation_arr, (list, np.ndarray)) and len(total_depreciation_arr) > 0 else 0
        ...
        f.write(format_aligned_section(macrs_summary, min_width=45, indent="    "))

        component_macrs_dep = annual_metrics_rpt.get("macrs_component_depreciation", {})
        if component_macrs_dep:
            ...
            for comp_name, dep_array_val in component_macrs_dep.items():
                if isinstance(dep_array_val, (list, np.ndarray)):
                    ...
                    asset_depreciation[comp_name.replace("_", " ")] = _format_currency(comp_sum)
                else: 
                    asset_depreciation[comp_name.replace("_", " ")] = "N/A (Data error)"
            f.write(format_aligned_section(asset_depreciation, min_width=45, indent="      "))
    else:
        f.write("    MACRS Depreciation not enabled or data not available for this analysis.\n")
    f.write("\n")
...
def _write_core_assumptions_section(
    ...
    f.write(format_aligned_section(global_assumptions, min_width=40, indent="    "))
    ...
    if capex_data:
        ...
        for comp, details in capex_data.items():
            ...
            unit_suffix = ref_cap_key.split('_')[-1] if ref_cap_key and '_' in ref_cap_key else 'units'
        ...
        f.write(format_aligned_section(capex_assumptions, min_width=40, indent="    "))
    else:
    ...
    if om_data:
        ...
        for comp, details in om_data.items():
            ...
            if "base_cost_percent_of_capex" in details:
                cost_info_parts.append(f"{_format_percentage(details['base_cost_percent_of_capex']*100)} of CAPEX")
        ...
        f.write(format_aligned_section(om_assumptions, min_width=40, indent="    "))
    else:
    ...
    if replacement_data:
        ...
        for comp, details in replacement_data.items():
            ...
            replacement_assumptions[comp.replace("_", " ")] = f"{cost_str} in years: {years_str}"
        f.write(format_aligned_section(replacement_assumptions, min_width=40, indent="    "))
    else:
    ...
    fixed_om_val_am = annual_metrics_rpt.get('Nuclear_Fixed_OM_USD_per_MW_month')
    variable_om_val_am = annual_metrics_rpt.get('Nuclear_Variable_OM_USD_per_MWh')
    fuel_cost_val_am = annual_metrics_rpt.get('Nuclear_Fuel_Cost_USD_per_MWh')
    additional_costs_val_am = annual_metrics_rpt.get('Nuclear_Additional_Costs_USD_per_MW_year')
    ...
    if populated_from_annual_metrics:
        logger.debug("Using nuclear cost unit parameters from annual_metrics_rpt.")
        nuclear_cost_params_items["Nuclear Fixed O&M"] = f"{_try_format_currency(fixed_om_val_am, default_na=True)}/MW/month"
        nuclear_cost_params_items["Nuclear Variable O&M"] = f"{_try_format_number(variable_om_val_am, 2, default_na=True)}/MWh"
        nuclear_cost_params_items["Nuclear Fuel Cost"] = f"{_try_format_number(fuel_cost_val_am, 2, default_na=True)}/MWh"
        nuclear_cost_params_items["Nuclear Additional Costs"] = f"{_try_format_currency(additional_costs_val_am, default_na=True)}/MW/year"
        nuclear_cost_params_items["Nuclear CAPEX (if greenfield)"] = f"{_try_format_currency(nuclear_capex_val_am, default_na=True)}/MW" 
    else:
        logger.debug("Nuclear cost unit parameters not in annual_metrics_rpt, attempting to load from tea.config.")
        try:
            ...
            if hasattr(tea_config, 'NUCLEAR_COST_PARAMETERS'):
                ...
                nuclear_cost_params_items["Nuclear Fixed O&M"] = f"{_try_format_currency(opex_p.get('fixed_om_per_mw_month'), default_na=True)}/MW/month"
                nuclear_cost_params_items["Nuclear Variable O&M"] = f"{_try_format_number(opex_p.get('variable_om_per_mwh'), 2, default_na=True)}/MWh"
                nuclear_cost_params_items["Nuclear Fuel Cost"] = f"{_try_format_number(opex_p.get('fuel_cost_per_mwh'), 2, default_na=True)}/MWh"
                nuclear_cost_params_items["Nuclear Additional Costs"] = f"{_try_format_currency(opex_p.get('additional_costs_per_mw_year'), default_na=True)}/MW/year"
                nuclear_cost_params_items["Nuclear CAPEX (if greenfield)"] = f"{_try_format_currency(nc_params.get('nuclear_capex_per_mw'), default_na=True)}/MW"
            else:
            ...
            logger.warning("tea.config could not be imported for NUCLEAR_COST_PARAMETERS.")
            nuclear_cost_params_items["Note"] = "tea.config not found or NUCLEAR_COST_PARAMETERS missing."
            for key in ["Nuclear Fixed O&M", "Nuclear Variable O&M", "Nuclear Fuel Cost", "Nuclear Additional Costs", "Nuclear CAPEX (if greenfield)"]:
                nuclear_cost_params_items[key] = "N/A" 
    f.write(format_aligned_section(nuclear_cost_params_items, min_width=40, indent="    "))
    ...
    try:
        ...
        if hasattr(tea_config, 'MACRS_CONFIG'):
            ...
            for years_val, comps_list in sorted(years_to_comps.items()):
                if comps_list: 
                     macrs_classifications[f"{years_val}-year MACRS"] = ", ".join(comps_list)
        else:
    ...
    f.write(format_aligned_section(macrs_classifications, min_width=40, indent="    "))
    f.write("\n")
...
def generate_comprehensive_case_comparison_charts(
    ...
    try:
        ...
        case_data = _extract_case_data(annual_metrics_rpt, financial_metrics_rpt, incremental_metrics_rpt)

        # Generate individual chart types
        _create_financial_metrics_comparison(case_data, output_dir, target_iso_rpt, plant_specific_title_rpt)
        _create_cost_breakdown_comparison(case_data, output_dir, target_iso_rpt, plant_specific_title_rpt)
        _create_lcoh_lcoe_comparison(case_data, output_dir, target_iso_rpt, plant_specific_title_rpt)
        _create_capacity_utilization_comparison(case_data, output_dir, target_iso_rpt, plant_specific_title_rpt)
        _create_tax_incentive_impact_comparison(case_data, output_dir, target_iso_rpt, plant_specific_title_rpt)
        _create_lifecycle_comparison_chart(case_data, output_dir, target_iso_rpt, plant_specific_title_rpt)
        _create_comprehensive_dashboard(case_data, output_dir, target_iso_rpt, plant_specific_title_rpt)

        logger.info(f"Comprehensive case comparison charts saved to {output_dir}")
        return True

    except Exception as e:
        logger.error(f"Error generating comprehensive case comparison charts: {str(e)}", exc_info=True)
        return False
...
def _extract_case_data(annual_metrics_rpt: dict, financial_metrics_rpt: dict, incremental_metrics_rpt: dict) -> dict:
    ...
    lifecycle_comparison = annual_metrics_rpt.get("lifecycle_comparison_analysis", {})
    ...
    if case_data['case1'] and case_data['case1'].get('annual_generation', 0) > 0:
        ...
        case_data['case1']['lcoe'] = _calculate_case1_lcoe(annual_perf, plant_params, financial_metrics)
...
def _create_financial_metrics_comparison(case_data: dict, output_dir: Path, target_iso: str, plant_name: str):
    ...
    for i, (case_key, data) in enumerate(case_data.items()):
        if data:  # Only include cases with data
            ...
            paybacks.append(data.get('payback', 0) if data.get('payback', 0) > 0 else None)
    ...
    if valid_irrs:
        bars2 = ax2.bar(valid_cases_irr, valid_irrs, color=valid_colors_irr, alpha=0.8)
        ax2.set_ylabel('IRR (%)')
    ...
    valid_cases_pb = [case for case, pb in zip(cases, paybacks) if pb is not None and pb > 0]
    valid_colors_pb = [colors[i] for i, pb in enumerate(paybacks) if pb is not None and pb > 0]

    if valid_paybacks:
        bars3 = ax3.bar(valid_cases_pb, valid_paybacks, color=valid_colors_pb, alpha=0.8)
    ...
    if len(valid_irrs) >= 2:
        ...
        normalized_npv = [(npv - min(npvs)) / (max(npvs) - min(npvs)) * 100 if max(npvs) != min(npvs) else 50 for npv in npvs]
        normalized_irr = [irr / max(valid_irrs) * 100 if irr > 0 else 0 for irr in irrs]
        ...
        bars4 = ax4.bar(cases, performance_scores, color=colors[:len(cases)], alpha=0.8)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'financial_metrics_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def _create_cost_breakdown_comparison(case_data: dict, output_dir: Path, target_iso: str, plant_name: str):
    ...
    for case_key, data in case_data.items():
        if data and data.get('type') != 'existing':  # Skip Case 1 for CAPEX
            cases.append(data.get('name', case_key))
            capex_values.append(data.get('capex', 0) / 1e6)  # Convert to millions
            opex_values.append(data.get('annual_opex', 0) / 1e6)
            revenue_values.append(data.get('annual_revenue', 0) / 1e6)

    colors = ['#ff7f0e', '#2ca02c', '#d62728', '#9467bd']  # Skip blue for Case 1

    # CAPEX Comparison
    if capex_values:
        bars1 = ax1.bar(cases, capex_values, color=colors[:len(cases)], alpha=0.8)
    ...
    if opex_values:
        bars2 = ax2.bar(cases, opex_values, color=colors[:len(cases)], alpha=0.8)
        ax2.set_ylabel('Annual OPEX (Million USD)')
        ax2.set_title('Annual Operating Expenditure Comparison', fontweight='bold')
        ax2.tick_params(axis='x', rotation=45)
    ...
    if revenue_values:
        bars3 = ax3.bar(cases, revenue_values, color=colors[:len(cases)], alpha=0.8)
    ...
    if revenue_values and opex_values:
        ratios = [opex/rev if rev > 0 else 0 for opex, rev in zip(opex_values, revenue_values)]
    ...
    plt.suptitle(f'Cost Breakdown Comparison - {plant_name} ({target_iso})',
                fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig(output_dir / 'cost_breakdown_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def _create_lcoh_lcoe_comparison(case_data: dict, output_dir: Path, target_iso: str, plant_name: str):
    ...
    for i, (case_key, data) in enumerate(case_data.items()):
        if data:
            ...
            if data.get('lcoh', 0) > 0:
                ...
                h2_production.append(data.get('h2_production', 0) / 1e6)  # Convert to million kg
    ...
    if lcoh_values:
        ...
        ax1.set_title('Levelized Cost of Hydrogen Comparison', fontweight='bold')
        ax1.tick_params(axis='x', rotation=45)

        # Add DOE target line
        ax1.axhline(y=2.0, color='red', linestyle='--', alpha=0.7, label='DOE 2030 Target ($2/kg)')
        ax1.legend()
    ...
    if lcoe_values:
        ...
        ax2.set_title('Levelized Cost of Electricity Comparison', fontweight='bold')
        ax2.tick_params(axis='x', rotation=45)
    ...
    if h2_production:
        ...
        ax3.set_title('Annual Hydrogen Production Comparison', fontweight='bold')
        ax3.tick_params(axis='x', rotation=45)
    ...
    if lcoh_values and h2_production:
        efficiency = [prod/lcoh if lcoh > 0 else 0 for prod, lcoh in zip(h2_production, lcoh_values)]
    ...
    plt.suptitle(f'LCOH & LCOE Comparison - {plant_name} ({target_iso})',
                fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig(output_dir / 'lcoh_lcoe_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def _create_capacity_utilization_comparison(case_data: dict, output_dir: Path, target_iso: str, plant_name: str):
    ...
    for i, (case_key, data) in enumerate(case_data.items()):
        if data:
            ...
            nuclear_cf.append(data.get('capacity_factor', 0) * 100)  # Convert to percentage
    ...
    ax1.set_title('Nuclear Plant Capacity Factor Comparison', fontweight='bold')
    ax1.tick_params(axis='x', rotation=45)
    ax1.set_ylim(0, 100)
    ...
    valid_cases_elec = [case for case, cf in zip(cases, electrolyzer_cf) if cf > 0]
    valid_colors_elec = [colors[i] for i, cf in enumerate(electrolyzer_cf) if cf > 0]

    if valid_elec_cf:
        bars2 = ax2.bar(valid_cases_elec, valid_elec_cf, color=valid_colors_elec, alpha=0.8)
    ...
    elec_capacities = [cap['electrolyzer'] for cap in capacities if cap['electrolyzer'] > 0]
    elec_cases = [case for case, cap in zip(cases, capacities) if cap['electrolyzer'] > 0]
    elec_colors = [colors[i] for i, cap in enumerate(capacities) if cap['electrolyzer'] > 0]

    if elec_capacities:
        bars3 = ax3.bar(elec_cases, elec_capacities, color=elec_colors, alpha=0.8)
    ...
    if valid_elec_cf and elec_capacities:
        ...
        for case_key, data in case_data.items():
            if data and data.get('h2_production', 0) > 0 and data.get('electrolyzer_capacity', 0) > 0:
                prod_per_mw = data.get('h2_production', 0) / data.get('electrolyzer_capacity', 1) / 1000  # kg per kW
        ...
        if production_per_mw:
            prod_colors = [colors[i] for i, (_, data) in enumerate(case_data.items())
                          if data and data.get('h2_production', 0) > 0 and data.get('electrolyzer_capacity', 0) > 0]
            bars4 = ax4.bar(prod_cases, production_per_mw, color=prod_colors, alpha=0.8)
            ax4.set_ylabel('H2 Production per kW (kg/kW/year)')
            ax4.set_title('Electrolyzer Productivity Comparison', fontweight='bold')
            ax4.tick_params(axis='x', rotation=45)
    ...
    plt.suptitle(f'Capacity Utilization Comparison - {plant_name} ({target_iso})',
                fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig(output_dir / 'capacity_utilization_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def _create_tax_incentive_impact_comparison(case_data: dict, output_dir: Path, target_iso: str, plant_name: str):
    ...
    if case1_data:
        ...
        ax1.set_ylabel('NPV (Million USD)')
        ax1.set_title('Case 1: 45U PTC Impact on Existing Nuclear', fontweight='bold')
        ax1.axhline(y=0, color='black', linestyle='--', alpha=0.5)
    ...
    ax2.set_title('Cases 4 & 5: Federal Tax Incentive Impact', fontweight='bold')
    ...
    if lifetimes:
        ...
        bars3 = ax3.bar(lifetime_cases, lifetimes, color=colors_lifetime, alpha=0.8)
    ...
    if lifetimes and lifetime_npvs:
        npv_per_year = [npv/lifetime if lifetime > 0 else 0 for npv, lifetime in zip(lifetime_npvs, lifetimes)]
        bars4 = ax4.bar(lifetime_cases, npv_per_year, color=colors_lifetime, alpha=0.8)
    ...
    plt.suptitle(f'Tax Incentive & Lifetime Impact - {plant_name} ({target_iso})',
                fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig(output_dir / 'tax_incentive_impact_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def _create_lifecycle_comparison_chart(case_data: dict, output_dir: Path, target_iso: str, plant_name: str):
    ...
    if case4_data and case5_data:
        ...
        if all(lcoh > 0 for lcoh in lcoh_values):
            ...
            ax2.set_title('LCOH: 60-Year vs 80-Year Lifecycle', fontweight='bold')
            ax2.axhline(y=2.0, color='red', linestyle='--', alpha=0.7, label='DOE Target')
            ax2.legend()
        ...
        if all(lcoe > 0 for lcoe in lcoe_values):
            ...
            ax3.set_title('LCOE: 60-Year vs 80-Year Lifecycle', fontweight='bold')
        ...
        lcoh_improvement = lcoh_values[0] - lcoh_values[1] if all(lcoh > 0 for lcoh in lcoh_values) else 0

        benefits = ['NPV Improvement\n(80y vs 60y)', 'LCOH Reduction\n(60y vs 80y)']
        benefit_values = [npv_improvement, lcoh_improvement * 1000]  # Scale LCOH for visibility
        benefit_colors = ['green' if val > 0 else 'red' for val in benefit_values]

        bars4 = ax4.bar(benefits, benefit_values, color=benefit_colors, alpha=0.8)
    ...
    plt.suptitle(f'Lifecycle Comparison (60y vs 80y) - {plant_name} ({target_iso})',
                fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig(output_dir / 'lifecycle_comparison_chart.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def _create_comprehensive_dashboard(case_data: dict, output_dir: Path, target_iso: str, plant_name: str):
    ...
    for i, (case_key, data) in enumerate(case_data.items()):
        if data:
            cases.append(data.get('name', case_key).replace('Case ', '').replace(': ', ':\n'))
    ...
    bars1 = ax1.bar(range(len(cases)), npvs, color=colors[:len(cases)], alpha=0.8)
    ax1.set_ylabel('NPV (Million USD)')
    ax1.set_title('NPV Comparison Across All Cases', fontweight='bold', fontsize=12)
    ...
    valid_cases_lcoh = [case for case, lcoh in zip(cases, lcoh_values) if lcoh > 0]
    valid_colors_lcoh = [colors[i] for i, lcoh in enumerate(lcoh_values) if lcoh > 0]

    if valid_lcoh:
        bars2 = ax2.bar(range(len(valid_cases_lcoh)), valid_lcoh, color=valid_colors_lcoh, alpha=0.8)
        ax2.set_ylabel('LCOH (USD/kg)')
        ax2.set_title('LCOH Comparison', fontweight='bold', fontsize=12)
        ax2.set_xticks(range(len(valid_cases_lcoh)))
        ax2.set_xticklabels(valid_cases_lcoh, rotation=45, ha='right', fontsize=9)
        ax2.axhline(y=2.0, color='red', linestyle='--', alpha=0.7, label='DOE Target')
        ax2.legend(fontsize=8)
        ax2.grid(True, alpha=0.3)
    else:
        ax2.text(0.5, 0.5, 'No LCOH Data', ha='center', va='center', transform=ax2.transAxes)
        ax2.set_title('LCOH Comparison', fontweight='bold', fontsize=12)
    ...
    valid_colors_cap = [colors[i] for i, cap in enumerate(capacities) if cap > 0]

    if valid_cap:
        bars3 = ax3.bar(range(len(valid_cases_cap)), valid_cap, color=valid_colors_cap, alpha=0.8)
        ax3.set_ylabel('Electrolyzer Capacity (MW)')
        ax3.set_title('System Capacity', fontweight='bold', fontsize=12)
        ax3.set_xticks(range(len(valid_cases_cap)))
        ax3.set_xticklabels(valid_cases_cap, rotation=45, ha='right', fontsize=9)
        ax3.grid(True, alpha=0.3)
    else:
        ax3.text(0.5, 0.5, 'No Capacity Data', ha='center', va='center', transform=ax3.transAxes)
        ax3.set_title('System Capacity', fontweight='bold', fontsize=12)

    # 4. Project Lifetime (middle-left)
    ax4 = fig.add_subplot(gs[1, 0])
    bars4 = ax4.bar(range(len(cases)), lifetimes, color=colors[:len(cases)], alpha=0.8)
    ax4.set_ylabel('Project Lifetime (Years)')
    ax4.set_title('Project Lifetime Comparison', fontweight='bold', fontsize=12)
    ...
    if type_counts:
        ...
        ax5.text(0.5, 0.5, 'No Type Data', ha='center', va='center', transform=ax5.transAxes)
        ax5.set_title('Case Type Distribution', fontweight='bold', fontsize=12)
    ...
    for i, (case_key, data) in enumerate(case_data.items()):
        if data:
            ...
            irr_score = 1 if data.get('irr', 0) > 8 else 0  # Above discount rate
            payback_score = 1 if data.get('payback', 0) > 0 and data.get('payback', 0) < 10 else 0
            performance_data.append([npv_score, irr_score, payback_score])

    if performance_data:
        performance_array = np.array(performance_data)
        im = ax6.imshow(performance_array, cmap='RdYlGn', aspect='auto', vmin=0, vmax=1)

        ax6.set_xticks(range(3))
        ax6.set_xticklabels(['NPV > 0', 'IRR > 8%', 'Payback < 10y'], rotation=45, ha='right', fontsize=9)
        ax6.set_yticks(range(len(cases)))
        ax6.set_yticklabels(cases, fontsize=9)
        ax6.set_title('Financial Performance Matrix', fontweight='bold', fontsize=12)

        # Add text annotations
        for i in range(len(cases)):
            for j in range(3):
                text = '✓' if performance_array[i, j] == 1 else '✗'
                ax6.text(j, i, text, ha='center', va='center', fontweight='bold', fontsize=14)
    else:
        ax6.text(0.5, 0.5, 'No Performance Data', ha='center', va='center', transform=ax6.transAxes)
        ax6.set_title('Financial Performance Matrix', fontweight='bold', fontsize=12)
    ...
    table_data = []
    headers = ['Case', 'NPV ($M)', 'LCOH ($/kg)', 'LCOE ($/MWh)', 'Capacity (MW)', 'Lifetime (y)']
    ...
    for i, (case_key, data) in enumerate(case_data.items()):
        if data:
            row = [
                ...
                f"${data.get('lcoh', 0):.2f}/kg" if data.get('lcoh', 0) > 0 else 'N/A',
                f"${data.get('lcoe', 0):.1f}/MWh" if data.get('lcoe', 0) > 0 else 'N/A',
                f"{data.get('electrolyzer_capacity', 0):.0f} MW" if data.get('electrolyzer_capacity', 0) > 0 else 'N/A',
                f"{lifetimes[i]:.0f}y"
            ]
    ...
    ax7.set_title('Summary Statistics Table', fontweight='bold', fontsize=14, pad=20)
