...
def load_hourly_data(target_iso: str, base_dir: str | Path = "../input/hourly_data") -> Optional[Dict[str, pd.DataFrame]]:
    ...
    # Essential files
    required_files_spec = {
        "df_price_hourly": {"path": iso_path / "Price_hourly.csv", "index_col": None},
        "df_ANSprice_hourly": {"path": iso_path / "Price_ANS_hourly.csv", "index_col": None},
        "df_system": {"path": common_path / "sys_data_advanced.csv", "index_col": 0},
    }
    ...
    optional_files_spec = {
        "df_ANSmile_hourly": {"path": iso_path / "MileageMultiplier_hourly.csv", "index_col": None},
        "df_ANSdeploy_hourly": {"path": iso_path / "DeploymentFactor_hourly.csv", "index_col": None},
        "df_ANSwinrate_hourly": {"path": iso_path / "WinningRate_hourly.csv", "index_col": None},
    }
        ...
        try:
            ...
            if "_hourly" in key:
                if len(df) < HOURS_IN_YEAR:
                    logger.warning(f"{fpath.name} has {len(df)} rows (expected at least {HOURS_IN_YEAR}). This might be intended for shorter test runs.")
                elif len(df) > HOURS_IN_YEAR:
                    logger.warning(f"{fpath.name} has {len(df)} rows, truncating to {HOURS_IN_YEAR}.")
                    df = df.iloc[:HOURS_IN_YEAR]
            
            data[key] = df
            logger.info(f"Loaded required {key} ({len(df)} rows) from {fpath.name}")
        except Exception as e:
            logger.error(f"Failed to load or process required file {fpath}: {e}", exc_info=True)
            return None
    ...
    for key, spec in optional_files_spec.items():
        ...
        if fpath.exists():
            try:
                df = pd.read_csv(fpath, index_col=spec["index_col"])
                if df.empty:
                    logger.warning(f"Optional file {fpath} exists but loaded as empty DataFrame. Skipping.")
                    continue

                if "_hourly" in key: # All optional files here are hourly
                    if len(df) < HOURS_IN_YEAR:
                        logger.warning(f"{fpath.name} has {len(df)} rows (expected at least {HOURS_IN_YEAR}).")
                    elif len(df) > HOURS_IN_YEAR:
                        logger.warning(f"{fpath.name} has {len(df)} rows, truncating to {HOURS_IN_YEAR}.")
                        df = df.iloc[:HOURS_IN_YEAR]
                ...
                logger.warning(f"Failed to load or process optional file {fpath}, it will be skipped: {e}", exc_info=True)
        else:
