...
def load_hourly_data(target_iso: str, base_dir: str | Path = "../input/hourly_data") -> Optional[Dict[str, pd.DataFrame]]:
    ...
    * Requires standardized AS parameters in Price_ANS_hourly.csv (e.g., RegCap_*, RegPerf_*, SR_Price_*, NSR_Price_*, Loc_* etc.).
    ...
    required = {
        ...
        # Expects standardized columns now
        "df_ANSprice_hourly": iso_path / "Price_ANS_hourly.csv",
        "df_system": common_path / "sys_data_advanced.csv",  # Expects new system params
    }

    # Optional files
    optional_files = {
        # For performance/mileage factors
        "df_ANSmile_hourly": iso_path / "MileageMultiplier_hourly.csv",
        # For reserve deployment factors
        "df_ANSdeploy_hourly": iso_path / "DeploymentFactor_hourly.csv",
    ...
    for key, fpath in required.items():
        if not fpath.exists():
            ...
            return None
        try:
            df = pd.read_csv(fpath, index_col=0 if key ==
                             "df_system" else None)
            if df.empty:
            ...
            if "_hourly" in key and len(df) < HOURS_IN_YEAR:
                logger.warning("%s has %d rows (expected at least %d)",
                               fpath.name, len(df), HOURS_IN_YEAR)
            elif "_hourly" in key:
                # Truncate dataframe to HOURS_IN_YEAR if it's longer
                if len(df) > HOURS_IN_YEAR:
                    logger.warning("%s has %d rows, truncating to %d",
                                   fpath.name, len(df), HOURS_IN_YEAR)
                    df = df.iloc[:HOURS_IN_YEAR]
            data[key] = df
            logger.info("Loaded required %s (%d rows) from %s",
                        key, len(df), fpath.name)
    ...
    for key, fpath in optional_files.items():
        if fpath.exists():
            try:
                ...
                if df.empty:
                    logger.warning(
                        "Optional file %s exists but loaded as empty DataFrame.", fpath)
                    continue  # Skip if empty
                # Ensure hourly data has enough rows (allow for testing with fewer hours)
                if "_hourly" in key and len(df) < HOURS_IN_YEAR:
                    logger.warning(
                        "%s has %d rows (expected at least %d)", fpath.name, len(df), HOURS_IN_YEAR)
                elif "_hourly" in key:
                    # Truncate dataframe to HOURS_IN_YEAR if it's longer
                    if len(df) > HOURS_IN_YEAR:
                        logger.warning(
                            "%s has %d rows, truncating to %d", fpath.name, len(df), HOURS_IN_YEAR)
                        df = df.iloc[:HOURS_IN_YEAR]
                data[key] = df
                logger.info("Loaded optional %s (%d rows) from %s",
                            key, len(df), fpath.name)
            except Exception as e:
                logger.warning(
