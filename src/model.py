...
def get_sys_param(param_name, default=None, required=False):
    ...
    try:
        ...
        if pd.isna(val):
            if required:
                raise ValueError(f"Missing essential system parameter (NaN value): {param_name}")
            return default
        ...
        try:
            ...
            logger.debug(f"Parameter '{param_name}' value '{val}' not converted. Returning as is or default.")
            return val if default is None and not required else default
    except KeyError:
        if required:
            raise ValueError(f"Missing essential system parameter (KeyError): {param_name}")
        return default
...
def build_piecewise_constraints(model: pyo.ConcreteModel, *, component_prefix: str,
    ...
    if not hasattr(model, input_var_name):
        logger.error(f"Input variable '{input_var_name}' not found for PWL {component_prefix}.")
        return
    if not hasattr(model, output_var_name):
        logger.error(f"Output variable '{output_var_name}' not found for PWL {component_prefix}.")
        return
    if not hasattr(model, breakpoint_set_name):
        logger.error(f"Breakpoint set '{breakpoint_set_name}' not found for PWL {component_prefix}.")
        return
    ...
    if isinstance(value_param_name, str):
        if hasattr(model, value_param_name):
            ...
            logger.error(f"Value parameter/dict named '{value_param_name}' not found for PWL {component_prefix}.")
            return
    elif isinstance(value_param_name, (dict, pyo.Param)):
        value_data_source = value_param_name
    else:
        logger.error(f"Value source '{value_param_name}' for PWL {component_prefix} is invalid type.")
        return

    if not (isinstance(value_data_source, pyo.Param) or isinstance(value_data_source, dict)):
        logger.error(f"Value source for '{component_prefix}' is not a Pyomo Param or dict.")
        return

    breakpoint_set_to_use = breakpoint_set_orig
    if not breakpoint_set_orig.isordered():
        logger.warning(f"Breakpoint set {breakpoint_set_name} for {component_prefix} is not ordered. Sorting.")
        try:
            ...
            logger.error(f"Cannot sort breakpoint set {breakpoint_set_name} for {component_prefix}: {e}")
    ...
    if not hasattr(model, output_link_constr_name):
        def _output_link(m, t):
            ...
            logger.critical(f"Value source for {component_prefix} output link is invalid type.")
...
def create_model(data_inputs, target_iso: str, simulate_dispatch: bool) -> pyo.ConcreteModel:
    ...
    logger.info(f"  Nonlinear Turbine: {ENABLE_NONLINEAR_TURBINE_EFF and model.ENABLE_NUCLEAR_GENERATOR}")
    ...
    try:
        ...
        user_elec_cap_mw_str = get_sys_param('user_specified_electrolyzer_capacity_MW', default=None)
        user_batt_power_mw_str = get_sys_param('user_specified_battery_power_MW', default=None)
        user_batt_energy_mwh_str = get_sys_param('user_specified_battery_energy_MWh', default=None)
        ...
        if user_elec_cap_mw_str is not None:
            try:
                ...
                if user_elec_cap_mw < 0:
                    logger.warning("User specified electrolyzer capacity negative. Optimizing.")
                    user_elec_cap_mw = None
            except ValueError:
                logger.warning(f"Invalid user_specified_electrolyzer_capacity_MW. Optimizing.")
        ...
        if user_batt_energy_mwh_str is not None:
            try:
                ...
                if user_batt_energy_mwh < 0:
                    logger.warning("User specified battery energy negative. Optimizing.")
                    user_batt_energy_mwh = None
            except ValueError:
                logger.warning(f"Invalid user_specified_battery_energy_MWh. Optimizing.")
        ...
        if user_batt_power_mw is not None and user_batt_energy_mwh is not None:
            ...
            logger.warning("Battery power and energy must BOTH be specified to fix. Optimizing.")
            user_batt_power_mw = None
            user_batt_energy_mwh = None
        ...
        model.HOURS_IN_YEAR = pyo.Param(within=pyo.PositiveReals, initialize=HOURS_IN_YEAR)
            ...
            if ENABLE_NONLINEAR_TURBINE_EFF:
                try:
                    ...
                    if len(q_breakpoints) != len(p_values):
                        raise ValueError("Turbine steam breakpoints and power output lists have different lengths.")
                    ...
                    logger.error(f"Error loading non-linear turbine piecewise data: {e}. Falling back to linear.")
                    ...
                    model.nonlinear_turbine_enabled_in_model = True # Still uses PWL, just linear
        ...
        if model.ENABLE_ELECTROLYZER:
            ...
            model.hydrogen_subsidy_per_kg = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param('hydrogen_subsidy_value_usd_per_kg', 0.0))
            ...
            if user_elec_cap_mw is None: # Optimizing capacity
                model.pElectrolyzer_max_upper_bound = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param('pElectrolyzer_max_upper_bound_MW', required=True))
                model.pElectrolyzer_max_lower_bound = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param('pElectrolyzer_max_lower_bound_MW', 0.0))
            ...
            model.pElectrolyzer_min = pyo.Param(within=pyo.NonNegativeReals, initialize=p_elec_min_val) # This is the crucial pElectrolyzer_min param
            ...
            try:
                ...
                if not isinstance(p_elec_bps_str, str) or not isinstance(ke_vals_str, str):
                    raise TypeError(f"Elec breakpoint/ke data not string for {elec_type_suffix}.")
                ...
                if not p_elec_breakpoints:
                    raise ValueError(f"Elec power breakpoints list empty for {elec_type_suffix}.")
                if len(p_elec_breakpoints) != len(ke_values):
                    raise ValueError(f"Elec breakpoints/ke lengths differ for {elec_type_suffix}.")
                ...
                if not model.LTE_MODE:
                    ...
                    if not isinstance(kt_vals_str, str):
                        raise TypeError(f"HTE kt_H2 data not string for {elec_type_suffix}.")
                    kt_values = [float(x.strip()) for x in kt_vals_str.split(',') if x.strip()]
                    if len(p_elec_breakpoints) != len(kt_values):
                        raise ValueError(f"HTE breakpoints/kt lengths differ for {elec_type_suffix}.")
        ...
        if model.ENABLE_BATTERY:
            ...
            if not battery_capacity_fixed: # Optimizing capacity
                model.BatteryCapacity_max_param = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param('BatteryCapacity_max_MWh', required=True))
                model.BatteryCapacity_min_param = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param('BatteryCapacity_min_MWh', 0.0))
            ...
            batt_cyclic_val = True if str(batt_cyclic_val_str).strip().lower() == 'true' else False
            ...
            model.vom_battery_per_mwh_cycled = pyo.Param(within=pyo.NonNegativeReals, initialize=float(vom_batt_val) if vom_batt_val is not None else 0.0)
        ...
        if len(df_price) < nT:
            raise ValueError(f"Energy price data length mismatch ({len(df_price)} vs {nT}).")
        ...
        df_ANSmile = data_inputs.get('df_ANSmile_hourly', None) # For mileage_factor and performance_factor
        ...
        def get_hourly_param_from_df_model(t_idx, df, col_name, default=0.0, required_param=False):
            ...
            if col_name in df.columns:
                try:
                    ...
                    logger.warning(f"Index {t_idx - 1} out of bounds for '{col_name}' in {filename}. Defaulting.")
                    if required_param:
        ...
        logger.info(f"Loading AS parameters for {target_iso} using standardized naming...")
        ...
        if model.CAN_PROVIDE_ANCILLARY_SERVICES:
            for service_key_in_map in iso_service_map[target_iso]:
                ...
                is_regulation_service = "RegU" in service_key_in_map or "RegD" in service_key_in_map or \
                                        "RegUp" in service_key_in_map or "RegDown" in service_key_in_map
                if is_regulation_service:
                    # Mileage Factor (mileage_factor_*)
                    mileage_col_name_in_csv = f'mileage_factor_{csv_col_base}'
                    param_dict_mileage = {t: get_hourly_param_from_df_model(t, df_ANSmile, mileage_col_name_in_csv, default=1.0) for t in model.TimePeriods}
                    model.add_component(f'mileage_factor_{param_name_base_on_model}_{target_iso}', pyo.Param(model.TimePeriods, initialize=param_dict_mileage, within=pyo.NonNegativeReals))

                    # Performance Factor (performance_factor_*)
                    perf_col_name_in_csv = f'performance_factor_{csv_col_base}'
                    param_dict_perf = {t: get_hourly_param_from_df_model(t, df_ANSmile, perf_col_name_in_csv, default=1.0) for t in model.TimePeriods} # Assuming perf factors are also in df_ANSmile
                    model.add_component(f'performance_factor_{param_name_base_on_model}_{target_iso}', pyo.Param(model.TimePeriods, initialize=param_dict_perf, within=pyo.NonNegativeReals))
                else: # Reserve service
                    # Deploy Factor (deploy_factor_*)
                    deploy_col_name_in_csv = f'deploy_factor_{csv_col_base}'
                    param_dict_deploy = {t: get_hourly_param_from_df_model(t, df_ANSdeploy, deploy_col_name_in_csv, default=0.0) for t in model.TimePeriods}
                    model.add_component(f'deploy_factor_{param_name_base_on_model}_{target_iso}', pyo.Param(model.TimePeriods, initialize=param_dict_deploy, within=pyo.PercentFraction))
        else:
    ...
    try:
        ...
        if model.ENABLE_ELECTROLYZER:
            ...
            if user_elec_cap_mw is not None: # Fixed capacity
                if not hasattr(model, 'pElectrolyzer_max'):
                    model.pElectrolyzer_max = pyo.Param(within=pyo.NonNegativeReals, initialize=user_elec_cap_mw)
                    logger.info(f"Using fixed electrolyzer capacity: {user_elec_cap_mw} MW")
                ...
                if not hasattr(model, 'pElectrolyzer_max_lower_bound') or not hasattr(model, 'pElectrolyzer_max_upper_bound'):
                    raise ValueError("Electrolyzer capacity bounds parameters missing for optimization.")
                if not hasattr(model, 'pElectrolyzer_max'):
                    model.pElectrolyzer_max = pyo.Var(within=pyo.NonNegativeReals,
                                                      bounds=(pyo.value(model.pElectrolyzer_max_lower_bound), pyo.value(model.pElectrolyzer_max_upper_bound)),
                                                      initialize=pyo.value(model.pElectrolyzer_max_upper_bound)) # Initialize to upper bound
                    logger.info(f"Optimizing electrolyzer capacity between {pyo.value(model.pElectrolyzer_max_lower_bound)} and {pyo.value(model.pElectrolyzer_max_upper_bound)} MW")

            if not hasattr(model, 'pElectrolyzer_max'): # Should be defined by now
                raise AttributeError("Failed to define model.pElectrolyzer_max as Var or Param.")
            ...
            if user_elec_cap_mw is None: # pElectrolyzer_max is a Var
                if hasattr(model, 'pElectrolyzer_max') and hasattr(model, 'pElectrolyzer_min'):
                    def enforce_min_capacity_rule(m):
                        return m.pElectrolyzer_max >= m.pElectrolyzer_min
                    model.enforce_min_electrolyzer_capacity_constr = pyo.Constraint(rule=enforce_min_capacity_rule)
                    logger.info("Added constraint: pElectrolyzer_max >= pElectrolyzer_min for optimized capacity.")
                else: # Should not happen if logic is correct
                    logger.error("Could not add pElectrolyzer_max >= pElectrolyzer_min constraint: params/vars missing.")
            ...
            if model.ENABLE_H2_STORAGE:
                ...
                h2_discharge_max_val_loc = pyo.value(model.H2_storage_discharge_rate_max)
                ...
                model.H2_to_storage = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals, bounds=(0, h2_charge_max_val_loc))
        ...
        if model.ENABLE_BATTERY:
            if battery_capacity_fixed:
                if not hasattr(model, 'BatteryPower_MW'):
                    model.BatteryPower_MW = pyo.Param(within=pyo.NonNegativeReals, initialize=user_batt_power_mw)
                if not hasattr(model, 'BatteryCapacity_MWh'):
                    model.BatteryCapacity_MWh = pyo.Param(within=pyo.NonNegativeReals, initialize=user_batt_energy_mwh)
                logger.info(f"Using fixed battery power: {user_batt_power_mw} MW and energy: {user_batt_energy_mwh} MWh")
            else: # Optimizing
                if not hasattr(model, 'BatteryCapacity_min_param') or not hasattr(model, 'BatteryCapacity_max_param'):
                    raise ValueError("Battery capacity bounds parameters missing for optimization.")
                if not hasattr(model, 'BatteryCapacity_MWh'):
                    model.BatteryCapacity_MWh = pyo.Var(within=pyo.NonNegativeReals,
                                                        bounds=(pyo.value(model.BatteryCapacity_min_param), pyo.value(model.BatteryCapacity_max_param)),
                                                        initialize=(pyo.value(model.BatteryCapacity_min_param) + pyo.value(model.BatteryCapacity_max_param)) / 2)
                if not hasattr(model, 'BatteryPower_MW'):
                    model.BatteryPower_MW = pyo.Var(within=pyo.NonNegativeReals) # Linked to MWh by ratio constraint
                logger.info(f"Optimizing battery energy capacity (between {pyo.value(model.BatteryCapacity_min_param)}-{pyo.value(model.BatteryCapacity_max_param)} MWh) and power.")

            if not hasattr(model, 'BatteryPower_MW') or not hasattr(model, 'BatteryCapacity_MWh'):
                raise AttributeError("Failed to define battery power/capacity components.")
        ...
        as_service_list_internal = ['RegUp', 'RegDown', 'SR', 'NSR', 'ECRS',
                                    'ThirtyMin', 'RampUp', 'RampDown', 'UncU']
        ...
        if model.SIMULATE_AS_DISPATCH_EXECUTION and model.CAN_PROVIDE_ANCILLARY_SERVICES:
            ...
            components_providing_as_deployment = [] # Redefine for clarity in this scope
        ...
        if model.ENABLE_ELECTROLYZER:
            if hasattr(model, 'ke_H2_values') and hasattr(model, 'pElectrolyzer_efficiency_breakpoints'):
                if not list(model.pElectrolyzer_efficiency_breakpoints):
                    raise ValueError("pElectrolyzer_efficiency_breakpoints is empty for ke_H2_inv precomputation.")
            ...
            if not model.LTE_MODE: # HTE specific precomputation for steam consumption PWL
                if hasattr(model, 'kt_H2_values') and hasattr(model, 'ke_H2_inv_values') and hasattr(model, 'pElectrolyzer_efficiency_breakpoints'):
                    if not list(model.pElectrolyzer_efficiency_breakpoints):
                        raise ValueError("pElectrolyzer_efficiency_breakpoints is empty for qSteam_values precomputation.")
                    ...
                    logger.warning("kt_H2_values, ke_H2_inv_values, or breakpoints missing. Cannot calculate qSteam_values_at_pElec_bp for HTE.")
    ...
    try:
        ...
        if model.ENABLE_ELECTROLYZER and not hasattr(model, 'pElectrolyzer_max'):
            raise AttributeError("model.pElectrolyzer_max was not defined before constraints section.")
        if model.ENABLE_BATTERY and (not hasattr(model, 'BatteryPower_MW') or not hasattr(model, 'BatteryCapacity_MWh')):
            raise AttributeError("Battery power/capacity components were not defined before constraints section.")
        ...
        if model.ENABLE_NUCLEAR_GENERATOR:
            ...
            if model.ENABLE_ELECTROLYZER and model.LTE_MODE: # LTE mode fixes turbine output
        ...
        if model.ENABLE_ELECTROLYZER:
            ...
            if not model.LTE_MODE and hasattr(model, 'qSteam_values_at_pElec_bp'): # HTE steam consumption
                ...
                logger.warning("Cannot build SteamConsumption piecewise for HTE: qSteam_values_at_pElec_bp Param missing.")
            ...
            if not model.LTE_MODE and hasattr(model, 'qSteamElectrolyzerRampPos'): # HTE Steam Ramping
                def qSteam_ramp_linearization_rule(m, t):
                    ...
                    if not hasattr(m, 'qSteam_Electrolyzer'): return pyo.Constraint.Skip # Should exist for HTE
        ...
        if model.CAN_PROVIDE_ANCILLARY_SERVICES:
            ...
            if hasattr(model, 'Total_RampUp') and isinstance(model.Total_RampUp, pyo.Var):
                model.link_Total_RampUp_constr = pyo.Constraint(model.TimePeriods, rule=link_Total_RampUp_rule)
            if hasattr(model, 'Total_RampDown') and isinstance(model.Total_RampDown, pyo.Var):
                model.link_Total_RampDown_constr = pyo.Constraint(model.TimePeriods, rule=link_Total_RampDown_rule)
            if hasattr(model, 'Total_UncU') and isinstance(model.Total_UncU, pyo.Var):
                model.link_Total_UncU_constr = pyo.Constraint(model.TimePeriods, rule=link_Total_UncU_rule)
        ...
        if model.SIMULATE_AS_DISPATCH_EXECUTION and model.CAN_PROVIDE_ANCILLARY_SERVICES:
            ...
            components_with_deployed_vars = [] # Determine which components have deployed vars
            ...
            for comp_name_iter in components_with_deployed_vars:
                for service_internal_iter in as_service_list_internal:
                    ...
                    if hasattr(model, deployed_var_name_check): # Check if this specific deployed var exists
                        def _rule_factory_deployed_link(s_name, c_name): # Closure to capture s_name, c_name
                        ...
                        if not hasattr(model, constr_name_deployed): # Add if not exists
    ...
    try:
        ...
        def AnnualizedCapex_rule(m):
            ...
            try:
                ...
                hours_in_year_val = pyo.value(m.HOURS_IN_YEAR) # Use value from model param
                scaling_factor_for_period = total_hours_sim / hours_in_year_val if hours_in_year_val > 0 else 0
            except Exception as e:
        ...
        def TotalProfit_Objective_rule(m):
            try:
                ...
                capex_cost_for_period = getattr(m, 'AnnualizedCapexExpr', 0.0) # Cost for the simulation period
