...
# Assuming corrected constraints.py is used
...
# Import revenue rules (assuming corrected revenue_cost.py is used)
...
# --- HELPER FUNCTION (get_sys_param - assuming correct version from previous context) ---
    ...
    try:
        ...
        if any(indicator in param_name for indicator in ['MinUpTime', 'MinDownTime', 'initial_status', 'Lifetime_years', 'plant_lifetime_years', 'hours']):
            try: return int(float(val))
                ...
                if required: raise
                return default # Return default on conversion error if not required
        # String for breakpoint/value parameters
        ...
        if required: raise
        return default # Return default on unexpected error if not required

# --- Piecewise Helper (Local to model.py, improved version, n_segments removed) ---
def build_piecewise_constraints(model: pyo.ConcreteModel, *, component_prefix: str,
    ...
    # Handle value_param potentially being a precomputed dict or a Param name string
    value_data_source = None
    if isinstance(value_param_name, str): # If a name is provided
    ...
    else: value_data_source = value_param_name # Assume value_param_name is the dict/object itself
    ...
    # Ensure breakpoint set is ordered; if not, try to sort and replace on model
    ...
    if not hasattr(model, output_link_constr_name):
        def _output_link(m, t):
            ...
            return pyo.Constraint.Skip # Or raise
...
def create_model(data_inputs, target_iso: str, simulate_dispatch: bool) -> pyo.ConcreteModel:
    ...
    logger.info(f"  >> AS Simulation Mode: {'Dispatch Execution' if simulate_dispatch else 'Bidding Strategy'}")
    ...
    try:
        ...
        if ENABLE_NUCLEAR_GENERATOR:
            ...
            if ENABLE_NONLINEAR_TURBINE_EFF:
                 try:
                    ...
                    logger.error(f"Error loading non-linear turbine piecewise data: {e}. Falling back to constant efficiency representation.")
                    q_turb_min_val_fb = get_sys_param('qSteam_Turbine_min_MWth', required=True)
                    q_turb_max_val_fb = get_sys_param('qSteam_Turbine_max_MWth', required=True)
                    conv_const_fb = get_sys_param('Turbine_Thermal_Elec_Efficiency_Const', 0.4)
                    model.qTurbine_efficiency_breakpoints = pyo.Set(initialize=[q_turb_min_val_fb, q_turb_max_val_fb], ordered=True)
                    min_p_fallback = q_turb_min_val_fb * conv_const_fb; max_p_fallback = q_turb_max_val_fb * conv_const_fb
                    ...
                    model.nonlinear_turbine_enabled_in_model = True
                    logger.warning("Fell back to a linear piecewise representation for turbine efficiency.")
            if ENABLE_ELECTROLYZER and not model.LTE_MODE: model.Ramp_qSteam_Electrolyzer_limit = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param('Ramp_qSteam_Electrolyzer_limit_MWth_per_Hour', float('inf')))
            if ENABLE_ELECTROLYZER and model.LTE_MODE: model.pTurbine_LTE_setpoint = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param('pTurbine_LTE_setpoint_MW', p_turb_max_val, required=False))
        ...
        if ENABLE_ELECTROLYZER:
            ...
            try:
                ...
                else: kt_zero_dict = {bp: 0.0 for bp in p_elec_breakpoints}; model.kt_H2_values = pyo.Param(model.pElectrolyzer_efficiency_breakpoints, initialize=kt_zero_dict)
            ...
            if ENABLE_H2_CAP_FACTOR: model.h2_target_capacity_factor = pyo.Param(within=pyo.PercentFraction, initialize=get_sys_param('h2_target_capacity_factor_fraction', 0.0))
        ...
        if ENABLE_BATTERY:
            ...
            batt_cyclic_val = get_sys_param('BatteryRequireCyclicSOC', "True"); model.BatteryRequireCyclicSOC = pyo.Param(within=pyo.Boolean, initialize=bool(batt_cyclic_val))
        ...
        p_ies_min_default = -(p_turb_max_val) if ENABLE_NUCLEAR_GENERATOR and 'p_turb_max_val' in locals() else -1000.0
        p_ies_max_default = p_turb_max_val if ENABLE_NUCLEAR_GENERATOR and 'p_turb_max_val' in locals() else 1000.0
        model.pIES_min = pyo.Param(within=pyo.Reals, initialize=get_sys_param('pIES_min_MW', p_ies_min_default))
        model.pIES_max = pyo.Param(within=pyo.Reals, initialize=get_sys_param('pIES_max_MW', p_ies_max_default))
        ...
        def get_hourly_param_from_df(t_idx, df, col_name, default=0.0, required_param=False):
             """Helper to safely get data from optional hourly dataframes."""
             if df is None:
                 if required_param:
                     raise ValueError(f"Required data file for {col_name} not loaded.")
                 return default # File not found, not required
             filename = getattr(df, 'attrs', {}).get('filename', 'DataFrame') # Get filename for logging
             if col_name in df.columns:
                 try:
                     # Check index bounds before access
                     if not (0 <= t_idx - 1 < len(df)):
                         raise IndexError("Index out of bounds")
                     val = df[col_name].iloc[t_idx-1] # t_idx is 1-based from RangeSet
                     # Check for Pandas NA or Numpy NaN
                     # Use pd.isna as it handles more types including None, np.nan
                     return default if pd.isna(val) else val
                 except IndexError:
                      logger.warning(f"Index {t_idx-1} out of bounds for '{col_name}' in {filename} (len {len(df)}). Defaulting.")
                      if required_param:
                           raise ValueError(f"Index error accessing required parameter '{col_name}'.")
                      return default
                 except Exception as e_param:
                      logger.error(f"Error reading '{col_name}' @ index {t_idx-1} from {filename}: {e_param}")
                      if required_param:
                           raise # Re-raise if required
                      return default # Error reading, not required
                 ...
                 return default # Column not found, not required

        # --- (AS Param Loading Logic - Remains the same) ---
        iso_service_map = { # Copied from constraints.py for consistency check
        ...
        if CAN_PROVIDE_ANCILLARY_SERVICES:
            for service_key_in_map in iso_service_map[target_iso]:
                ...
                # Price (p_*) - Skip for factor-like keys
                if not is_factor_type_param:
                     price_col_name_in_csv = f'p_{csv_col_base}'; param_dict = {t: get_hourly_param_from_df(t, df_ANSprice, price_col_name_in_csv, default=0.0) for t in model.TimePeriods}
                ...
                deploy_col_name_in_csv = f'deploy_factor_{csv_col_base}'; param_dict = {t: get_hourly_param_from_df(t, df_ANSdeploy, deploy_col_name_in_csv, default=0.0) for t in model.TimePeriods}
                ...
                # Adder (loc_*)
                loc_col_name_in_csv = f'loc_{csv_col_base}'; param_dict = {t: get_hourly_param_from_df(t, df_ANSprice, loc_col_name_in_csv, default=0.0) for t in model.TimePeriods}
                ...
                win_col_name_in_csv = f'winning_rate_{csv_col_base}'; param_dict = {t: get_hourly_param_from_df(t, df_ANSwinrate, win_col_name_in_csv, default=1.0) for t in model.TimePeriods}
                ...
                if target_iso == 'CAISO' and service_key_in_map in ['RegU', 'RegD']:
                     mileage_col_name_in_csv = f'mileage_factor_{service_key_in_map}_{target_iso}'; param_dict = {t: get_hourly_param_from_df(t, df_ANSmile, mileage_col_name_in_csv, default=1.0) for t in model.TimePeriods}
                ...
                if target_iso == 'PJM':
                     if service_key_in_map == 'performance_score':
                          perf_col_name_in_csv = f'performance_score_{target_iso}'; param_dict = {t: get_hourly_param_from_df(t, df_ANSmile, perf_col_name_in_csv, default=1.0) for t in model.TimePeriods}
                     ...
                     if service_key_in_map == 'mileage_ratio':
                          mileage_col_name_in_csv = f'mileage_ratio_{target_iso}'; param_dict = {t: get_hourly_param_from_df(t, df_ANSmile, mileage_col_name_in_csv, default=1.0) for t in model.TimePeriods}
    ...
    # =========================================================================
    # VARIABLES (Logic remains the same)
    # =========================================================================
    logger.info("Defining variables...")
    try:
        # ... (Variable definitions remain the same as provided in the last model.py version) ...
        ...
        if ENABLE_ELECTROLYZER:
            ...
            if ENABLE_STARTUP_SHUTDOWN: model.uElectrolyzer = pyo.Var(model.TimePeriods, within=pyo.Binary); model.vElectrolyzerStartup = pyo.Var(model.TimePeriods, within=pyo.Binary); model.wElectrolyzerShutdown = pyo.Var(model.TimePeriods, within=pyo.Binary)
            if ENABLE_ELECTROLYZER_DEGRADATION_TRACKING: model.DegradationState = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals)
            if pyo.value(model.cost_electrolyzer_ramping) > 1e-9: model.pElectrolyzerRampPos = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals); model.pElectrolyzerRampNeg = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals)
            if not model.LTE_MODE and pyo.value(model.Ramp_qSteam_Electrolyzer_limit) < float('inf'): model.qSteamElectrolyzerRampPos = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals); model.qSteamElectrolyzerRampNeg = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals)
            if ENABLE_H2_STORAGE:
                 ...
                 model.H2_storage_level = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals, bounds=(h2_storage_min_val_loc, h2_storage_max_val_loc)); model.H2_to_storage = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals, bounds=(0, h2_charge_max_val_loc)); model.H2_from_storage = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals, bounds=(0, h2_discharge_max_val_loc)); model.H2_to_market = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals)
        if ENABLE_BATTERY:
            ...
            model.BatteryPower_MW = pyo.Var(within=pyo.NonNegativeReals); model.BatterySOC = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals)
            model.BatteryCharge = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals); model.BatteryDischarge = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals)
            model.BatteryBinaryCharge = pyo.Var(model.TimePeriods, within=pyo.Binary); model.BatteryBinaryDischarge = pyo.Var(model.TimePeriods, within=pyo.Binary)
        # AS Bids
        as_service_list_internal = ['RegUp', 'RegDown', 'SR', 'NSR', 'ECRS', 'ThirtyMin', 'RampUp', 'RampDown', 'UncU']
        ...
        if model.CAN_PROVIDE_ANCILLARY_SERVICES:
            ...
            if ENABLE_NUCLEAR_GENERATOR and (ENABLE_ELECTROLYZER or ENABLE_BATTERY): components_for_as_bids.append('Turbine')
            if ENABLE_ELECTROLYZER: components_for_as_bids.append('Electrolyzer')
            if ENABLE_BATTERY: components_for_as_bids.append('Battery')
            for comp_name in components_for_as_bids: model.component_as_bid_vars[comp_name] = []; [ (model.add_component(f"{s}_{comp_name}", pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals, initialize=0.0)), model.component_as_bid_vars[comp_name].append(f"{s}_{comp_name}")) for s in as_service_list_internal ]
            for s in as_service_list_internal: model.add_component(f"Total_{s}", pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals, initialize=0.0))
        else: logger.info("Ancillary Services disabled. Fixing AS Total bids to 0."); [ model.add_component(f"Total_{s}", pyo.Param(model.TimePeriods, initialize=0.0)) for s in as_service_list_internal ]
        # AS Deployed Vars
        ...
        if model.SIMULATE_AS_DISPATCH_EXECUTION and model.CAN_PROVIDE_ANCILLARY_SERVICES:
            ...
            if ENABLE_ELECTROLYZER: components_providing_as_deployment.append('Electrolyzer')
            if ENABLE_BATTERY: components_providing_as_deployment.append('Battery')
            if ENABLE_NUCLEAR_GENERATOR and (ENABLE_ELECTROLYZER or ENABLE_BATTERY): components_providing_as_deployment.append('Turbine')
            for comp_name in components_providing_as_deployment:
                 model.component_as_deployed_vars[comp_name] = []
                 for s in as_service_list_internal:
                    bid_var_name_check = f"{s}_{comp_name}"
                    if hasattr(model, bid_var_name_check) and isinstance(getattr(model, bid_var_name_check), pyo.Var):
                         deployed_var_name = f"{s}_{comp_name}_Deployed"; model.add_component(deployed_var_name, pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals, initialize=0.0)); model.component_as_deployed_vars[comp_name].append(deployed_var_name)
        ...
        if ENABLE_ELECTROLYZER:
            ...
            if not model.LTE_MODE: # HTE specific
                 if hasattr(model, 'kt_H2_values') and hasattr(model, 'ke_H2_inv_values') and hasattr(model, 'pElectrolyzer_efficiency_breakpoints'):
                      ...
                      q_steam_at_pElec_bp_dict = { p_bp: (pyo.value(model.kt_H2_values[p_bp]) * pyo.value(model.ke_H2_inv_values[p_bp]) * p_bp if abs(pyo.value(model.ke_H2_values[p_bp])) > 1e-9 else 0.0) for p_bp in model.pElectrolyzer_efficiency_breakpoints }
                      ...
                      if hasattr(model, 'pElectrolyzer_efficiency_breakpoints') and list(model.pElectrolyzer_efficiency_breakpoints): model.qSteam_values_at_pElec_bp = pyo.Param(model.pElectrolyzer_efficiency_breakpoints, initialize=0.0, default=0.0)
    ...
    try:
        ...
        if ENABLE_NUCLEAR_GENERATOR:
            ...
            # MODIFICATION: Corrected syntax for elif and else blocks
            elif ENABLE_NONLINEAR_TURBINE_EFF:
                 logger.warning("Nonlinear turbine enabled but PWL params missing/invalid. Using linear fallback constraint.")
            ...
            model.Turbine_RampUp_constr = pyo.Constraint(model.TimePeriods, rule=Turbine_RampUp_rule); model.Turbine_RampDown_constr = pyo.Constraint(model.TimePeriods, rule=Turbine_RampDown_rule)
            if CAN_PROVIDE_ANCILLARY_SERVICES: model.turbine_as_zero_constr = pyo.Constraint(model.TimePeriods, rule=Turbine_AS_Zero_rule); model.Turbine_AS_Pmax_constr = pyo.Constraint(model.TimePeriods, rule=Turbine_AS_Pmax_rule); model.Turbine_AS_Pmin_constr = pyo.Constraint(model.TimePeriods, rule=Turbine_AS_Pmin_rule); model.Turbine_AS_RU_constr = pyo.Constraint(model.TimePeriods, rule=Turbine_AS_RU_rule); model.Turbine_AS_RD_constr = pyo.Constraint(model.TimePeriods, rule=Turbine_AS_RD_rule)
            if ENABLE_ELECTROLYZER and model.LTE_MODE: model.const_turbine_power_constr = pyo.Constraint(model.TimePeriods, rule=constant_turbine_power_rule)
        # --- Electrolyzer Constraints ---
        if ENABLE_ELECTROLYZER:
            def electrolyzer_capacity_limit_rule(m, t): return m.pElectrolyzer[t] <= m.pElectrolyzer_max; model.electrolyzer_capacity_limit_constr = pyo.Constraint(model.TimePeriods, rule=electrolyzer_capacity_limit_rule)
            def electrolyzer_setpoint_capacity_limit_rule(m,t): return m.pElectrolyzerSetpoint[t] <= m.pElectrolyzer_max; model.electrolyzer_setpoint_capacity_limit_constr = pyo.Constraint(model.TimePeriods, rule=electrolyzer_setpoint_capacity_limit_rule)
            if hasattr(model, 'ke_H2_inv_values'): build_piecewise_constraints(model, component_prefix='HydrogenProduction', input_var_name='pElectrolyzer', output_var_name='mHydrogenProduced', breakpoint_set_name='pElectrolyzer_efficiency_breakpoints', value_param_name=model.ke_H2_inv_values)
            else: logger.error("Cannot build HydrogenProduction piecewise: ke_H2_inv_values dict missing.")
            if not model.LTE_MODE and hasattr(model, 'qSteam_values_at_pElec_bp'): build_piecewise_constraints(model, component_prefix='SteamConsumption', input_var_name='pElectrolyzer', output_var_name='qSteam_Electrolyzer', breakpoint_set_name='pElectrolyzer_efficiency_breakpoints', value_param_name='qSteam_values_at_pElec_bp')
            ...
            model.Electrolyzer_RampUp_constr = pyo.Constraint(model.TimePeriods, rule=Electrolyzer_RampUp_rule); model.Electrolyzer_RampDown_constr = pyo.Constraint(model.TimePeriods, rule=Electrolyzer_RampDown_rule)
            if not model.LTE_MODE and hasattr(model, 'qSteamElectrolyzerRampPos'):
                 ...
                 model.qSteam_ramp_linearization_constr = pyo.Constraint(model.TimePeriods, rule=qSteam_ramp_linearization_rule); model.Steam_Electrolyzer_Ramp_constr = pyo.Constraint(model.TimePeriods, rule=Steam_Electrolyzer_Ramp_rule)
            if model.CAN_PROVIDE_ANCILLARY_SERVICES: model.Electrolyzer_AS_Pmax_constr = pyo.Constraint(model.TimePeriods, rule=Electrolyzer_AS_Pmax_rule); model.Electrolyzer_AS_Pmin_constr = pyo.Constraint(model.TimePeriods, rule=Electrolyzer_AS_Pmin_rule); model.Electrolyzer_AS_RU_constr = pyo.Constraint(model.TimePeriods, rule=Electrolyzer_AS_RU_rule); model.Electrolyzer_AS_RD_constr = pyo.Constraint(model.TimePeriods, rule=Electrolyzer_AS_RD_rule)
            if ENABLE_STARTUP_SHUTDOWN: model.electrolyzer_on_off_logic_constr = pyo.Constraint(model.TimePeriods, rule=electrolyzer_on_off_logic_rule); model.electrolyzer_min_power_when_on_constr = pyo.Constraint(model.TimePeriods, rule=electrolyzer_min_power_when_on_rule); model.electrolyzer_max_power_constr = pyo.Constraint(model.TimePeriods, rule=electrolyzer_max_power_rule); model.electrolyzer_startup_shutdown_exclusivity_constr = pyo.Constraint(model.TimePeriods, rule=electrolyzer_startup_shutdown_exclusivity_rule); model.electrolyzer_min_uptime_constr = pyo.Constraint(model.TimePeriods, rule=electrolyzer_min_uptime_rule); model.electrolyzer_min_downtime_constr = pyo.Constraint(model.TimePeriods, rule=electrolyzer_min_downtime_rule)
            else: model.electrolyzer_min_power_sds_disabled_constr = pyo.Constraint(model.TimePeriods, rule=electrolyzer_min_power_sds_disabled_rule)
            ...
            if ENABLE_H2_STORAGE: model.h2_storage_balance_constr = pyo.Constraint(model.TimePeriods, rule=h2_storage_balance_adj_rule); model.h2_prod_dispatch_constr = pyo.Constraint(model.TimePeriods, rule=h2_prod_dispatch_rule); model.h2_storage_charge_limit_constr = pyo.Constraint(model.TimePeriods, rule=h2_storage_charge_limit_rule); model.h2_storage_discharge_limit_constr = pyo.Constraint(model.TimePeriods, rule=h2_storage_discharge_limit_rule); model.h2_storage_level_max_constr = pyo.Constraint(model.TimePeriods, rule=h2_storage_level_max_rule); model.h2_storage_level_min_constr = pyo.Constraint(model.TimePeriods, rule=h2_storage_level_min_rule)
            if model.SIMULATE_AS_DISPATCH_EXECUTION and model.CAN_PROVIDE_ANCILLARY_SERVICES: model.define_actual_electrolyzer_power_constr = pyo.Constraint(model.TimePeriods, rule=define_actual_electrolyzer_power_rule)
        # --- Battery Constraints ---
        if ENABLE_BATTERY:
            model.battery_soc_balance_constr = pyo.Constraint(model.TimePeriods, rule=battery_soc_balance_rule); model.battery_charge_limit_constr = pyo.Constraint(model.TimePeriods, rule=battery_charge_limit_rule); model.battery_discharge_limit_constr = pyo.Constraint(model.TimePeriods, rule=battery_discharge_limit_rule); model.battery_binary_exclusivity_constr = pyo.Constraint(model.TimePeriods, rule=battery_binary_exclusivity_rule); model.battery_soc_max_constr = pyo.Constraint(model.TimePeriods, rule=battery_soc_max_rule); model.battery_soc_min_constr = pyo.Constraint(model.TimePeriods, rule=battery_soc_min_rule); model.battery_charge_ramp_up_constr = pyo.Constraint(model.TimePeriods, rule=battery_ramp_up_rule); model.battery_charge_ramp_down_constr = pyo.Constraint(model.TimePeriods, rule=battery_ramp_down_rule); model.battery_discharge_ramp_up_constr = pyo.Constraint(model.TimePeriods, rule=battery_discharge_ramp_up_rule); model.battery_discharge_ramp_down_constr = pyo.Constraint(model.TimePeriods, rule=battery_discharge_ramp_down_rule)
            if pyo.value(model.BatteryRequireCyclicSOC): model.battery_cyclic_soc_lower_constr = pyo.Constraint(rule=battery_cyclic_soc_lower_rule); model.battery_cyclic_soc_upper_constr = pyo.Constraint(rule=battery_cyclic_soc_upper_rule)
            model.battery_power_capacity_link_constr = pyo.Constraint(rule=battery_power_capacity_link_rule); model.battery_min_cap_constr = pyo.Constraint(rule=battery_min_cap_rule)
            if model.CAN_PROVIDE_ANCILLARY_SERVICES: model.Battery_AS_Pmax_constr = pyo.Constraint(model.TimePeriods, rule=Battery_AS_Pmax_rule); model.Battery_AS_Pmin_constr = pyo.Constraint(model.TimePeriods, rule=Battery_AS_Pmin_rule); model.Battery_AS_SOC_Up_constr = pyo.Constraint(model.TimePeriods, rule=Battery_AS_SOC_Up_rule); model.Battery_AS_SOC_Down_constr = pyo.Constraint(model.TimePeriods, rule=Battery_AS_SOC_Down_rule); model.Battery_AS_RU_constr = pyo.Constraint(model.TimePeriods, rule=Battery_AS_RU_rule); model.Battery_AS_RD_constr = pyo.Constraint(model.TimePeriods, rule=Battery_AS_RD_rule)
        ...
        if model.CAN_PROVIDE_ANCILLARY_SERVICES:
            ...
            if hasattr(model, 'Total_30Min') and isinstance(model.Total_30Min, pyo.Var): model.link_Total_30Min_constr = pyo.Constraint(model.TimePeriods, rule=link_Total_30Min_rule)
        ...
        if model.SIMULATE_AS_DISPATCH_EXECUTION and model.CAN_PROVIDE_ANCILLARY_SERVICES:
            ...
            if ENABLE_ELECTROLYZER and model.component_as_deployed_vars.get('Electrolyzer'): components_with_deployed_vars.append('Electrolyzer')
            if ENABLE_BATTERY and model.component_as_deployed_vars.get('Battery'): components_with_deployed_vars.append('Battery')
            if ENABLE_NUCLEAR_GENERATOR and (ENABLE_ELECTROLYZER or ENABLE_BATTERY) and model.component_as_deployed_vars.get('Turbine'): components_with_deployed_vars.append('Turbine')
                ...
                for service_internal_iter in as_service_list_internal:
                    ...
                    if hasattr(model, deployed_var_name_check):
                        def _rule_factory_deployed_link(s_name, c_name):
                            def _actual_rule(m_inner, t_inner): return link_deployed_to_bid_rule(m_inner, t_inner, s_name, c_name); return _actual_rule
                        constr_name_deployed = f"link_deployed_{service_internal_iter}_{comp_name_iter}_constr"
                        model.add_component(constr_name_deployed, pyo.Constraint(model.TimePeriods, rule=_rule_factory_deployed_link(service_internal_iter, comp_name_iter)))

    except Exception as e: logger.error(f"Error during constraint definition: {e}", exc_info=True); raise

    # =========================================================================
    # OBJECTIVE FUNCTION (Corrected for Symbolic Definition)
    # =========================================================================
    logger.info("Defining objective function (Maximize Profit)...")
    try:
        # Revenue Expressions (Rules return symbolic sums)
        ...
        # Operational Costs (Rule returns symbolic sum)
        model.OpexCostExpr = pyo.Expression(rule=OpexCost_rule)

        # Capital Costs (Annualized - Uses symbolic Vars for capacity)
        def AnnualizedCapex_rule(m):
             """Calculate annualized CAPEX expression for the objective."""
             ...
             try:
                 ...
                 scaling_factor = total_hours_sim / 8760.0 if total_hours_sim > 0 else 0.0
                  ...
                  return 0.0 # Return zero expression on error

             # Electrolyzer Capex (Use symbolic m.pElectrolyzer_max)
             if m.ENABLE_ELECTROLYZER and hasattr(m, 'cost_electrolyzer_capacity') and hasattr(m, 'pElectrolyzer_max'):
                  cost_elec_cap_param = m.cost_electrolyzer_capacity # Param
                  total_annual_capex_expr += m.pElectrolyzer_max * cost_elec_cap_param * scaling_factor

             # Battery Capex (Use symbolic m.BatteryCapacity_MWh and m.BatteryPower_MW)
             if m.ENABLE_BATTERY and hasattr(m, 'BatteryCapacity_MWh') and hasattr(m, 'BatteryPower_MW'):
                 batt_cap_cost_param = m.BatteryCapex_USD_per_MWh_year # Param
                 batt_pow_cost_param = m.BatteryCapex_USD_per_MW_year # Param
                 batt_fom_cost_param = m.BatteryFixedOM_USD_per_MWh_year # Param

                 total_annual_capex_expr += (m.BatteryCapacity_MWh * batt_cap_cost_param +
                                             m.BatteryPower_MW * batt_pow_cost_param +
                                             m.BatteryCapacity_MWh * batt_fom_cost_param) * scaling_factor
             return total_annual_capex_expr
        model.AnnualizedCapexExpr = pyo.Expression(rule=AnnualizedCapex_rule)

        # Total Profit Calculation (Sums symbolic expressions)
