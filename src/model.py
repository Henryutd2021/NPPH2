...
def create_model(data_inputs, target_iso: str, simulate_dispatch: bool) -> pyo.ConcreteModel:
    ...
    try:
        ...
        if user_elec_cap_mw_str is not None:
            try:
                ...
                if user_elec_cap_mw < 0:
                    logger.warning(f"User specified electrolyzer capacity {user_elec_cap_mw} MW is negative. Will optimize capacity.")
                    user_elec_cap_mw = None # Revert to optimizing
            except ValueError:
                logger.warning(f"Invalid value for user_specified_electrolyzer_capacity_MW: '{user_elec_cap_mw_str}'. Will optimize capacity.")
        ...
        if user_batt_power_mw_str is not None:
            try:
                ...
                if user_batt_power_mw < 0:
                    logger.warning(f"User specified battery power {user_batt_power_mw} MW is negative. Will optimize capacity.")
                    user_batt_power_mw = None
            except ValueError:
                logger.warning(f"Invalid value for user_specified_battery_power_MW: '{user_batt_power_mw_str}'. Will optimize capacity.")
        ...
        if user_batt_energy_mwh_str is not None:
            try:
                ...
                if user_batt_energy_mwh < 0:
                    logger.warning(f"User specified battery energy {user_batt_energy_mwh} MWh is negative. Will optimize capacity.")
                    user_batt_energy_mwh = None
            except ValueError:
                logger.warning(f"Invalid value for user_specified_battery_energy_MWh: '{user_batt_energy_mwh_str}'. Will optimize capacity.")
        ...
        if user_batt_power_mw is not None and user_batt_energy_mwh is not None:
            ...
            logger.warning("Battery power and energy must BOTH be specified to fix battery capacity. Optimizing battery capacity instead.")
            user_batt_power_mw = None
            user_batt_energy_mwh = None
        ...
        model.HOURS_IN_YEAR = pyo.Param(within=pyo.PositiveReals, initialize=HOURS_IN_YEAR) # For TEA subsidy calc later
        ...
        if model.ENABLE_BATTERY: # Use model.attr
            ...
            if not battery_capacity_fixed: # Only load bounds if optimizing
                model.BatteryCapacity_max_param = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param('BatteryCapacity_max_MWh', required=True)) # Store original param
                model.BatteryCapacity_min_param = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param('BatteryCapacity_min_MWh', 0.0)) # Store original param
            ...
            batt_cyclic_val_str = get_sys_param('BatteryRequireCyclicSOC', "True") # Default to string "True"
            batt_cyclic_val = True if str(batt_cyclic_val_str).strip().lower() == 'true' else False # Ensure proper bool conversion
        ...
        def get_hourly_param_from_df_model(t_idx, df, col_name, default=0.0, required_param=False):
             ...
             if col_name in df.columns:
                 try:
                     ...
                     if not (0 <= t_idx - 1 < len(df)): raise IndexError(f"Index {t_idx-1} out of bounds")
                     val = df[col_name].iloc[t_idx-1]
                     # Check for NaN specifically, as it can cause issues later
                     return default if pd.isna(val) else float(val) # Convert to float for consistency
                 except IndexError:
        ...
        if model.ENABLE_ELECTROLYZER:
            ...
            if user_elec_cap_mw is not None:
                ...
                if not hasattr(model, 'pElectrolyzer_max_lower_bound') or not hasattr(model, 'pElectrolyzer_max_upper_bound'):
                     logger.error("Electrolyzer capacity bounds parameters missing while optimizing.")
                     raise ValueError("Electrolyzer capacity bounds parameters missing.")
            ...
            if hasattr(model, 'aux_power_consumption_per_kg_h2') and pyo.value(model.aux_power_consumption_per_kg_h2) > 1e-6 : model.pAuxiliary = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals, initialize=0.0)
            ...
            if hasattr(model, 'cost_electrolyzer_ramping') and pyo.value(model.cost_electrolyzer_ramping) > 1e-9: # Only if ramping has a cost
            ...
            if not model.LTE_MODE and hasattr(model, 'Ramp_qSteam_Electrolyzer_limit') and pyo.value(model.Ramp_qSteam_Electrolyzer_limit) < float('inf'): # HTE steam ramping vars
        ...
        if model.ENABLE_BATTERY:
            ...
            if battery_capacity_fixed:
                 ...
                 if not hasattr(model, 'BatteryCapacity_min_param') or not hasattr(model, 'BatteryCapacity_max_param'):
                      logger.error("Battery capacity bounds parameters missing while optimizing.")
                      raise ValueError("Battery capacity bounds parameters missing.")
        ...
        if model.SIMULATE_AS_DISPATCH_EXECUTION and model.CAN_PROVIDE_ANCILLARY_SERVICES:
            ...
            for comp_name in components_providing_as_deployment:
                 ...
                 for s_internal in as_service_list_internal:
                    ...
                    if iso == 'PJM':
                         if s_internal in ['RegUp', 'RegDown']: service_key_for_param = 'Reg'
                         if s_internal == 'SR': service_key_for_param = 'Syn'
                         if s_internal == 'NSR': service_key_for_param = 'Rse'
                         if s_internal == 'ThirtyMin': service_key_for_param = 'TMR'
                    elif iso == 'NYISO':
                         if s_internal in ['RegUp', 'RegDown']: service_key_for_param = 'RegC'
                         if s_internal == 'SR': service_key_for_param = 'Spin10'
                         if s_internal == 'NSR': service_key_for_param = 'NSpin10'
                         if s_internal == 'ThirtyMin': service_key_for_param = 'Res30'
                    elif iso == 'ISONE':
                         if s_internal == 'SR': service_key_for_param = 'Spin10'
                         if s_internal == 'NSR': service_key_for_param = 'NSpin10'
                         if s_internal == 'ThirtyMin': service_key_for_param = 'OR30'
                    elif iso == 'MISO':
                         if s_internal in ['RegUp', 'RegDown']: service_key_for_param = 'Reg'
                         if s_internal == 'NSR': service_key_for_param = 'Sup'
                         if s_internal == 'ThirtyMin': service_key_for_param = 'STR'
                    elif iso == 'CAISO':
                         if s_internal == 'SR': service_key_for_param = 'Spin'
                         if s_internal == 'NSR': service_key_for_param = 'NSpin'
                         if s_internal == 'RampUp': service_key_for_param = 'RMU'
                         if s_internal == 'RampDown': service_key_for_param = 'RMD'
                    elif iso == 'SPP':
                         if s_internal == 'SR': service_key_for_param = 'Spin'
                         if s_internal == 'NSR': service_key_for_param = 'Sup'
