...
def get_sys_param(param_name, default=None, required=False):
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                f"Parameter '{param_name}' value '{val}' not converted to numeric/bool. Returning as string or default.")
...
def build_piecewise_constraints(model: pyo.ConcreteModel, *, component_prefix: str,
    ...
    logger.info(
        "Building piece-wise constraints for %s using SOS2 (model.py local helper)", component_prefix)
    ...
    if isinstance(value_param_name, str):
        if hasattr(model, value_param_name):
            ...
            return
    # Allow passing dict or Param directly
    elif isinstance(value_param_name, (dict, pyo.Param)):
        value_data_source = value_param_name
    else:
        logger.error(
            f"Value source '{value_param_name}' for PWL {component_prefix} is not a string name, dict, or Pyomo Param.")
        return

    if not (isinstance(value_data_source, pyo.Param) or isinstance(value_data_source, dict)):
        logger.error(
            f"Value source for '{component_prefix}' (from '{value_param_name}') is not a Pyomo Param or dict.")
        return

    breakpoint_set_to_use = breakpoint_set_orig
    if not breakpoint_set_orig.isordered():
        logger.warning(
            f"Breakpoint set {breakpoint_set_name} for {component_prefix} is not ordered. Attempting to sort and replace.")
        try:
            ...
            if hasattr(model, ordered_set_attr_name):
                ...
                logger.info(
                    f"Using existing dynamically created ordered breakpoint set: {ordered_set_attr_name}")
            else:
                new_ordered_set = pyo.Set(
                    initialize=sorted_breakpoints_values, ordered=True, name=ordered_set_attr_name)
                setattr(model, ordered_set_attr_name, new_ordered_set)
                breakpoint_set_to_use = new_ordered_set
                logger.info(
                    f"Created and using new ordered breakpoint set: {ordered_set_attr_name} for {component_prefix}.")
        except Exception as e:
            logger.error(
                f"Cannot sort or replace breakpoint set {breakpoint_set_name} for {component_prefix}: {e}")
            raise ValueError(
                f"Breakpoint set {breakpoint_set_name} must be ordered for SOS2. Automatic sorting failed.")

    lam_var_name = f"lambda_{component_prefix}"
    if not hasattr(model, lam_var_name):
        lam = pyo.Var(model.TimePeriods, breakpoint_set_to_use,
    ...
    if not hasattr(model, sum_lambda_constr_name):
        ...
        model.add_component(sum_lambda_constr_name, pyo.Constraint(
            model.TimePeriods, rule=_sum_rule))

    if not hasattr(model, input_link_constr_name):
        def _input_link(m, t):
            return input_var[t] == sum(lam[t, bp] * bp for bp in breakpoint_set_to_use)
        model.add_component(input_link_constr_name, pyo.Constraint(
            model.TimePeriods, rule=_input_link))
    ...
    if not hasattr(model, output_link_constr_name):
        def _output_link(m, t):
            ...
            # Check if it's a dict (e.g., ke_H2_inv_values)
            ...
            logger.critical(
                f"Value source for {component_prefix} output link is invalid type in _output_link. This is unexpected.")
            return pyo.Constraint.Skip
        model.add_component(output_link_constr_name, pyo.Constraint(
            model.TimePeriods, rule=_output_link))

    if not hasattr(model, sos2_constr_name):
        def _sos2_rule(m, t):
            return [lam[t, bp] for bp in breakpoint_set_to_use]
        model.add_component(sos2_constr_name, pyo.SOSConstraint(
            model.TimePeriods, rule=_sos2_rule, sos=2))
...
def create_model(data_inputs, target_iso: str, simulate_dispatch: bool) -> pyo.ConcreteModel:
    ...
    try:
        if 'df_price_hourly' not in data_inputs or data_inputs['df_price_hourly'] is None:
            raise ValueError(
                "Essential data 'df_price_hourly' not found in data_inputs.")
        ...
        if 'df_system' not in data_inputs or data_inputs['df_system'] is None:
            raise ValueError(
                "Essential data 'df_system' not found in data_inputs.")
        global df_system
        df_system = data_inputs['df_system']

        # --- Fetch user-specified capacities ---
        ...
        if user_elec_cap_mw_str is not None:
            try:
                ...
                if user_elec_cap_mw < 0:
                    logger.warning(
                        f"User specified electrolyzer capacity {user_elec_cap_mw} MW is negative. Will optimize capacity.")
                    user_elec_cap_mw = None  # Revert to optimizing
            except ValueError:
                logger.warning(
                    f"Invalid value for user_specified_electrolyzer_capacity_MW: '{user_elec_cap_mw_str}'. Will optimize capacity.")
        ...
        if user_batt_power_mw_str is not None:
            try:
                ...
                if user_batt_power_mw < 0:
                    logger.warning(
                        f"User specified battery power {user_batt_power_mw} MW is negative. Will optimize capacity.")
                    user_batt_power_mw = None
            except ValueError:
                logger.warning(
                    f"Invalid value for user_specified_battery_power_MW: '{user_batt_power_mw_str}'. Will optimize capacity.")
        ...
        if user_batt_energy_mwh_str is not None:
            try:
                ...
                if user_batt_energy_mwh < 0:
                    logger.warning(
                        f"User specified battery energy {user_batt_energy_mwh} MWh is negative. Will optimize capacity.")
                    user_batt_energy_mwh = None
            except ValueError:
                logger.warning(
                    f"Invalid value for user_specified_battery_energy_MWh: '{user_batt_energy_mwh_str}'. Will optimize capacity.")
        ...
        elif user_batt_power_mw is not None or user_batt_energy_mwh is not None:  # Only one is specified
            logger.warning(
                "Battery power and energy must BOTH be specified to fix battery capacity. Optimizing battery capacity instead.")
            user_batt_power_mw = None
            user_batt_energy_mwh = None
        # --- End Fetch user-specified capacities ---

        model.delT_minutes = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
            'delT_minutes', 60.0, required=True))
        model.AS_Duration = pyo.Param(
            within=pyo.NonNegativeReals, initialize=get_sys_param('AS_Duration', 0.25))  # hours
        model.plant_lifetime_years = pyo.Param(
            within=pyo.PositiveIntegers, initialize=get_sys_param('plant_lifetime_years', 30))
        model.HOURS_IN_YEAR = pyo.Param(
            within=pyo.PositiveReals, initialize=HOURS_IN_YEAR)  # For TEA subsidy calc later

        if model.ENABLE_NUCLEAR_GENERATOR:  # Use model.attr for consistency
            model.qSteam_Total = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                'qSteam_Total_MWth', required=True))
            model.qSteam_Turbine_min = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                'qSteam_Turbine_min_MWth', required=True))
            model.qSteam_Turbine_max = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                'qSteam_Turbine_max_MWth', required=True))
            model.pTurbine_min = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                'pTurbine_min_MW', required=True))
            ...
            model.RU_Turbine_hourly = pyo.Param(
                within=pyo.NonNegativeReals, initialize=ramp_up_pct_min * 60 / 100 * p_turb_max_val)
            model.RD_Turbine_hourly = pyo.Param(
                within=pyo.NonNegativeReals, initialize=ramp_down_pct_min * 60 / 100 * p_turb_max_val)
            model.vom_turbine = pyo.Param(
                within=pyo.NonNegativeReals, initialize=get_sys_param('vom_turbine_USD_per_MWh', 0))
            model.convertTtE_const = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                'Turbine_Thermal_Elec_Efficiency_Const', 0.4))
            # Flag to track if PWL is successfully set up
            ...
            if ENABLE_NONLINEAR_TURBINE_EFF:  # Check global config flag for intent
                try:
                    ...
                    p_values = [float(x.strip())
                                for x in p_vals_str.split(',') if x.strip()]
                    ...
                    model.pTurbine_values_at_qTurbine_bp = pyo.Param(
                        model.qTurbine_efficiency_breakpoints, initialize=pTurbine_vals_at_qTurbine_bp_dict)
                    model.nonlinear_turbine_enabled_in_model = True
                    logger.info(
                        "Enabled non-linear turbine efficiency with provided breakpoints.")
                except Exception as e:
                    logger.error(
                        f"Error loading non-linear turbine piecewise data: {e}. Falling back to constant efficiency representation if this was intended.")
                    ...
                    conv_const_fb = get_sys_param(
                        'Turbine_Thermal_Elec_Efficiency_Const', 0.4)
                    model.qTurbine_efficiency_breakpoints = pyo.Set(
                        initialize=[q_turb_min_val_fb, q_turb_max_val_fb], ordered=True)
                    min_p_fallback = q_turb_min_val_fb * conv_const_fb
                    max_p_fallback = q_turb_max_val_fb * conv_const_fb
                    model.pTurbine_values_at_qTurbine_bp = pyo.Param(model.qTurbine_efficiency_breakpoints, initialize={
                                                                     q_turb_min_val_fb: min_p_fallback, q_turb_max_val_fb: max_p_fallback})
                    # Still uses PWL, just a linear one
                    model.nonlinear_turbine_enabled_in_model = True
                    logger.warning(
                        "Fell back to a linear piecewise representation for turbine efficiency using min/max points.")

            if model.ENABLE_ELECTROLYZER and not model.LTE_MODE:  # HTE needs steam ramp limit
                model.Ramp_qSteam_Electrolyzer_limit = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                    'Ramp_qSteam_Electrolyzer_limit_MWth_per_Hour', float('inf')))
            if model.ENABLE_ELECTROLYZER and model.LTE_MODE:  # LTE has fixed turbine setpoint
                model.pTurbine_LTE_setpoint = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                    'pTurbine_LTE_setpoint_MW', p_turb_max_val, required=False))
        ...
        if model.ENABLE_ELECTROLYZER:
            ...
            # Use 'hydrogen_subsidy_value_usd_per_kg' for the model parameter
            model.hydrogen_subsidy_per_kg = pyo.Param(
                within=pyo.NonNegativeReals, initialize=get_sys_param('hydrogen_subsidy_value_usd_per_kg', 0.0))
            # 'hydrogen_subsidy_duration_years' will be used by TEA, not directly as a model.TimePeriods-indexed param for an 8760h run

            model.aux_power_consumption_per_kg_h2 = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                'aux_power_consumption_per_kg_h2', 0.0))  # MW_aux per kg/hr H2

            # pElectrolyzer_max is now defined in VARIABLES section conditionally
            # Load bounds parameters here IF optimizing (user_elec_cap_mw is None)
            if user_elec_cap_mw is None:
                model.pElectrolyzer_max_upper_bound = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                    'pElectrolyzer_max_upper_bound_MW', required=True))
                model.pElectrolyzer_max_lower_bound = pyo.Param(
                    within=pyo.NonNegativeReals, initialize=get_sys_param('pElectrolyzer_max_lower_bound_MW', 0.0))

            p_elec_min_val = get_sys_param(f'pElectrolyzer_min_MW_{elec_type_suffix}', default=get_sys_param(
                'pElectrolyzer_min_MW', required=True))
            model.pElectrolyzer_min = pyo.Param(
                within=pyo.NonNegativeReals, initialize=p_elec_min_val)
            ramp_up_elec_pct_min = get_sys_param(f'Electrolyzer_RampUp_Rate_Percent_per_Min_{elec_type_suffix}', default=get_sys_param(
                'Electrolyzer_RampUp_Rate_Percent_per_Min', 10.0))
            ramp_down_elec_pct_min = get_sys_param(f'Electrolyzer_RampDown_Rate_Percent_per_Min_{elec_type_suffix}', default=get_sys_param(
                'Electrolyzer_RampDown_Rate_Percent_per_Min', 10.0))
            model.RU_Electrolyzer_percent_hourly = pyo.Param(
                within=pyo.NonNegativeReals, initialize=ramp_up_elec_pct_min * 60 / 100)  # Fraction of Pmax per hour
            model.RD_Electrolyzer_percent_hourly = pyo.Param(
                within=pyo.NonNegativeReals, initialize=ramp_down_elec_pct_min * 60 / 100)  # Fraction of Pmax per hour
            model.vom_electrolyzer = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                f'vom_electrolyzer_USD_per_MWh_{elec_type_suffix}', default=get_sys_param('vom_electrolyzer_USD_per_MWh', 0)))
            model.cost_water_per_kg_h2 = pyo.Param(
                within=pyo.NonNegativeReals, initialize=get_sys_param('cost_water_USD_per_kg_h2', 0))
            model.cost_electrolyzer_ramping = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                f'cost_electrolyzer_ramping_USD_per_MW_ramp_{elec_type_suffix}', default=get_sys_param('cost_electrolyzer_ramping_USD_per_MW_ramp', 0)))
            model.cost_electrolyzer_capacity = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                f'cost_electrolyzer_capacity_USD_per_MW_year_{elec_type_suffix}', default=get_sys_param('cost_electrolyzer_capacity_USD_per_MW_year', 0)))
            try:
                p_elec_bps_str = get_sys_param(f'pElectrolyzer_Breakpoints_MW_{elec_type_suffix}', default=get_sys_param(
                    'pElectrolyzer_Breakpoints_MW', required=True))
                ke_vals_str = get_sys_param(f'ke_H2_Values_MWh_per_kg_{elec_type_suffix}', default=get_sys_param(
                    'ke_H2_Values_MWh_per_kg', required=True))  # MWh_elec / kg_H2
                ...
                ke_values = [float(x.strip())
                             for x in ke_vals_str.split(',') if x.strip()]
                ...
                model.ke_H2_values = pyo.Param(
                    model.pElectrolyzer_efficiency_breakpoints, initialize=ke_vals_dict, within=pyo.NonNegativeReals)
                if not model.LTE_MODE:  # HTE specific
                    kt_vals_str = get_sys_param(f'kt_H2_Values_MWth_per_kg_{elec_type_suffix}', default=get_sys_param(
                        'kt_H2_Values_MWth_per_kg', required=True))  # MWth_steam / kg_H2
                    if not isinstance(kt_vals_str, str):
                        raise TypeError(
                            f"HTE kt_H2 data not string for {elec_type_suffix}.")
                    kt_values = [float(x.strip())
                                 for x in kt_vals_str.split(',') if x.strip()]
                    ...
                    model.kt_H2_values = pyo.Param(
                        model.pElectrolyzer_efficiency_breakpoints, initialize=kt_vals_dict, within=pyo.NonNegativeReals)
                else:  # LTE: kt is effectively zero
                    kt_zero_dict = {bp: 0.0 for bp in p_elec_breakpoints}
                    # Ensure it exists for HTE logic
                    model.kt_H2_values = pyo.Param(
                        model.pElectrolyzer_efficiency_breakpoints, initialize=kt_zero_dict)
                logger.info(
            ...
            if ENABLE_STARTUP_SHUTDOWN:  # Check global config flag
                model.cost_startup_electrolyzer = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                    f'cost_startup_electrolyzer_USD_per_startup_{elec_type_suffix}', default=get_sys_param('cost_startup_electrolyzer_USD_per_startup', 0)))
                model.MinUpTimeElectrolyzer = pyo.Param(within=pyo.PositiveIntegers, initialize=get_sys_param(
                    f'MinUpTimeElectrolyzer_hours_{elec_type_suffix}', default=get_sys_param('MinUpTimeElectrolyzer_hours', 1)))
                model.MinDownTimeElectrolyzer = pyo.Param(within=pyo.PositiveIntegers, initialize=get_sys_param(
                    f'MinDownTimeElectrolyzer_hours_{elec_type_suffix}', default=get_sys_param('MinDownTimeElectrolyzer_hours', 1)))
            ...
            if ENABLE_ELECTROLYZER_DEGRADATION_TRACKING:  # Check global config flag
                model.DegradationStateInitial = pyo.Param(
                    within=pyo.NonNegativeReals, initialize=get_sys_param('DegradationStateInitial_Units', 0.0))
                model.DegradationFactorOperation = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                    f'DegradationFactorOperation_Units_per_Hour_at_MaxLoad_{elec_type_suffix}', default=get_sys_param('DegradationFactorOperation_Units_per_Hour_at_MaxLoad', 0.0)))
                model.DegradationFactorStartup = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                    f'DegradationFactorStartup_Units_per_Startup_{elec_type_suffix}', default=get_sys_param('DegradationFactorStartup_Units_per_Startup', 0.0)))

            if ENABLE_H2_CAP_FACTOR:  # Check global config flag
                model.h2_target_capacity_factor = pyo.Param(within=pyo.PercentFraction, initialize=get_sys_param(
                    'h2_target_capacity_factor_fraction', 0.0))  # e.g., 0.9 for 90%

            model.H2_value = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                'H2_value_USD_per_kg', required=True))
            ...
            if model.ENABLE_H2_STORAGE:  # Use model.attr
                ...
                initial_level = max(h2_storage_min, min(
                    h2_storage_max, float(initial_level_raw)))
                model.H2_storage_level_initial = pyo.Param(
                    within=pyo.NonNegativeReals, initialize=initial_level)
                model.H2_storage_charge_rate_max = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                    'H2_storage_charge_rate_max_kg_per_hr', required=True))
                model.H2_storage_discharge_rate_max = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                    'H2_storage_discharge_rate_max_kg_per_hr', required=True))
                model.storage_charge_eff = pyo.Param(
                    within=pyo.PercentFraction, initialize=get_sys_param('storage_charge_eff_fraction', 1.0))
                model.storage_discharge_eff = pyo.Param(
                    within=pyo.PercentFraction, initialize=get_sys_param('storage_discharge_eff_fraction', 1.0))
                model.vom_storage_cycle = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
        ...
        if model.ENABLE_BATTERY:  # Use model.attr
            ...
            # BatteryCapacity_MWh and BatteryPower_MW defined in VARIABLES section conditionally
            # Load other battery parameters
            if not battery_capacity_fixed:  # Only load bounds if optimizing
                model.BatteryCapacity_max_param = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                    'BatteryCapacity_max_MWh', required=True))  # Store original param
                model.BatteryCapacity_min_param = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                    'BatteryCapacity_min_MWh', 0.0))  # Store original param

            model.BatteryPowerRatio = pyo.Param(within=pyo.NonNegativeReals, initialize=get_sys_param(
                'BatteryPowerRatio_MW_per_MWh', 0.25, required=True))  # MW/MWh
            model.BatteryChargeEff = pyo.Param(
                within=pyo.PercentFraction, initialize=get_sys_param('BatteryChargeEff', 0.95))
            model.BatteryDischargeEff = pyo.Param(
                within=pyo.PercentFraction, initialize=get_sys_param('BatteryDischargeEff', 0.95))
            model.BatterySOC_min_fraction = pyo.Param(
                within=pyo.PercentFraction, initialize=get_sys_param('BatterySOC_min_fraction', 0.10))
            model.BatterySOC_initial_fraction = pyo.Param(
                within=pyo.PercentFraction, initialize=get_sys_param('BatterySOC_initial_fraction', 0.50))
            batt_cyclic_val_str = get_sys_param(
                'BatteryRequireCyclicSOC', "True")  # Default to string "True"
            batt_cyclic_val = True if str(batt_cyclic_val_str).strip(
            ).lower() == 'true' else False  # Ensure proper bool conversion
            model.BatteryRequireCyclicSOC = pyo.Param(
                within=pyo.Boolean, initialize=batt_cyclic_val)
            model.BatteryRampRate = pyo.Param(within=pyo.PercentFraction, initialize=get_sys_param(
                'BatteryRampRate_fraction_per_hour', 1.0))  # Fraction of Capacity_MWh per hour
            model.BatteryCapex_USD_per_MWh_year = pyo.Param(
                within=pyo.NonNegativeReals, initialize=get_sys_param('BatteryCapex_USD_per_MWh_year', 0.0))
            model.BatteryCapex_USD_per_MW_year = pyo.Param(
                within=pyo.NonNegativeReals, initialize=get_sys_param('BatteryCapex_USD_per_MW_year', 0.0))
            model.BatteryFixedOM_USD_per_MWh_year = pyo.Param(
                within=pyo.NonNegativeReals, initialize=get_sys_param('BatteryFixedOM_USD_per_MWh_year', 0.0))
            # $/MWh cycled (charge+discharge)/2
            vom_batt_val = get_sys_param('vom_battery_per_mwh_cycled', None)
            # Ensure model.vom_battery_per_mwh_cycled exists even if None initially
            model.vom_battery_per_mwh_cycled = pyo.Param(within=pyo.NonNegativeReals, initialize=float(
                vom_batt_val) if vom_batt_val is not None else 0.0)

        # Grid Interaction Params
        p_ies_min_default_val = - \
            (p_turb_max_val) if model.ENABLE_NUCLEAR_GENERATOR and 'p_turb_max_val' in locals(
            ) else -1000.0
        p_ies_max_default_val = p_turb_max_val if model.ENABLE_NUCLEAR_GENERATOR and 'p_turb_max_val' in locals() else 1000.0
        model.pIES_min = pyo.Param(within=pyo.Reals, initialize=get_sys_param(
            'pIES_min_MW', p_ies_min_default_val))
        model.pIES_max = pyo.Param(within=pyo.Reals, initialize=get_sys_param(
            'pIES_max_MW', p_ies_max_default_val))
        ...
        energy_price_dict = {
            t_idx: df_price[energy_price_col].iloc[t_idx-1] for t_idx in model.TimePeriods}
        ...
        # Mileage, performance score
        df_ANSmile = data_inputs.get('df_ANSmile_hourly', None)
        df_ANSdeploy = data_inputs.get(
            'df_ANSdeploy_hourly', None)  # Deployment factor
        df_ANSwinrate = data_inputs.get(
            'df_ANSwinrate_hourly', None)  # Winning rate
        ...
        def get_hourly_param_from_df_model(t_idx, df, col_name, default=0.0, required_param=False):
            ...
            if col_name in df.columns:
                try:
                    # Use t_idx - 1 because iloc is 0-indexed and model.TimePeriods (RangeSet) is 1-indexed
                    if not (0 <= t_idx - 1 < len(df)):
                        raise IndexError(f"Index {t_idx-1} out of bounds")
                    val = df[col_name].iloc[t_idx-1]
                    # Check for NaN specifically, as it can cause issues later
                    # Convert to float for consistency
                    return default if pd.isna(val) else float(val)
                except IndexError:
                    logger.warning(
                        f"Index {t_idx-1} out of bounds for '{col_name}' in {filename} (len {len(df)}). Defaulting.")
                    if required_param:
                        raise ValueError(
        ...
        if model.CAN_PROVIDE_ANCILLARY_SERVICES:
            for service_key_in_map in iso_service_map[target_iso]:
                ...
                # e.g., RegU (used for param prefix)
                param_name_base_on_model = f"{service_key_in_map}"

                is_factor_type_param = any(f in service_key_in_map.lower() for f in [
                                           'factor', 'score', 'ratio'])
                ...
                if not is_factor_type_param or service_key_in_map in ['RegCap', 'RegPerf']:
                    ...
                    param_dict = {t: get_hourly_param_from_df_model(
                        t, df_ANSprice, price_col_name_in_csv, default=0.0) for t in model.TimePeriods}
                    model.add_component(f'p_{param_name_base_on_model}_{target_iso}', pyo.Param(
                        model.TimePeriods, initialize=param_dict, within=pyo.Reals))

                # Deploy Factor (deploy_factor_*) - default 0.0 if not applicable/found
                deploy_col_name_in_csv = f'deploy_factor_{csv_col_base}'
                param_dict = {t: get_hourly_param_from_df_model(
                    t, df_ANSdeploy, deploy_col_name_in_csv, default=0.0) for t in model.TimePeriods}
                model.add_component(f'deploy_factor_{param_name_base_on_model}_{target_iso}', pyo.Param(
                    model.TimePeriods, initialize=param_dict, within=pyo.PercentFraction))

                # Adder (loc_*) - default 0.0
                loc_col_name_in_csv = f'loc_{csv_col_base}'
                param_dict = {t: get_hourly_param_from_df_model(
                    t, df_ANSprice, loc_col_name_in_csv, default=0.0) for t in model.TimePeriods}
                model.add_component(f'loc_{param_name_base_on_model}_{target_iso}', pyo.Param(
                    model.TimePeriods, initialize=param_dict, within=pyo.Reals))

                # Winning Rate (winning_rate_*) - default 1.0 (assume 100% win if not specified)
                win_col_name_in_csv = f'winning_rate_{csv_col_base}'
                param_dict = {t: get_hourly_param_from_df_model(
                    t, df_ANSwinrate, win_col_name_in_csv, default=1.0) for t in model.TimePeriods}
                model.add_component(f'winning_rate_{param_name_base_on_model}_{target_iso}', pyo.Param(
                    model.TimePeriods, initialize=param_dict, within=pyo.PercentFraction))

                # Specific factors (mileage, performance)
                # CAISO has mileage_factor_RegU_CAISO etc.
                if target_iso == 'CAISO' and service_key_in_map in ['RegU', 'RegD']:
                    mileage_col_name_in_csv = f'mileage_factor_{service_key_in_map}_{target_iso}'
                    param_dict = {t: get_hourly_param_from_df_model(
                        t, df_ANSmile, mileage_col_name_in_csv, default=1.0) for t in model.TimePeriods}
                    model.add_component(f'mileage_factor_{service_key_in_map}_{target_iso}', pyo.Param(
                        model.TimePeriods, initialize=param_dict, within=pyo.NonNegativeReals))
                if target_iso == 'PJM':  # PJM has specific named factors
                    if service_key_in_map == 'performance_score':
                        # e.g. performance_score_PJM
                        perf_col_name_in_csv = f'performance_score_{target_iso}'
                        param_dict = {t: get_hourly_param_from_df_model(
                            t, df_ANSmile, perf_col_name_in_csv, default=1.0) for t in model.TimePeriods}
                        model.add_component(f'performance_score_{target_iso}', pyo.Param(
                            model.TimePeriods, initialize=param_dict, within=pyo.NonNegativeReals))
                    if service_key_in_map == 'mileage_ratio':
                        # e.g. mileage_ratio_PJM
                        mileage_col_name_in_csv = f'mileage_ratio_{target_iso}'
                        param_dict = {t: get_hourly_param_from_df_model(
                            t, df_ANSmile, mileage_col_name_in_csv, default=1.0) for t in model.TimePeriods}
                        model.add_component(f'mileage_ratio_{target_iso}', pyo.Param(
                            model.TimePeriods, initialize=param_dict, within=pyo.NonNegativeReals))
        else:
            logger.info(
                "Ancillary services disabled by configuration. Skipping AS parameter loading.")
    ...
    try:
        ...
        model.pIES = pyo.Var(model.TimePeriods, within=pyo.Reals,
        ...
        if model.ENABLE_NUCLEAR_GENERATOR:
            ...
            model.qSteam_Turbine = pyo.Var(
                model.TimePeriods, within=pyo.NonNegativeReals, bounds=(q_turb_min_val, q_turb_max_val))
            model.pTurbine = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals, bounds=(
                p_turb_min_val_loc, p_turb_max_val_loc))

        if model.ENABLE_ELECTROLYZER:
            # --- Define pElectrolyzer_max FIRST ---
            if user_elec_cap_mw is not None:
                # Define as Param if not already defined
                if not hasattr(model, 'pElectrolyzer_max'):
                    model.pElectrolyzer_max = pyo.Param(
                        within=pyo.NonNegativeReals, initialize=user_elec_cap_mw)
                    logger.info(
                        f"Using fixed electrolyzer capacity: {user_elec_cap_mw} MW")
            else:  # Optimizing
                # Bounds params should have been loaded earlier if optimizing
                if not hasattr(model, 'pElectrolyzer_max_lower_bound') or not hasattr(model, 'pElectrolyzer_max_upper_bound'):
                    logger.error(
                        "Electrolyzer capacity bounds parameters missing while optimizing.")
                    raise ValueError(
                        "Electrolyzer capacity bounds parameters missing.")
                # Define as Var if not already defined
                if not hasattr(model, 'pElectrolyzer_max'):
                    model.pElectrolyzer_max = pyo.Var(within=pyo.NonNegativeReals,
                                                      bounds=(pyo.value(model.pElectrolyzer_max_lower_bound), pyo.value(
            ...
            # --- NOW Define other electrolyzer Vars ---
            # Ensure pElectrolyzer_max exists before proceeding
            if not hasattr(model, 'pElectrolyzer_max'):
            ...
            model.mHydrogenProduced = pyo.Var(
                model.TimePeriods, within=pyo.NonNegativeReals)
            if hasattr(model, 'aux_power_consumption_per_kg_h2') and pyo.value(model.aux_power_consumption_per_kg_h2) > 1e-6:
                model.pAuxiliary = pyo.Var(
                    model.TimePeriods, within=pyo.NonNegativeReals, initialize=0.0)
            ...
            # Only if ramping has a cost
            if hasattr(model, 'cost_electrolyzer_ramping') and pyo.value(model.cost_electrolyzer_ramping) > 1e-9:
                model.pElectrolyzerRampPos = pyo.Var(
                    model.TimePeriods, within=pyo.NonNegativeReals)
                model.pElectrolyzerRampNeg = pyo.Var(
                    model.TimePeriods, within=pyo.NonNegativeReals)
            # HTE steam ramping vars
            if not model.LTE_MODE and hasattr(model, 'Ramp_qSteam_Electrolyzer_limit') and pyo.value(model.Ramp_qSteam_Electrolyzer_limit) < float('inf'):
            ...
            if model.ENABLE_H2_STORAGE:
                ...
                model.H2_storage_level = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals, bounds=(
                    h2_storage_min_val_loc, h2_storage_max_val_loc))
                model.H2_to_storage = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals, bounds=(
                    0, h2_charge_max_val_loc))  # kg/hr
                model.H2_from_storage = pyo.Var(model.TimePeriods, within=pyo.NonNegativeReals, bounds=(
                    0, h2_discharge_max_val_loc))  # kg/hr
                model.H2_to_market = pyo.Var(
                    model.TimePeriods, within=pyo.NonNegativeReals)  # kg/hr

        if model.ENABLE_BATTERY:
            # --- Define BatteryPower_MW and BatteryCapacity_MWh FIRST ---
            if battery_capacity_fixed:
                # Define as Params if not already defined
                if not hasattr(model, 'BatteryPower_MW'):
                    model.BatteryPower_MW = pyo.Param(
                        within=pyo.NonNegativeReals, initialize=user_batt_power_mw)
                if not hasattr(model, 'BatteryCapacity_MWh'):
                    model.BatteryCapacity_MWh = pyo.Param(
                        within=pyo.NonNegativeReals, initialize=user_batt_energy_mwh)
                logger.info(
                    f"Using fixed battery power: {user_batt_power_mw} MW and energy: {user_batt_energy_mwh} MWh")
            else:  # Optimizing
                # Bounds params should have been loaded earlier if optimizing
                if not hasattr(model, 'BatteryCapacity_min_param') or not hasattr(model, 'BatteryCapacity_max_param'):
                    logger.error(
                        "Battery capacity bounds parameters missing while optimizing.")
                    raise ValueError(
                        "Battery capacity bounds parameters missing.")
                # Define as Vars if not already defined
                if not hasattr(model, 'BatteryCapacity_MWh'):
                    model.BatteryCapacity_MWh = pyo.Var(within=pyo.NonNegativeReals,
                                                        bounds=(pyo.value(model.BatteryCapacity_min_param), pyo.value(
                                                            model.BatteryCapacity_max_param)),
                                                        initialize=(pyo.value(model.BatteryCapacity_min_param) + pyo.value(model.BatteryCapacity_max_param)) / 2)
                if not hasattr(model, 'BatteryPower_MW'):
            ...
            # --- NOW define other battery Vars ---
            # Ensure BatteryPower_MW and BatteryCapacity_MWh exist before proceeding
            if not hasattr(model, 'BatteryPower_MW') or not hasattr(model, 'BatteryCapacity_MWh'):
                raise AttributeError(
        ...
        as_service_list_internal = ['RegUp', 'RegDown', 'SR', 'NSR', 'ECRS',
                                    'ThirtyMin', 'RampUp', 'RampDown', 'UncU']  # Internal names
        ...
        if model.CAN_PROVIDE_ANCILLARY_SERVICES:
            ...
            for s_internal in as_service_list_internal:
                model.add_component(f"Total_{s_internal}", pyo.Param(
                    model.TimePeriods, initialize=0.0, within=pyo.NonNegativeReals))
        ...
        if model.SIMULATE_AS_DISPATCH_EXECUTION and model.CAN_PROVIDE_ANCILLARY_SERVICES:
            ...
            for comp_name in components_providing_as_deployment:
                ...
                for s_internal in as_service_list_internal:
                    ...
                    # --- Parameter Name Logic for Deployed Vars Link ---
                    # This maps internal service names to the keys used in the ISO service map for parameter lookup
                    service_key_for_param = s_internal  # Default assumption
                    if iso == 'PJM':
                        if s_internal in ['RegUp', 'RegDown']:
                            service_key_for_param = 'Reg'
                        if s_internal == 'SR':
                            service_key_for_param = 'Syn'
                        if s_internal == 'NSR':
                            service_key_for_param = 'Rse'
                        if s_internal == 'ThirtyMin':
                            service_key_for_param = 'TMR'
                    elif iso == 'NYISO':
                        if s_internal in ['RegUp', 'RegDown']:
                            service_key_for_param = 'RegC'
                        if s_internal == 'SR':
                            service_key_for_param = 'Spin10'
                        if s_internal == 'NSR':
                            service_key_for_param = 'NSpin10'
                        if s_internal == 'ThirtyMin':
                            service_key_for_param = 'Res30'
                    elif iso == 'ISONE':
                        if s_internal == 'SR':
                            service_key_for_param = 'Spin10'
                        if s_internal == 'NSR':
                            service_key_for_param = 'NSpin10'
                        if s_internal == 'ThirtyMin':
                            service_key_for_param = 'OR30'
                    elif iso == 'MISO':
                        if s_internal in ['RegUp', 'RegDown']:
                            service_key_for_param = 'Reg'
                        if s_internal == 'NSR':
                            service_key_for_param = 'Sup'
                        if s_internal == 'ThirtyMin':
                            service_key_for_param = 'STR'
                    elif iso == 'CAISO':
                        if s_internal == 'SR':
                            service_key_for_param = 'Spin'
                        if s_internal == 'NSR':
                            service_key_for_param = 'NSpin'
                        if s_internal == 'RampUp':
                            service_key_for_param = 'RMU'
                        if s_internal == 'RampDown':
                            service_key_for_param = 'RMD'
                    elif iso == 'SPP':
                        if s_internal == 'SR':
                            service_key_for_param = 'Spin'
                        if s_internal == 'NSR':
                            service_key_for_param = 'Sup'
                    # ERCOT names seem to match mostly (RegU, RegD, Spin, NSpin, ECRS)

                    win_param = f"winning_rate_{service_key_for_param}_{iso}"
                    deploy_param = f"deploy_factor_{service_key_for_param}_{iso}"
                    # --- End Parameter Name Logic ---
                    ...
                    if (hasattr(model, bid_var_name)
                        ...
                        if not hasattr(model, deployed_var_name):
                            model.add_component(
                                deployed_var_name,
                                pyo.Var(
                                    model.TimePeriods, within=pyo.NonNegativeReals, initialize=0.0)
                        ...
                        logger.debug(
                            ...
                            f"missing bid_var '{bid_var_name}', win_param '{win_param}', or deploy_param '{deploy_param}'"
                        )
    except Exception as e:
        ...
        if model.ENABLE_ELECTROLYZER:
            if hasattr(model, 'ke_H2_values') and hasattr(model, 'pElectrolyzer_efficiency_breakpoints'):
                if not list(model.pElectrolyzer_efficiency_breakpoints):
                    raise ValueError(
                        "Cannot precompute ke_H2_inv_values: pElectrolyzer_efficiency_breakpoints is empty.")
                # ke_H2_values is MWh_elec / kg_H2. Inverse is kg_H2 / MWh_elec
                model.ke_H2_inv_values = {bp: (1.0 / model.ke_H2_values[bp] if abs(pyo.value(
                    model.ke_H2_values[bp])) > 1e-9 else 1e9) for bp in model.pElectrolyzer_efficiency_breakpoints}
            else:
                raise ValueError(
                    "Missing ke_H2_values or pElectrolyzer_efficiency_breakpoints for precomputation of inverse efficiency.")
            ...
            if not model.LTE_MODE:  # HTE specific precomputation for steam consumption PWL
                if hasattr(model, 'kt_H2_values') and hasattr(model, 'ke_H2_inv_values') and hasattr(model, 'pElectrolyzer_efficiency_breakpoints'):
                    if not list(model.pElectrolyzer_efficiency_breakpoints):
                        raise ValueError(
                            "Cannot precompute qSteam_values_at_pElec_bp: pElectrolyzer_efficiency_breakpoints is empty.")
                    ...
                    q_steam_at_pElec_bp_dict = {
                        p_bp: (
                            p_bp * pyo.value(model.kt_H2_values[p_bp]) * pyo.value(model.ke_H2_inv_values[p_bp]))
                        for p_bp in model.pElectrolyzer_efficiency_breakpoints
                    }
                    model.qSteam_values_at_pElec_bp = pyo.Param(
                        model.pElectrolyzer_efficiency_breakpoints, initialize=q_steam_at_pElec_bp_dict)
                    logger.info(
                        "Calculated qSteam values at pElectrolyzer breakpoints for HTE.")
                else:
                    logger.warning(
                        "kt_H2_values, ke_H2_inv_values, or pElectrolyzer_efficiency_breakpoints missing. Cannot calculate qSteam_values_at_pElec_bp for HTE.")
                    if hasattr(model, 'pElectrolyzer_efficiency_breakpoints') and list(model.pElectrolyzer_efficiency_breakpoints):
                        model.qSteam_values_at_pElec_bp = pyo.Param(
                            model.pElectrolyzer_efficiency_breakpoints, initialize=0.0, default=0.0)  # Fallback
    except Exception as e:
    ...
    try:
        ...
        if model.ENABLE_BATTERY and (not hasattr(model, 'BatteryPower_MW') or not hasattr(model, 'BatteryCapacity_MWh')):
            raise AttributeError(
        ...
        if model.ENABLE_NUCLEAR_GENERATOR:
            ...
            if model.nonlinear_turbine_enabled_in_model and hasattr(model, 'pTurbine_values_at_qTurbine_bp'):
                ...
                def linear_pTurbine_rule(
                    m, t): return m.pTurbine[t] == m.qSteam_Turbine[t] * m.convertTtE_const
                model.linear_pTurbine_constr = pyo.Constraint(
                    model.TimePeriods, rule=linear_pTurbine_rule)
            # Linear efficiency by explicit choice (ENABLE_NONLINEAR_TURBINE_EFF is False)
            else:
                def linear_pTurbine_rule(
                    m, t): return m.pTurbine[t] == m.qSteam_Turbine[t] * m.convertTtE_const
                model.linear_pTurbine_constr = pyo.Constraint(
        ...
        if model.ENABLE_ELECTROLYZER:
            ...
            # model.pElectrolyzer_max can be a Var or Param now.
            def electrolyzer_setpoint_capacity_limit_rule(
                m, t): return m.pElectrolyzerSetpoint[t] <= m.pElectrolyzer_max
            model.electrolyzer_setpoint_capacity_limit_constr = pyo.Constraint(
                model.TimePeriods, rule=electrolyzer_setpoint_capacity_limit_rule)
            ...
            if ENABLE_STARTUP_SHUTDOWN:  # Use global config flag
                ...
                model.electrolyzer_max_power_constr = pyo.Constraint(
                    model.TimePeriods, rule=electrolyzer_max_power_rule)  # This links pElec to pElecMax * uElec
                model.electrolyzer_startup_shutdown_exclusivity_constr = pyo.Constraint(
                    model.TimePeriods, rule=electrolyzer_startup_shutdown_exclusivity_rule)
                ...
                # and pElectrolyzer <= pElectrolyzer_max (the optimized capacity or fixed param)
                model.electrolyzer_min_power_sds_disabled_constr = pyo.Constraint(
                    model.TimePeriods, rule=electrolyzer_min_power_sds_disabled_rule)

                def electrolyzer_max_power_sds_disabled_rule(
                    m, t): return m.pElectrolyzer[t] <= m.pElectrolyzer_max
                model.electrolyzer_max_power_sds_disabled_constr = pyo.Constraint(
                    model.TimePeriods, rule=electrolyzer_max_power_sds_disabled_rule)
            ...
            if hasattr(model, 'pElectrolyzerRampPos'):  # This var only exists if cost > 0
                ...
                model.electrolyzer_ramp_linearization_constr = pyo.Constraint(
                    model.TimePeriods, rule=electrolyzer_ramp_linearization_rule)
            ...
            if model.SIMULATE_AS_DISPATCH_EXECUTION:
                ...
                model.define_actual_electrolyzer_power_constr = pyo.Constraint(
                    model.TimePeriods, rule=define_actual_electrolyzer_power_rule)
            else:
                # --- ADDING NEW CONSTRAINT for non-dispatch simulation mode ---
                model.link_setpoint_to_actual_power_constr = pyo.Constraint(
                    model.TimePeriods, rule=link_setpoint_to_actual_power_if_not_simulating_dispatch_rule)
        ...
        if model.ENABLE_BATTERY:
            ...
            model.battery_charge_ramp_up_constr = pyo.Constraint(
                model.TimePeriods, rule=battery_ramp_up_rule)  # Charge ramp up
            model.battery_charge_ramp_down_constr = pyo.Constraint(
                model.TimePeriods, rule=battery_ramp_down_rule)  # Charge ramp down
            ...
            if hasattr(model, 'BatteryRequireCyclicSOC') and pyo.value(model.BatteryRequireCyclicSOC):
            ...
            # Conditional addition of power/capacity link constraint
            if not battery_capacity_fixed:  # Only add if optimizing
        ...
        if model.CAN_PROVIDE_ANCILLARY_SERVICES:
            ...
            if hasattr(model, 'Total_RegDown') and isinstance(model.Total_RegDown, pyo.Var):
            ...
            if hasattr(model, 'Total_ThirtyMin') and isinstance(model.Total_ThirtyMin, pyo.Var):
                model.link_Total_30Min_constr = pyo.Constraint(
                    model.TimePeriods, rule=link_Total_30Min_rule)  # Corrected rule name
            if hasattr(model, 'Total_RampUp') and isinstance(model.Total_RampUp, pyo.Var):
                model.link_Total_RampUp_constr = pyo.Constraint(
                    model.TimePeriods, rule=link_Total_RampUp_rule)
            if hasattr(model, 'Total_RampDown') and isinstance(model.Total_RampDown, pyo.Var):
        ...
        if model.SIMULATE_AS_DISPATCH_EXECUTION and model.CAN_PROVIDE_ANCILLARY_SERVICES:
            ...
            if model.ENABLE_ELECTROLYZER and model.component_as_deployed_vars.get('Electrolyzer'):
            ...
            for comp_name_iter in components_with_deployed_vars:
                for service_internal_iter in as_service_list_internal:  # Use internal service names
                    ...
                    if hasattr(model, deployed_var_name_check):
                        # Create a unique rule for each service and component combination
                        # s_name is internal service name
                        def _rule_factory_deployed_link(s_name, c_name):
                        ...
                        if not hasattr(model, constr_name_deployed):
    ...
    try:
        ...
        def AnnualizedCapex_rule(m):
            ...
            try:
                ...
                hours_in_year_val = pyo.value(m.HOURS_IN_YEAR)
                scaling_factor_for_period = total_hours_sim / \
                    hours_in_year_val if hours_in_year_val > 0 else 0
            ...
            if m.ENABLE_ELECTROLYZER and hasattr(m, 'cost_electrolyzer_capacity') and hasattr(m, 'pElectrolyzer_max'):
                ...
                # pElectrolyzer_max can be Var or Param
                total_annual_capex_expr += m.pElectrolyzer_max * \
                    cost_elec_cap_param_per_year * scaling_factor_for_period

            if m.ENABLE_BATTERY and hasattr(m, 'BatteryCapacity_MWh') and hasattr(m, 'BatteryPower_MW'):
                # BatteryCapacity_MWh and BatteryPower_MW can be Vars or Params
        ...
        def TotalProfit_Objective_rule(m):
            try:
                ...
                capex_cost_for_period = getattr(m, 'AnnualizedCapexExpr', 0.0)
                ...
                logger.critical(
                    f"Error defining TotalProfit_Objective_rule expression: {e}", exc_info=True)
                raise
