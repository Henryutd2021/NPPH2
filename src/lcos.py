...
def calculate_lcos(
    ...
    # --- Input Validation ---
    if not all([
        capex_per_kwh >= 0, capacity_mwh > 0, power_mw > 0,
        lifetime_years > 0, cycles_per_year >= 0,
        0 < dod_percent <= 100, 0 < rte_percent <= 100, fom_rate_per_kw_yr >= 0,
        avg_charging_price_per_mwh >= 0, discount_rate_percent >= 0,
        0 <= apply_itc_percent <= 100, other_vom_per_mwh_discharged >= 0
    ]):
    ...
    if total_pv_energy_discharged <= 0:
        print("Error: Total discounted energy discharged is zero or negative. Cannot calculate LCOS.")
        return None
...
if __name__ == "__main__":
    ...
    if calculated_lcos is not None:
        ...
        print("Differences are expected due to varying assumptions and model complexity.")
    else:
